# Spring 事务传播、隔离级别与一致性边界 的核心机制是什么？

## 面试定位

这道题关联 Spring 事务传播、隔离级别与一致性边界，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Spring 事务传播、隔离级别与一致性边界」切入：Spring 声明式事务要同时理解代理生效条件、传播行为、隔离级别、回滚规则、连接绑定和数据库锁/MVCC。
...
- 把所有 service 方法都加事务
- 事务里调用外部接口
- 不了解 checked exception 默认不回滚

## 30 秒回答

先给定义和边界：事务传播定义当前方法如何参与已有事务。；隔离级别定义并发事务之间的可见性。；一致性边界应优先落在本地数据库事务内。；Spring 声明式事务要同时理解代理生效条件、传播行为、隔离级别、回滚规则、连接绑定和数据库锁/MVCC。；传播行为决定事务加入或新建。
...
- 先给定义和边界：事务传播定义当前方法如何参与已有事务。；隔离级别定义并发事务之间的可见性。；一致性边界应优先落在本地数据库事务内。；Spring 声明式事务要同时理解代理生效条件、传播行为、隔离级别、回滚规则、连接绑定和数据库锁/MVCC。；传播行为决定事务加入或新建。
- 再讲机制：事务越短越稳定。；远程调用和本地事务不能假装一个原子动作。；回滚规则要和业务异常体系一致。；幂等和补偿是分布式一致性的基础。；REQUIRED、REQUIRES_NEW、NESTED 等传播行为会影响连接、保存点、提交和回滚边界。。
- 工程落地要说清楚：@Transactional propagation/isolation。；Outbox pattern。；Saga compensation。；Deadlock retry。；关键写接口通过唯一约束和幂等键保护重复提交。；捕获异常后如需回滚要重新抛出或显式标记 rollback-only。。
- 最后补指标、失败模式和取舍：transaction_duration_p95；rollback_rate；deadlock_count；lock_wait_time；connection_pool_usage；把所有 service 方法都加事务；事务里调用外部接口；不了解 checked exception 默认不回滚。
- Spring 声明式事务要同时理解代理生效条件、传播行为、隔离级别、回滚规则、连接绑定和数据库锁/MVCC。
- 事务传播定义当前方法如何参与已有事务。
- 隔离级别定义并发事务之间的可见性。
- 一致性边界应优先落在本地数据库事务内。
- 事务越短越稳定。
...
- 回滚规则要和业务异常体系一致。
- 幂等和补偿是分布式一致性的基础。
- REQUIRED、REQUIRES_NEW、NESTED 等传播行为会影响连接、保存点、提交和回滚边界。
- 隔离级别解决脏读、不可重复读、幻读等问题，但最终效果取决于数据库实现。
...
- Saga compensation。
- Deadlock retry。
- 关键写接口通过唯一约束和幂等键保护重复提交。
- 捕获异常后如需回滚要重新抛出或显式标记 rollback-only。
- 死锁和锁等待要记录 SQL、事务时长、行数和调用链。
- 长事务会放大锁等待、连接占用和死锁概率。
- 事务内避免远程调用、MQ 发送和不可回滚外部副作用，必要时用 Outbox/Saga 补偿。
...
### 追问 1：如果面试官深挖 Spring 事务传播、隔离级别与一致性边界 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：事务传播定义当前方法如何参与已有事务。；隔离级别定义并发事务之间的可见性。；一致性边界应优先落在本地数据库事务内。；Spring 声明式事务要同时理解代理生效条件、传播行为、隔离级别、回滚规则、连接绑定和数据库锁/MVCC。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：关键写接口通过唯一约束和幂等键保护重复提交。；捕获异常后如需回滚要重新抛出或显式标记 rollback-only。；死锁和锁等待要记录 SQL、事务时长、行数和调用链。；长事务会放大锁等待、连接占用和死锁概率。；事务内避免远程调用、MQ 发送和不可回滚外部副作用，必要时用 Outbox/Saga 补偿。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：transaction_duration_p95；rollback_rate；deadlock_count；lock_wait_time；connection_pool_usage。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 Spring 事务传播、隔离级别与一致性边界 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；长事务会放大锁等待、连接占用和死锁概率。；事务内避免远程调用、MQ 发送和不可回滚外部副作用，必要时用 Outbox/Saga 补偿。
...
- [Microservices.io: Transactional outbox pattern](https://microservices.io/patterns/data/transactional-outbox.html)：用于确认官方语义边界、命令行为和工程约束。
