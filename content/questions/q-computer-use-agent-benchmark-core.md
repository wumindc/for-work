# Computer Use Agent 和 Browser Agent 的工程差异是什么？

## 面试定位

这道题关联 Computer Use Agent 与 OSWorld，难度 4/5，出现频率 medium。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Computer Use Agent 与 OSWorld」切入：Computer Use Agent 把观察和动作扩展到真实桌面环境，评测重点从网页成功率扩展到跨应用任务完成和安全边界。
...
我会先划边界：Computer Use Agent 把观察和动作扩展到真实桌面环境，评测重点从网页成功率扩展到跨应用任务完成和安全边界。；真实桌面比浏览器更开放；benchmark 不等于生产可用；安全边界必须前置。
...
- 我会先划边界：Computer Use Agent 把观察和动作扩展到真实桌面环境，评测重点从网页成功率扩展到跨应用任务完成和安全边界。；真实桌面比浏览器更开放；benchmark 不等于生产可用；安全边界必须前置。
- 再讲运行机制：Agent S 和 OSWorld 类 benchmark 代表桌面操作 Agent 的发展方向。；桌面任务需要屏幕观察、窗口状态、文件系统、应用权限和动作恢复。。
- 工程实现要落到可执行设计：trace 字段包含 screenshot_ref、active_app、action_type、target_region、verifier_state、unsafe_action_blocked。；评测指标包括 task_success_rate、step_error_rate、recovery_rate、unsafe_action_rate。。
- 如果被要求画设计，可以按这个结构展开：。
- 最后补风险、指标和取舍：用 benchmark 分数替代产品安全评估；没有敏感操作确认；截图观察无法复盘；跟踪 task_success_rate；跟踪 step_error_rate；跟踪 recovery_rate；跟踪 unsafe_action_block_rate；跟踪 trace_replay_success_rate。
...
### 追问 1：如何评测一个能操作电脑的 Agent 是否可靠和安全？

**回答要点**：我会先回到适用边界，再解释机制和工程代价：。回答时不能只说概念，要把选择标准、失败模式和可观测指标一起讲出来。

**考察点**：边界判断、工程取舍

### 追问 2：如果把它落到 pe-web-agent 项目里，你会怎么设计？

**回答要点**：我会按输入、状态、执行、验证和指标展开：trace 字段包含 screenshot_ref、active_app、action_type、target_region、verifier_state、unsafe_action_blocked。；评测指标包括 task_success_rate、step_error_rate、recovery_rate、unsafe_action_rate。；观察层保存 screenshot、active_app、window_title、ocr_text 和可操作区域。；动作层限制 click/type/hotkey/file 操作，并对高风险动作加 confirmation。；每步后用 verifier 检查 UI 状态、文件变化或任务产物。；评测集覆盖跨应用、文件处理、网络失败、弹窗和权限拦截。。项目表达里要说明模块边界、数据流、错误恢复和上线后的指标，而不是只说用了某个框架。

**考察点**：系统设计、项目证据

### 追问 3：线上出问题时，你会看哪些日志、trace 或指标？

**回答要点**：我会先按失败类型归因，再看 trace 中的输入、状态、工具参数、返回结果、耗时、成本和 verdict；重点指标包括 task_success_rate；step_error_rate；recovery_rate；unsafe_action_block_rate；trace_replay_success_rate。如果问题来自设计边界，还要回到 schema、权限、上下文和评测集，而不是只调 prompt。

**考察点**：trace、指标归因

### 延伸追问 1：如何评测一个能操作电脑的 Agent 是否可靠和安全？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-web-agent；trace 字段包含 screenshot_ref、active_app、action_type、target_region、verifier_state、unsafe_action_blocked。；评测指标包括 task_success_rate、step_error_rate、recovery_rate、unsafe_action_rate。

### 延伸追问 2：如果把它落到 pe-web-agent 项目里，你会怎么讲？
