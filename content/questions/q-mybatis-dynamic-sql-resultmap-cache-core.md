# MyBatis 动态 SQL、ResultMap 与缓存边界 的核心机制是什么？

## 面试定位

这道题关联 MyBatis 动态 SQL、ResultMap 与缓存边界，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「MyBatis 动态 SQL、ResultMap 与缓存边界」切入：MyBatis 的动态 SQL、ResultMap、延迟加载和缓存能提升表达力，但也容易引入 SQL 分支膨胀、N+1、映射错误和缓存一致性问题。
...
先给定义和边界：动态 SQL 是按条件生成 SQL 片段的机制。；ResultMap 定义结果集到对象图的映射规则。；MyBatis 缓存包括 SqlSession 级一级缓存和 namespace 级二级缓存。；MyBatis 的动态 SQL、ResultMap、延迟加载和缓存能提升表达力，但也容易引入 SQL 分支膨胀、N+1、映射错误和缓存一致性问题。；动态 SQL 要可控。
...
- 先给定义和边界：动态 SQL 是按条件生成 SQL 片段的机制。；ResultMap 定义结果集到对象图的映射规则。；MyBatis 缓存包括 SqlSession 级一级缓存和 namespace 级二级缓存。；MyBatis 的动态 SQL、ResultMap、延迟加载和缓存能提升表达力，但也容易引入 SQL 分支膨胀、N+1、映射错误和缓存一致性问题。；动态 SQL 要可控。
- 再讲机制：SQL 分支越多越需要测试和日志样本。；对象映射不能掩盖数据库访问成本。；缓存必须明确失效边界。；复杂查询优先让数据库用合适索引和执行计划完成。；动态 SQL 用 if/choose/foreach/trim 等标签组合查询条件，适合可选条件和批量参数。。
- 工程落地要说清楚：Dynamic SQL tags。；ResultMap association/collection。；SQL fragment reuse。；Cache disabled by default for critical consistency。；生产开启 SQL 参数脱敏日志和慢查询采样。；大 IN 查询拆分或改临时表/批量 join。。
- 最后补指标、失败模式和取舍：dynamic_sql_branch_count；slow_sql_count；n_plus_one_count；cache_hit_rate；sql_length；foreach 大集合导致 SQL 过长；ResultMap 嵌套查询形成 N+1；开启二级缓存却没有失效策略。
...
### 追问 1：如果面试官深挖 MyBatis 动态 SQL、ResultMap 与缓存边界 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：动态 SQL 是按条件生成 SQL 片段的机制。；ResultMap 定义结果集到对象图的映射规则。；MyBatis 缓存包括 SqlSession 级一级缓存和 namespace 级二级缓存。；MyBatis 的动态 SQL、ResultMap、延迟加载和缓存能提升表达力，但也容易引入 SQL 分支膨胀、N+1、映射错误和缓存一致性问题。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：生产开启 SQL 参数脱敏日志和慢查询采样。；大 IN 查询拆分或改临时表/批量 join。；对复杂报表查询建立独立读模型或 ES，而不是让动态 SQL 无限膨胀。；动态 SQL 分支要有测试样例，覆盖空条件、单条件、多条件和边界参数。；二级缓存要谨慎使用，涉及多表、多服务写入或事务外更新时容易不一致。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：dynamic_sql_branch_count；slow_sql_count；n_plus_one_count；cache_hit_rate；sql_length。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 MyBatis 动态 SQL、ResultMap 与缓存边界 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；动态 SQL 分支要有测试样例，覆盖空条件、单条件、多条件和边界参数。；二级缓存要谨慎使用，涉及多表、多服务写入或事务外更新时容易不一致。
