...
图 1：Agent Guardrails 从输入、上下文、工具到输出的分层控制链路。

这张图里，input guardrail 负责识别请求风险，context guardrail 负责把外部内容隔离成不可信证据，Model 只能提出工具调用或答案草稿，不能直接决定权限。Output Validator 检查生成内容是否泄露 PII、缺引用或违反格式规则；Tool Permission Gate 则在执行前按用户、租户、资源、riskLevel 和参数做确定性校验。Human-in-the-loop 只出现在高风险动作上，Audit + Safety Eval 负责把 allow、deny、confirm、误拦和漏拦沉淀成可回放证据。
...
第一轮追问：为什么不能只靠 system prompt 做安全？  
回答要点：system prompt 能表达策略，但执行权必须在宿主程序、权限系统和工具运行时。考察点是确定性授权边界。陷阱是让模型自己判断“我是否有权限”，这会把安全决策交给概率输出。

第二轮追问：哪些工具调用需要 human-in-the-loop？  
回答要点：不可逆写操作、财务动作、对外发送、跨租户访问、权限变更、生产发布和敏感数据导出。考察点是 riskLevel 与 side effect 识别。陷阱是所有动作都人工确认，导致系统不可用，或者所有动作都自动执行，导致风险失控。

第三轮追问：Guardrails 误拦太多怎么办？  
回答要点：先用 shadow mode 记录命中样本，按风险分层调阈值，对高价值误拦进入人工复核和 regression；不要直接关闭整层防护。考察点是安全和可用性的取舍。陷阱是只看 block rate，不看 post_block_task_success_rate 和 false_positive_rate。

第四轮追问：如何证明策略上线后更安全？  
回答要点：固定攻击样本集、线上 trace、人工复核、漏拦事故、PII 泄露数、unsafe tool block 和 approval bypass 等指标共同判断。考察点是评估闭环。陷阱是只说“通过红队测试”，但没有回归集和版本化 policy。
...
- [OpenAI Agents SDK Guardrails](https://openai.github.io/openai-agents-python/guardrails/)：用于支持输入、输出和工具调用前后防护的分层设计。
- [OWASP LLM01: Prompt Injection](https://genai.owasp.org/llmrisk/llm01-prompt-injection/)：用于说明外部内容试图覆盖指令时的主要风险类别。
- [OpenAI Agents SDK Tracing](https://openai.github.io/openai-agents-python/tracing/)：用于支持安全 verdict、工具调用、人工确认和事故复盘的 trace 设计。
