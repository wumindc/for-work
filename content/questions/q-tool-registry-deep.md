...
- [Model Context Protocol](https://modelcontextprotocol.io/)：用于说明远程工具发现、资源连接和协议边界，支撑“Registry 可以吸收外部能力，但协议发现不等于本地授权”的结论。
- [OpenAI Agents SDK Tools](https://openai.github.io/openai-agents-python/tools/)：用于确认工具是 Agent 执行动作的接口，支撑 schema、执行器、guardrails 和 trace 分层治理。
- [OpenAI A practical guide to building agents](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf)：用于支持 Agent 编排、工具调用、审批、人类确认和运行时治理的整体框架。
- [Anthropic: Writing effective tools for agents](https://www.anthropic.com/engineering/writing-tools-for-agents)：用于解释工具描述、工具评测和迭代改进的重要性，支撑“工具定义不是一次性配置”的观点。
