...
图 1：workflow、chatbot 与 Agent 的边界判断。图中先判断任务路径是否确定，确定路径优先 workflow；只有自然语言问答或轻交互时用 chatbot；路径开放、需要观察环境并根据反馈调整动作时，才进入 Agent loop。

这张图的核心是“动态性要能被验证”。Agent 不是因为名字高级才被选择，而是因为任务需要 observe、reason、act、verify 的闭环；如果结果无法验证，动态决策会变成不可控风险；如果路径本来固定，workflow 的可审计性和成本通常更好。
...
事故排查要先定影响面：是 Agent 分支成功率低、workflow 主干被错误绕过、工具误用，还是 stop condition 不生效。止血可以把动态分支临时降级为 workflow、限制工具白名单、降低 step budget、要求人工确认。根因要看 baseline 对比、observation、tool args、state transition、verifier verdict 和 cost trace。回归要把失败场景拆成 workflow-only、agent-enabled 和 agent-with-guardrails 三组对比。
...
**追问 1：什么时候应该坚决不用 Agent？**  
答题要点：路径固定、规则明确、失败代价高、事务一致性强或结果无法自动验证时，优先 workflow、规则引擎或人工审批。考察点是边界意识。陷阱是把 Agent 当默认架构。

**追问 2：Agent 和 workflow 如何组合？**  
答题要点：workflow 承担稳定主干，Agent 处理开放分支、异常恢复和探索任务；Agent 输出仍要回到 verifier 或状态机。考察点是系统编排。陷阱是让 Agent 接管整条业务链路。

**追问 3：如何证明 Agent 比 workflow 更好？**  
答题要点：先有 workflow baseline，再用同一 golden set 比较 success rate、recovery rate、latency、cost、unsafe_action_block_rate 和 human_override_rate。考察点是实验设计。陷阱是只展示一段成功录像。
...
- [OpenAI Agents SDK](https://openai.github.io/openai-agents-python/)：官方文档用于支持 Agent 需要工具、状态、编排、评估和安全边界，而不是简单聊天。
- [OpenAI Evals](https://platform.openai.com/docs/guides/evals)：官方文档用于说明 Agent 或 workflow 方案要通过样例、指标和回归结果验证。
- [LangGraph Overview](https://docs.langchain.com/oss/python/langgraph/overview)：官方文档用于支撑 workflow、state 和 agent 编排在工程实现中的边界。
- [LangSmith Evaluation](https://docs.smith.langchain.com/evaluation)：官方文档用于说明用数据集、评测器和实验结果比较 Agent 与 baseline。
- [Anthropic Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)：工程文章用于补充 workflow 更适合确定路径、Agent 更适合开放反馈循环的判断。
