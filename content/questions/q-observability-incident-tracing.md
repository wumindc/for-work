...
这道题的高分答案要有明确排障顺序：先看指标确定影响面和是否持续恶化，再用 Trace 找异常 span 和依赖路径，最后用结构化日志解释局部 error_code、release_id、参数摘要和业务状态。直接从海量日志里搜索关键字，通常只能说明经验不够体系化。

回答还要覆盖异步断链。HTTP 请求可以用 `traceparent` 传播，但 MQ、线程池、异步任务、定时任务和 Agent tool call 都需要显式传递和恢复上下文。消费者日志里要有 topic、partition、offset、message_id、retry_count 和 trace context；Agent run 要把 run_id、tool_call_id、model_version 和 trace_id 建立映射。

事故复盘部分要从“修好了”升级为“证明修好了”。证据包括错误率和 p95 回落、关键 span 不再异常、日志 error_code 消失、失败 run 可以 replay、告警阈值覆盖同类问题、runbook 更新并有人负责。没有这些证据，只能算临时止血。

安全边界也可以主动说：日志和 Trace 不能保存完整 prompt、token、身份证、订单隐私、完整 SQL 参数或工具原始参数。可用 hash、摘要、evidence_id、chunk_version、字段白名单和访问控制替代。这个补充能让答案从排障能力延伸到生产合规能力。
