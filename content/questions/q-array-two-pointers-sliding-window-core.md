...
### 追问 1：如果面试官深挖 数组双指针、前缀和与滑动窗口 的生产落地和排障，你怎么回答？
...
### 延伸追问 1：如果面试官深挖 数组双指针、前缀和与滑动窗口 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；写窗口题先定义窗口含义、合法条件、何时扩张、何时收缩、何时更新答案。；数组边界要覆盖空数组、单元素、全重复、无解和负数。
