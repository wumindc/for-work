# Docker 容器、镜像与仓库基础 的核心机制是什么？

## 面试定位

这道题关联 Docker 容器、镜像与仓库基础，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Docker 容器、镜像与仓库基础」切入：Docker 将应用进程、依赖和文件系统打包成可分发镜像，并用容器运行实例化进程，解决本地、测试和生产环境不一致问题。
...
先给定义和边界：镜像是应用运行环境的不可变模板。；容器是镜像在宿主机上的运行实例。；Registry 是镜像分发和版本管理中心。；Docker 将应用进程、依赖和文件系统打包成可分发镜像，并用容器运行实例化进程，解决本地、测试和生产环境不一致问题。；镜像是只读模板。
...
- 先给定义和边界：镜像是应用运行环境的不可变模板。；容器是镜像在宿主机上的运行实例。；Registry 是镜像分发和版本管理中心。；Docker 将应用进程、依赖和文件系统打包成可分发镜像，并用容器运行实例化进程，解决本地、测试和生产环境不一致问题。；镜像是只读模板。
- 再讲机制：容器应该只包含运行所需依赖，配置通过环境、文件或平台注入。；镜像要不可变、可追溯、可扫描和可回滚。；日志输出到 stdout/stderr，交给平台采集。；运行态状态不要写入镜像层，持久数据使用卷或外部存储。；镜像由多层 filesystem layer 组成，可通过 registry 分发和复用。。
- 工程落地要说清楚：Immutable image。；Image registry promotion。；Non-root container。；Healthcheck and structured logs。；镜像标签使用 app version、git sha、build number 和 base image digest，避免生产依赖 floating tag。；容器启动脚本只做轻量初始化，不在启动时下载不确定依赖。。
- 最后补指标、失败模式和取舍：image_size_mb；container_restart_count；startup_latency；healthcheck_failure_count；deployment_success_rate；把容器当完整虚拟机；latest 标签直接上生产；镜像里写死环境配置和密钥。
...
### 追问 1：如果面试官深挖 Docker 容器、镜像与仓库基础 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：镜像是应用运行环境的不可变模板。；容器是镜像在宿主机上的运行实例。；Registry 是镜像分发和版本管理中心。；Docker 将应用进程、依赖和文件系统打包成可分发镜像，并用容器运行实例化进程，解决本地、测试和生产环境不一致问题。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：镜像标签使用 app version、git sha、build number 和 base image digest，避免生产依赖 floating tag。；容器启动脚本只做轻量初始化，不在启动时下载不确定依赖。；敏感信息通过 Secret 或环境注入，不能 bake 进镜像。；镜像标签要可追溯到 git commit、构建时间、基础镜像和 SBOM/扫描结果。；生产容器要定义入口命令、健康检查、日志输出、资源限制、只读文件系统和非 root 用户。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：image_size_mb；container_restart_count；startup_latency；healthcheck_failure_count；deployment_success_rate。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 Docker 容器、镜像与仓库基础 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；镜像标签要可追溯到 git commit、构建时间、基础镜像和 SBOM/扫描结果。；生产容器要定义入口命令、健康检查、日志输出、资源限制、只读文件系统和非 root 用户。
