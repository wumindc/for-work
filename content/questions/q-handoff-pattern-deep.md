...
图 1：多 Agent handoff 的责任转移链路。图中 Sender 不直接把自然语言历史甩给 Target，而是通过 Orchestrator 写入 shared state，并在 Target accept 后变更 owner；Target 执行完成后写入 verdict，Orchestrator 再按 return_policy 把结果返回或触发兜底。

这张图里的关键状态变化是 `owner=pending -> owner=target -> returned`。只有 owner 和 allowed_write_scope 变化被记录下来，才能解释“谁现在负责”“谁能写哪个资源”“失败后回到谁手里”。如果没有这层状态机，handoff 会退化成多模型互相发消息，任务稍微长一点就会出现上下文丢失和双写。
...
事故处理时先判断影响面：是某个目标 Agent 经常 reject，还是所有 handoff 都出现 owner 冲突；止血可以冻结目标资源写权限，把新 handoff 切到 pending_review 或退回 sender；根因要查 handoff span、payload hash、artifact access log、owner version 和 return_policy 分支；回归用 accept、reject、timeout、blocked、human escalation 五类状态样本验证，不只测成功路径。
...
**追问 1：为什么 handoff payload 不直接传完整对话历史？**  
答题要点：完整历史成本高、包含无关和敏感信息，也无法表达写权限、owner、deadline 和 return policy；应传结构化 state_summary 与 artifact_refs。考察点是最小上下文和权限边界。陷阱是把“上下文越多越安全”当成原则。

**追问 2：目标 Agent accept 后，原 Agent 能不能继续写共享状态？**  
答题要点：不能写已转移资源，除非 return_policy 或 lock lease 明确允许；shared state 应有 owner/version，写入用 compare-and-swap 或租约锁。考察点是责任归属。陷阱是只做 prompt 约定，不做状态约束。

**追问 3：handoff timeout 后如何恢复？**  
答题要点：按 return_policy 决定重试、退回 sender、转 fallback Agent、转人工或取消；同时保留 timeout span 和已访问 artifact，避免重复执行高风险动作。考察点是失败回退。陷阱是 timeout 后任务悬挂。
...
- [OpenAI Agents SDK Handoffs](https://openai.github.io/openai-agents-python/handoffs/)：官方文档用于说明 handoff 是明确的 Agent 间转交机制，而不是普通文本聊天。
- [LangChain Multi-agent](https://docs.langchain.com/oss/python/langchain/multi-agent)：官方文档用于支持 supervisor、tool calling、handoff 等多 Agent 控制流取舍。
- [OpenAI Agents SDK Tracing](https://openai.github.io/openai-agents-python/tracing/)：官方文档用于说明 handoff span、payload 和最终 verdict 应进入可追踪链路。
