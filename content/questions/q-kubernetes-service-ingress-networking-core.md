# Kubernetes Service、Ingress 与网络入口 的核心机制是什么？
...
### 追问 1：如果面试官深挖 Kubernetes Service、Ingress 与网络入口 的生产落地和排障，你怎么回答？
...
### 延伸追问 1：如果面试官深挖 Kubernetes Service、Ingress 与网络入口 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-web-agent、pe-coding-agent；排查服务不可达时按 DNS、Service、EndpointSlice、Pod readiness、NetworkPolicy、Ingress Controller 和后端日志逐层看。；外部入口要配 TLS、超时、请求体大小、限流、访问日志和 trace header 透传。
