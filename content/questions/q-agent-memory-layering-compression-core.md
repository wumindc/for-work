...
这道题关联 Agent Memory 分层与压缩，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
...
### 追问 1：怎样证明记忆压缩既省 token 又没有丢关键约束？
...
### 延伸追问 1：怎样证明记忆压缩既省 token 又没有丢关键约束？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；Memory item 字段包含 scope、ttl、confidence、source_trace、privacy_level、last_used_at。；压缩质量用 lost_constraint_rate、resume_success_rate、token_saving_ratio 评估。
