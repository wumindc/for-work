# Dockerfile、BuildKit 与镜像层优化 的核心机制是什么？
...
### 追问 1：如果面试官深挖 Dockerfile、BuildKit 与镜像层优化 的生产落地和排障，你怎么回答？
...
### 延伸追问 1：如果面试官深挖 Dockerfile、BuildKit 与镜像层优化 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；先复制 lockfile 安装依赖，再复制源代码，提高缓存命中。；最终镜像只保留运行产物、必要证书和启动用户，删除构建工具和临时文件。
