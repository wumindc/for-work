...
图 1：Agentic RAG 的防漂移检索循环，每轮检索都必须从原始问题拆出的 evidence gap 出发，并由 evidence grader 与 drift detector 决定继续、回答、重规划或追问。

这张图的核心边界是：多轮检索不是让模型沿着上一条结果自由探索，而是让 `Subquestion plan` 约束 `Query rewrite`，让 `Evidence grader` 判断新材料是否能填补证据缺口，再由 `Drift detector` 把候选结论拉回 `Root question`。如果 `drift` 分支频繁出现，说明任务边界、检索维度或停止策略需要修正，而不是继续加轮数。
...
事故复盘要先看影响面：是某个问题类型漂移、某个检索源污染，还是所有多轮检索都在后半段偏离。止血可以限制最大轮数、提高 drift 阈值、只允许围绕已声明维度改写 query，并把高风险答案降级为“证据不足”。根因通常在三处：planner 没有拆出稳定维度，query rewrite 把上一轮材料当成新目标，或者 evidence grader 只看相关性不看 answerability。回归时要保存 root question、query_history、gap_reason、selected evidence、drift verdict 和 final claim，确认修复后不仅答案正确，检索路径也没有偏离。
...
**追问 1：related 和 answerable 有什么区别？**

Related 只是主题相关，answerable 才能支持当前 evidence gap。比如用户问 Agent 框架选型，LangGraph 的图数据库示例可能和框架相关，但不能回答“状态管理、部署、工具调用如何比较”。Eval 里要把相关但不能回答的证据单独分桶，否则 loop 会被看似相关的材料带偏。

**追问 2：drift detector 误杀怎么办？**

不要只靠单一模型判断。规则先检查核心实体、时间范围和比较维度，模型再判断语义目的；被判 drift 的 query 可以进入 replan，而不是直接失败。对于探索型问题，可以降低阈值，但必须把最终 claim 与 root question 做 alignment。

**追问 3：为什么追问用户也是一种成功？**

因为原问题多义或约束冲突时，继续检索只会放大漂移。比如“帮我选一个 Agent 框架”没有语言、部署环境、团队经验和状态复杂度，系统先问清约束，比盲目查更多资料更可靠。面试里要强调：Agentic RAG 的目标是可信答案，不是尽量多查。
...
- [LangChain Retrieval](https://docs.langchain.com/oss/python/langchain/retrieval)：官方文档用于支持把检索、query rewrite 和 evidence 组织成可控链路，而不是只依赖一次召回。
- [LangGraph Overview](https://docs.langchain.com/oss/python/langgraph/overview)：官方文档用于支持用状态机管理多步 Agent/RAG 流程，并把重规划、停止和继续作为显式状态。
- [LangSmith Evaluation](https://docs.smith.langchain.com/evaluation)：官方文档用于支持把 query history、drift verdict 和 final claim alignment 固化成可回归评测数据。
