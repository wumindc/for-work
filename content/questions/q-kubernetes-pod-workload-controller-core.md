# Kubernetes Pod、Deployment 与工作负载控制器 的核心机制是什么？

## 面试定位

这道题关联 Kubernetes Pod、Deployment 与工作负载控制器，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Kubernetes Pod、Deployment 与工作负载控制器」切入：Pod 是 Kubernetes 最小调度单元，Deployment、ReplicaSet、StatefulSet、DaemonSet 等控制器通过 reconciliation 保持期望副本和发布状态。
...
先给定义和边界：Pod 是 Kubernetes 中最小可部署、可调度的计算单元。；Deployment 是管理无状态副本和滚动发布的控制器。；Controller 通过调谐循环让实际状态接近期望状态。；Pod 是 Kubernetes 最小调度单元，Deployment、ReplicaSet、StatefulSet、DaemonSet 等控制器通过 reconciliation 保持期望副本和发布状态。；Pod 是调度单元。
...
- 先给定义和边界：Pod 是 Kubernetes 中最小可部署、可调度的计算单元。；Deployment 是管理无状态副本和滚动发布的控制器。；Controller 通过调谐循环让实际状态接近期望状态。；Pod 是 Kubernetes 最小调度单元，Deployment、ReplicaSet、StatefulSet、DaemonSet 等控制器通过 reconciliation 保持期望副本和发布状态。；Pod 是调度单元。
- 再讲机制：Pod 会被重建和迁移，所以状态不能只存在本地容器文件系统。；readiness 决定是否接流量，liveness 决定是否重启，startup 保护慢启动服务。；requests 影响调度，limits 影响运行边界。；滚动发布要控制 maxUnavailable、maxSurge 和回滚路径。；Pod 内的容器共享网络命名空间和部分存储卷，通常表达一个紧密协作的运行单元。。
- 工程落地要说清楚：Deployment rollout。；Readiness/liveness/startup probes。；Resource requests/limits。；PDB。；上线前验证镜像拉取、环境变量、Secret/ConfigMap、端口、探针和资源配置。；发布失败时用 rollout undo、暂停发布、扩旧版本或切流量止血。。
- 最后补指标、失败模式和取舍：deployment_available_replicas；pod_restart_count；probe_failure_count；rollout_duration；oom_killed_count；把 Pod 当永久机器使用；没有 readiness 就接流量；requests/limits 缺失导致调度和稳定性不可控。
...
### 追问 1：如果面试官深挖 Kubernetes Pod、Deployment 与工作负载控制器 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：Pod 是 Kubernetes 中最小可部署、可调度的计算单元。；Deployment 是管理无状态副本和滚动发布的控制器。；Controller 通过调谐循环让实际状态接近期望状态。；Pod 是 Kubernetes 最小调度单元，Deployment、ReplicaSet、StatefulSet、DaemonSet 等控制器通过 reconciliation 保持期望副本和发布状态。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：上线前验证镜像拉取、环境变量、Secret/ConfigMap、端口、探针和资源配置。；发布失败时用 rollout undo、暂停发布、扩旧版本或切流量止血。；Pod 频繁重启要查 exit code、OOMKilled、probe failure、依赖超时和启动脚本。；生产服务必须设置 requests/limits、readiness/liveness/startup probes、滚动发布策略和 PodDisruptionBudget。；排障先看 pod phase、events、container status、logs、describe、probe failure 和 rollout history。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：deployment_available_replicas；pod_restart_count；probe_failure_count；rollout_duration；oom_killed_count。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 Kubernetes Pod、Deployment 与工作负载控制器 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；生产服务必须设置 requests/limits、readiness/liveness/startup probes、滚动发布策略和 PodDisruptionBudget。；排障先看 pod phase、events、container status、logs、describe、probe failure 和 rollout history。
