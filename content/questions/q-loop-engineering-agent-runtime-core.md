# Loop Engineering 和传统 prompt engineering 的区别是什么？

## 面试定位

这道题关联 Loop Engineering 与 Agent Runtime，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
...
### 追问 1：如果让一个 Agent 自动跑半小时任务，你会如何设计 state、verifier、schedule 和恢复机制？
...
### 延伸追问 1：如果让一个 Agent 自动跑半小时任务，你会如何设计 state、verifier、schedule 和恢复机制？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent、pe-web-agent；为每次 run 建立 state file、step log、verifier verdict 和 next action。；把停止条件、预算、重试、人工确认写进 runtime，而不是交给模型自由判断。

### 延伸追问 2：如果把它落到 pe-coding-agent / pe-web-agent 项目里，你会怎么讲？
...
**项目证据怎么挂钩**
- 可以关联项目证据：pe-coding-agent、pe-web-agent
- 为每次 run 建立 state file、step log、verifier verdict 和 next action。
- 把停止条件、预算、重试、人工确认写进 runtime，而不是交给模型自由判断。
