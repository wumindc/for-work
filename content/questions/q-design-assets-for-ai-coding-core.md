# 为什么 AI 编程需要 DESIGN.md 这类设计资产？

## 面试定位

这道题关联 DESIGN.md 与 AI 可读设计资产，难度 3/5，出现频率 medium。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「DESIGN.md 与 AI 可读设计资产」切入：AI 编程质量越来越依赖可被模型读取的设计资产，例如 DESIGN.md、组件规范、视觉参考、交互规则和品牌约束。
...
我会先划边界：AI 编程质量越来越依赖可被模型读取的设计资产，例如 DESIGN.md、组件规范、视觉参考、交互规则和品牌约束。；设计资产要机器可读；审美要变成约束和示例；输出要能被视觉 QA 验证。
...
- 我会先划边界：AI 编程质量越来越依赖可被模型读取的设计资产，例如 DESIGN.md、组件规范、视觉参考、交互规则和品牌约束。；设计资产要机器可读；审美要变成约束和示例；输出要能被视觉 QA 验证。
- 再讲运行机制：awesome-design-md 和 taste-skill 类项目说明前端 AI 生成正在从组件库转向设计说明和风格约束。；高质量设计资产能减少默认模板味和返工。。
- 工程实现要落到可执行设计：DESIGN.md 应包含颜色、字体、间距、组件状态、布局规则、禁用模式、示例截图和 QA checklist。；验证要结合截图、移动端宽度、溢出检查和视觉回归。。
- 如果被要求画设计，可以按这个结构展开：。
- 最后补风险、指标和取舍：只给审美形容词；没有负例；缺少移动端和状态约束；跟踪 visual_regression_count；跟踪 mobile_overflow_count；跟踪 design_token_violation_count；跟踪 revision_rounds；跟踪 qa_pass_rate。
...
### 追问 1：如何把抽象审美要求变成 Agent 可执行、可验证的前端约束？
...
**回答要点**：我会按输入、状态、执行、验证和指标展开：DESIGN.md 应包含颜色、字体、间距、组件状态、布局规则、禁用模式、示例截图和 QA checklist。；验证要结合截图、移动端宽度、溢出检查和视觉回归。；记录颜色、字体、间距、栅格、组件状态、交互规则和禁用模式。；提供正例截图、负例说明、常见误区和页面级验收清单。；让 Agent 修改前先读取 DESIGN.md，再生成或调整组件。；用 Playwright 截图、移动端宽度、文本溢出和交互状态做 QA。。项目表达里要说明模块边界、数据流、错误恢复和上线后的指标，而不是只说用了某个框架。

**考察点**：系统设计、项目证据

### 追问 3：线上出问题时，你会看哪些日志、trace 或指标？

**回答要点**：我会先按失败类型归因，再看 trace 中的输入、状态、工具参数、返回结果、耗时、成本和 verdict；重点指标包括 visual_regression_count；mobile_overflow_count；design_token_violation_count；revision_rounds；qa_pass_rate。如果问题来自设计边界，还要回到 schema、权限、上下文和评测集，而不是只调 prompt。

**考察点**：trace、指标归因

### 延伸追问 1：如何把抽象审美要求变成 Agent 可执行、可验证的前端约束？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-web-agent；DESIGN.md 应包含颜色、字体、间距、组件状态、布局规则、禁用模式、示例截图和 QA checklist。；验证要结合截图、移动端宽度、溢出检查和视觉回归。
