# Kubernetes 发布、探针、HPA 与可观测性 的核心机制是什么？
...
### 追问 1：如果面试官深挖 Kubernetes 发布、探针、HPA 与可观测性 的生产落地和排障，你怎么回答？
...
### 延伸追问 1：如果面试官深挖 Kubernetes 发布、探针、HPA 与可观测性 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent、pe-web-agent；发布门禁包括镜像扫描、单测、集成测试、迁移检查、探针验证、SLO 观察和自动回滚条件。；DevOps 面试要能从 CI/CD、镜像、K8s rollout、Prometheus、日志、Trace 和 Runbook 讲完整发布链路。
