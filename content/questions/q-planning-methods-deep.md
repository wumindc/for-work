...
公开读者读这篇时，应该能意识到“重规划”不是让模型重新想一遍，而是一次受控的状态版本迁移。旧计划、失败 observation、新的约束、被废弃步骤和新计划 diff 都要被记录下来。否则系统表面上在自我修复，实际上只是把错误前提覆盖掉，后续无法解释为什么路线发生变化。

判断重规划是否成熟，可以看它是否区分冲突类型。工具失败、外部状态变化、用户约束变化、证据冲突、计划假设失效和风险策略阻断，处理方式完全不同。把所有冲突都交给同一个 prompt 重写计划，会让成本和错误都变得不可控；更好的做法是先分类，再决定 retry、换工具、局部修补、全量重规划、追问用户或转人工。

还有一个生产边界：重规划不能跨过副作用状态。已经发出的邮件、提交的订单、写入的配置和执行中的审批，都需要先确认外部系统真实状态，再决定后续计划。否则恢复逻辑可能重复执行写操作。公开评审时，`same_reason_replan_loop_count`、`abandoned_step_count` 和 `time_to_recover` 比“模型是否给出新计划”更能说明系统可靠性。
