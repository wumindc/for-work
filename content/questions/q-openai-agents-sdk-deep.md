...
图 1：客服或旅行 Agent 中 handoff 与 guardrails 的主链路。图中 Triage Agent 负责意图识别和状态摘要，Specialist Agent 只处理自己领域的只读分析和方案生成，Guardrails 在写动作前检查风险，Tool Runtime 才是真正执行副作用的确定性边界，Human confirmation 用来处理高风险操作。

这张图的关键分层是：Agent 负责理解和方案，Guardrails 负责拦截和校验，业务后端负责权限和状态变更。guardrails 不能替代权限系统；handoff 也不能只是一句自然语言转交。只有把 handoff payload、guardrail verdict、tool result 和 human approval 全部写入 trace，客服或旅行 Agent 才能被审计和回放。
...
事故处理建议拆成四步：影响面先看是 handoff 选错 Specialist、guardrails 误拦截，还是写工具绕过确认；止血可以把高风险工具切到 preview-only，暂停自动 apply，并把低置信 handoff 转人工；根因要查 handoff payload、risk_flags、guardrail verdict reason、approval_id、idempotency key 和 tool observation；回归要覆盖退款、改签、取消订单、越权查询、未确认输出承诺和 handoff loop 样本。
...
**追问 1：guardrails 应该放在模型前、工具前还是输出后？**  
答题要点：三层都需要。输入层处理越权和敏感请求，工具层处理副作用、金额、对象归属和确认状态，输出层防止承诺未确认结果。考察点是分层防线。陷阱是把 guardrails 当成单个 prompt。

**追问 2：handoff payload 里必须有哪些字段？**  
答题要点：`intent`、`confidence`、`handoff_reason`、`state_summary`、`user_constraints`、`completed_steps`、`risk_flags`、`return_policy`、`trace_id`。考察点是可恢复协议。陷阱是只传“交给退款 Agent”。

**追问 3：为什么业务权限不能只靠 Agent 判断？**  
答题要点：Agent 可做理解和风险提示，但订单归属、金额、退款窗口、身份和幂等必须由 Tool Runtime 或后端校验；Agent 输出不能作为业务事实。考察点是确定性边界。陷阱是让 LLM 直接决定副作用。
...
- [OpenAI Agents SDK Handoffs](https://openai.github.io/openai-agents-python/handoffs/)：官方文档用于说明 handoff 的设计语义和 Agent 间转交边界。
- [OpenAI Agents SDK Guardrails](https://openai.github.io/openai-agents-python/guardrails/)：官方文档用于支持输入、工具和输出校验分层的工程实践。
- [OpenAI Agents SDK Tracing](https://openai.github.io/openai-agents-python/tracing/)：官方文档用于说明 handoff、guardrail verdict 和 tool call 应进入可追踪链路。
- [OpenAI A practical guide to building agents](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf)：官方指南用于补充客服、旅行等业务 Agent 的设计模式和人机协作边界。
