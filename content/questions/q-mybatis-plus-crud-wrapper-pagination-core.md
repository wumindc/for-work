# MyBatis Plus CRUD、Wrapper、分页与插件机制 的核心机制是什么？
...
### 追问 1：如果面试官深挖 MyBatis Plus CRUD、Wrapper、分页与插件机制 的生产落地和排障，你怎么回答？
...
### 延伸追问 1：如果面试官深挖 MyBatis Plus CRUD、Wrapper、分页与插件机制 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；复杂查询不要硬塞 Wrapper，必要时回到 XML 或专门读模型。；分页接口要限制 page size，复杂 count 查询要定制或异步化。
