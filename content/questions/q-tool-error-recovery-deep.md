...
图 1：工具失败 feedback 从异常到恢复动作的决策链路。

图中 Boundary Adapter 把运行时异常归一化成 Error Envelope，Policy Engine 再根据 retryable、side effect、risk 和 remaining budget 决定修参、退避、fallback、状态检查或 handoff。feedback 同时服务模型、策略和排障，不能只是一段自然语言；模型只能看到安全摘要，真实重试和高风险动作由 Orchestrator 控制。
...
**追问 1：retryable 应该由模型判断吗？**
不应该。模型可以根据 feedback 修参数或生成替代 query，但是否可重试、退避多久、是否有副作用风险，应由 Policy Engine 根据 error_code、side_effect_state、idempotency 和预算决定。考察点是执行权归宿，陷阱是让模型看到 timeout 就无限重试。

**追问 2：支付或下单 timeout 后怎么恢复？**
先把 `side_effect_state` 标成 unknown，使用 idempotency key 查询订单或支付状态。确认副作用未发生才允许 retry；确认已发生但业务未完成时走补偿或人工工单；状态查不到时停止并提示用户，不允许直接再次扣款。考察点是一致性和补偿，陷阱是把 timeout 当作失败后立刻重试。

**追问 3：权限拒绝要不要告诉模型缺什么权限？**
要给可行动但脱敏的信息，例如“当前用户无权执行该动作，需要用户授权或管理员处理”；不能暴露完整权限树、内部角色名、敏感资源 id 或绕过建议。考察点是安全摘要，陷阱是把内部错误和权限模型原样放进上下文。

**追问 4：如何判断 feedback 设计有效？**
看 `repeat_error_rate` 是否下降、`argument_repair_success_rate` 是否提升、`retry_success_rate` 是否在预算内、`fallback_quality_score` 是否可接受、`unsupported_answer_rate` 是否下降。feedback 的目标不是让模型把失败说得更圆，而是让系统更可恢复、更诚实。考察点是指标和 eval，陷阱是只看最终完成率。
...
- [OpenAI A practical guide to building agents](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf)：用于支持工具错误、guardrail、人类确认和 workflow 恢复需要由宿主编排。
- [OpenAI Agents SDK Tools](https://openai.github.io/openai-agents-python/tools/)：官方文档用于说明工具调用结果应以明确接口返回给 Agent，支撑 error envelope 的设计。
- [Anthropic Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)：用于支持将 Agent 设计成清晰、可组合的工作流，并在工具反馈后由系统控制下一步。
