...
多数 MQ 是 at-least-once 语义，重复消息是正常情况。目标是最终可靠处理，而不是假设消息在业务端只出现一次。
...
### 追问 1：如果本地事务提交成功，但消息还没发出去服务就宕机怎么办？

回答要点：不要把“写库后立刻发 MQ”当成原子操作。更稳的是 outbox：业务表和 `outbox_events` 在同一个数据库事务里提交，relay 或 CDC 后续把事件发到 broker。relay 发送失败可以按 `next_retry_at` 重试，发送成功后标记 sent，旧事件积压要有告警。

考察点：生产端一致性、outbox 字段、relay 恢复能力。

陷阱：只说 producer confirm。producer confirm 只能说明消息到达 broker，不能覆盖本地事务和发消息之间的宕机窗口。

### 追问 2：consumer 处理成功但 ack 失败，如何避免重复扣款或重复加积分？

回答要点：按 at-least-once 设计，业务处理前先占用幂等键，例如 `event_id` 或 `business_id + event_type + version`。业务成功后记录处理结果和状态，重投时读到已成功就直接返回或跳过。外部支付、短信、库存这类副作用接口也要传自己的 idempotency key。

考察点：消费端幂等、状态机版本、外部副作用治理。

陷阱：把 MQ 的 exactly-once 语义误解成业务端不会重复产生副作用。broker 内部语义不能替代业务数据库和外部接口幂等。

### 追问 3：DLQ 重放为什么危险？上线前怎么设计重放流程？

回答要点：DLQ 里的消息往往已经失败多次，直接全量重放可能再次打爆下游或重复副作用。重放前要分类错误、修复代码或 payload、限制速率、保留原 event_id、复用幂等逻辑、写审计记录，并在灰度 consumer group 里先抽样验证。

考察点：故障恢复、人工审核、重放治理和审计。

陷阱：把 DLQ 当成“失败消息列表”，没有告警、修复、重放和回归机制。
...
exactly-once 不能被简单理解成“业务只发生一次”。某些 MQ 可以在 broker 和 stream processing 语义上减少重复，但外部数据库、支付接口、短信接口仍可能因为超时重试产生副作用。因此面试里要强调端到端幂等，而不是把希望押在 MQ 的单个特性上。
