# Agent 的 state file 应该存什么？

## 面试定位

这道题关联 State File、Verifier 与 Schedule，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「State File、Verifier 与 Schedule」切入：State file 保存目标、约束、进度、证据和下一步动作，verifier 负责判定是否继续、回滚、重试或交给人。
...
我会先划边界：State file 保存目标、约束、进度、证据和下一步动作，verifier 负责判定是否继续、回滚、重试或交给人。；state 是事实源索引；verifier 要看外部证据；schedule 要有预算和截止条件。
...
- 我会先划边界：State file 保存目标、约束、进度、证据和下一步动作，verifier 负责判定是否继续、回滚、重试或交给人。；state 是事实源索引；verifier 要看外部证据；schedule 要有预算和截止条件。
- 再讲运行机制：长任务 Agent 不能只依赖聊天历史。；schedule 负责把任务推进、暂停、恢复和超时策略显式化。。
- 工程实现要落到可执行设计：字段至少包含 goal、constraints、plan、completed_steps、open_risks、artifact_refs、verifier_verdict、next_actions。；verifier 不接受模型自评，必须读取测试、截图、引用或工具 observation。。
- 如果被要求画设计，可以按这个结构展开：。
- 最后补风险、指标和取舍：摘要覆盖真实 trace；verifier 只输出自然语言；没有版本号导致旧状态覆盖新状态；跟踪 state_conflict_rate；跟踪 lost_constraint_rate；跟踪 verifier_false_accept_rate；跟踪 duplicate_work_rate；跟踪 artifact_ref_missing_rate。
...
### 追问 1：如何设计一个不会被模型自我说服的 verifier？
...
**回答要点**：我会按输入、状态、执行、验证和指标展开：字段至少包含 goal、constraints、plan、completed_steps、open_risks、artifact_refs、verifier_verdict、next_actions。；verifier 不接受模型自评，必须读取测试、截图、引用或工具 observation。；state file 至少包含 state_version、goal、hard_constraints、plan、completed_steps、open_risks、artifact_refs。；verifier 输入必须包含测试结果、截图、citation、diff 或工具 observation。；每次状态更新带 before_version 和 after_version，避免旧摘要覆盖新状态。；恢复前先校验 hard constraints、artifact 可访问性和 next_actions 是否仍有效。。项目表达里要说明模块边界、数据流、错误恢复和上线后的指标，而不是只说用了某个框架。

**考察点**：系统设计、项目证据

### 追问 3：线上出问题时，你会看哪些日志、trace 或指标？

**回答要点**：我会先按失败类型归因，再看 trace 中的输入、状态、工具参数、返回结果、耗时、成本和 verdict；重点指标包括 state_conflict_rate；lost_constraint_rate；verifier_false_accept_rate；duplicate_work_rate；artifact_ref_missing_rate。如果问题来自设计边界，还要回到 schema、权限、上下文和评测集，而不是只调 prompt。

**考察点**：trace、指标归因

### 延伸追问 1：如何设计一个不会被模型自我说服的 verifier？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；字段至少包含 goal、constraints、plan、completed_steps、open_risks、artifact_refs、verifier_verdict、next_actions。；verifier 不接受模型自评，必须读取测试、截图、引用或工具 observation。
