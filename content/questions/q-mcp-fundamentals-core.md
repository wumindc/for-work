...
下面用多轮追问模拟展开，重点检验协议对象、运行时边界、安全治理和排障顺序。

## 多轮追问模拟

### 追问 1：Tools、Resources 和 Prompts 的区别是什么？

**回答要点**：Tools 是可执行动作，通常有副作用，例如创建 issue、写文件、查数据库；Resources 是可读取上下文，例如文档、代码片段、表结构、工单详情；Prompts 是 Server 提供的可复用提示模板。工程上要按风险分层：Resources 可以更偏读路径，Tools 必须声明输入 schema、权限、幂等、超时和审计字段，Prompts 要防止把过期流程或不安全操作注入 Host。

**考察点**：是否理解 MCP 不只是“工具列表”，而是把动作、上下文和提示模板分成不同协议对象。

**陷阱**：把只读资料包装成 Tool，会让模型误以为它可以执行动作；把写操作包装成 Resource，又会绕过权限和审计。

### 追问 2：MCP 和 Function Calling 怎么配合？

**回答要点**：Function Calling 描述模型如何以结构化形式表达“我要调用某个函数”；MCP 描述 AI 应用如何连接和治理外部能力。真实链路通常是模型输出 tool call，Host 校验用户权限、任务范围和参数 schema，再通过 MCP Client 调 Server。也就是说 Function Calling 更靠近模型输出层，MCP 更靠近应用集成层。

**考察点**：能否区分模型协议、应用协议和工具执行边界。

**陷阱**：说“用了 MCP 就不用 Function Calling”，会暴露对运行时链路的混淆；说“Function Calling 就是 MCP”，也会忽略 discovery、Resources、Prompts 和 Server 生命周期。

### 追问 3：如果一个 MCP Server 越权读文件，你怎么设计防线？

**回答要点**：我会做三层防线。Host 侧只按任务暴露必要能力，并维护 workspace root、allowlist 和用户 scope；Client 侧记录 server_id、tool_name、args_hash、policy verdict 和 trace_id；Server 侧再次校验路径、租户、token scope 和对象归属。高风险工具还要有人工确认、dry-run、幂等键和审计日志。

**考察点**：是否把安全放在 Host、Client、Server 三侧共同治理，而不是只靠提示词。

**陷阱**：只说“系统提示词禁止读敏感文件”，这不是工程权限控制；模型可能被 prompt injection 诱导，Server 必须有硬边界。

### 追问 4：MCP 调用失败时怎么排障？

**回答要点**：按链路拆：连接失败看 transport 和 server health；发现失败看 capabilities 和 schema version；调用前失败看 Host policy；调用中失败看 auth、timeout、business error；调用后失败看 result schema 和上下文写回。trace 至少要带 `server_id`、`capability_name`、`schema_version`、`user_scope`、`result_status` 和 `latency_ms`。

**考察点**：能否把协议问题、权限问题、业务执行问题和观测问题分开。

**陷阱**：一看到失败就重试，可能把权限拒绝、参数错误或非幂等写操作放大成事故。
...
- [MCP Architecture](https://modelcontextprotocol.io/docs/learn/architecture)：用于确认 Host、Client、Server、Tools、Resources、Prompts 的角色关系。
- [Model Context Protocol](https://modelcontextprotocol.io/)：用于确认 MCP 的协议定位，即为 AI 应用连接外部数据和工具提供通用接口。
