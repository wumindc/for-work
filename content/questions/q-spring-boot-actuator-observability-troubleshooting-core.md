# Spring Boot Actuator、观测指标与生产排障 的核心机制是什么？

## 面试定位

这道题关联 Spring Boot Actuator、观测指标与生产排障，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Spring Boot Actuator、观测指标与生产排障」切入：Actuator、Micrometer、日志、Trace、健康检查和线程/连接池指标共同支撑 Spring Boot 服务的生产排障和发布验证。
...
先给定义和边界：Actuator 是 Spring Boot 的生产管理和观测端点集合。；Micrometer 是应用指标门面。；生产排障是用日志、指标、Trace、事件和变更记录收敛根因。；Actuator、Micrometer、日志、Trace、健康检查和线程/连接池指标共同支撑 Spring Boot 服务的生产排障和发布验证。；健康检查不等于业务可用。
...
- 先给定义和边界：Actuator 是 Spring Boot 的生产管理和观测端点集合。；Micrometer 是应用指标门面。；生产排障是用日志、指标、Trace、事件和变更记录收敛根因。；Actuator、Micrometer、日志、Trace、健康检查和线程/连接池指标共同支撑 Spring Boot 服务的生产排障和发布验证。；健康检查不等于业务可用。
- 再讲机制：观测端点要受保护。；健康检查要区分 liveness、readiness 和业务依赖。；指标必须服务告警、容量和排障。；事故复盘要沉淀回测试和 runbook。；Actuator 暴露 health、metrics、env、beans、conditions、loggers 等端点，生产要控制访问权限和脱敏。。
- 工程落地要说清楚：Actuator health/metrics。；Micrometer + Prometheus。；Structured logging。；Distributed tracing。；Runbook。；自定义 health indicator 检查关键依赖但要设置超时。。
- 最后补指标、失败模式和取舍：http_server_requests_p95；jvm_memory_used；hikaricp_active_connections；thread_pool_queue_size；health_status_change_count；暴露 env/beans 端点泄密；只看 JVM 指标不看业务 SLO；健康检查过浅导致故障实例继续接流量。
- Actuator、Micrometer、日志、Trace、健康检查和线程/连接池指标共同支撑 Spring Boot 服务的生产排障和发布验证。
- Actuator 是 Spring Boot 的生产管理和观测端点集合。
- Micrometer 是应用指标门面。
- 生产排障是用日志、指标、Trace、事件和变更记录收敛根因。
- 观测端点要受保护。
- 健康检查要区分 liveness、readiness 和业务依赖。
- 指标必须服务告警、容量和排障。
- 事故复盘要沉淀回测试和 runbook。
- Actuator 暴露 health、metrics、env、beans、conditions、loggers 等端点，生产要控制访问权限和脱敏。
- Micrometer 将 JVM、HTTP、线程池、连接池、依赖、业务指标统一导出到 Prometheus 等系统。
...
- Distributed tracing。
- Runbook。
- 自定义 health indicator 检查关键依赖但要设置超时。
- 核心业务指标用 counter/timer/histogram 表达。
- 排障记录最近发布、配置变更、流量变化和依赖状态。
- 生产开启哪些 actuator 端点必须经过安全评估。
- 发布后观察启动、健康、HTTP p95、错误率、依赖超时、慢 SQL、线程池和连接池。
...
**重点指标**
- http_server_requests_p95
- jvm_memory_used
- hikaricp_active_connections
- thread_pool_queue_size
- health_status_change_count

## 多轮追问模拟

### 追问 1：如果面试官深挖 Spring Boot Actuator、观测指标与生产排障 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：Actuator 是 Spring Boot 的生产管理和观测端点集合。；Micrometer 是应用指标门面。；生产排障是用日志、指标、Trace、事件和变更记录收敛根因。；Actuator、Micrometer、日志、Trace、健康检查和线程/连接池指标共同支撑 Spring Boot 服务的生产排障和发布验证。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：自定义 health indicator 检查关键依赖但要设置超时。；核心业务指标用 counter/timer/histogram 表达。；排障记录最近发布、配置变更、流量变化和依赖状态。；生产开启哪些 actuator 端点必须经过安全评估。；发布后观察启动、健康、HTTP p95、错误率、依赖超时、慢 SQL、线程池和连接池。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：http_server_requests_p95；jvm_memory_used；hikaricp_active_connections；thread_pool_queue_size；health_status_change_count。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 Spring Boot Actuator、观测指标与生产排障 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；生产开启哪些 actuator 端点必须经过安全评估。；发布后观察启动、健康、HTTP p95、错误率、依赖超时、慢 SQL、线程池和连接池。
