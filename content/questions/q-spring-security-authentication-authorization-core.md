# Spring Security 认证、授权与权限边界 的核心机制是什么？

## 面试定位

这道题关联 Spring Security 认证、授权与权限边界，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Spring Security 认证、授权与权限边界」切入：Spring Security 通过 FilterChain、Authentication、SecurityContext 和授权管理器处理认证、授权、会话、JWT/OAuth2 和方法级权限。
...
先给定义和边界：认证确认是谁。；授权确认能做什么。；SecurityContext 保存当前请求的安全上下文。；Spring Security 通过 FilterChain、Authentication、SecurityContext 和授权管理器处理认证、授权、会话、JWT/OAuth2 和方法级权限。；FilterChain 是入口。
...
- 先给定义和边界：认证确认是谁。；授权确认能做什么。；SecurityContext 保存当前请求的安全上下文。；Spring Security 通过 FilterChain、Authentication、SecurityContext 和授权管理器处理认证、授权、会话、JWT/OAuth2 和方法级权限。；FilterChain 是入口。
- 再讲机制：默认拒绝，显式放行。；认证和业务用户模型要解耦。；权限判断要覆盖接口、方法和数据范围。；Token 生命周期、刷新和吊销要设计清楚。；请求进入 Security FilterChain 后完成认证提取、校验、上下文设置、异常处理和授权判断。。
- 工程落地要说清楚：SecurityFilterChain。；AuthenticationProvider。；JWT resource server。；Method security。；RBAC/ABAC。；多租户系统权限查询必须带 tenant/resource scope。。
- 最后补指标、失败模式和取舍：auth_failure_rate；access_denied_count；token_expired_count；privilege_escalation_incident；security_audit_event_count；只配 URL 权限不管数据权限；JWT 无法撤销和轮换；关闭 CSRF 却不理解应用场景。
...
### 追问 1：如果面试官深挖 Spring Security 认证、授权与权限边界 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：认证确认是谁。；授权确认能做什么。；SecurityContext 保存当前请求的安全上下文。；Spring Security 通过 FilterChain、Authentication、SecurityContext 和授权管理器处理认证、授权、会话、JWT/OAuth2 和方法级权限。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：多租户系统权限查询必须带 tenant/resource scope。；管理端高风险操作要二次确认和审计。；登录失败、权限拒绝和 token 过期要有不同错误码和安全事件。；权限模型要区分用户、角色、资源、动作、数据范围和租户。；安全日志要记录主体、资源、动作、结果和 trace_id，但不能记录 token 或密码。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：auth_failure_rate；access_denied_count；token_expired_count；privilege_escalation_incident；security_audit_event_count。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 Spring Security 认证、授权与权限边界 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-web-agent、pe-coding-agent；权限模型要区分用户、角色、资源、动作、数据范围和租户。；安全日志要记录主体、资源、动作、结果和 trace_id，但不能记录 token 或密码。
