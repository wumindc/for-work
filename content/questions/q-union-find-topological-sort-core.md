# 并查集、连通性、环检测与拓扑排序 的核心机制是什么？

## 面试定位

这道题关联 并查集、连通性、环检测与拓扑排序，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「并查集、连通性、环检测与拓扑排序」切入：并查集适合动态连通性和集合合并，拓扑排序适合有向无环依赖顺序，二者是图题高频补充模式。
...
先给定义和边界：并查集维护若干不相交集合，支持查找和合并。；拓扑排序给有向无环图的节点排依赖顺序。；入度表示有多少前置依赖指向该节点。；并查集适合动态连通性和集合合并，拓扑排序适合有向无环依赖顺序，二者是图题高频补充模式。；并查集管连通分量。
...
- 先给定义和边界：并查集维护若干不相交集合，支持查找和合并。；拓扑排序给有向无环图的节点排依赖顺序。；入度表示有多少前置依赖指向该节点。；并查集适合动态连通性和集合合并，拓扑排序适合有向无环依赖顺序，二者是图题高频补充模式。；并查集管连通分量。
- 再讲机制：并查集适合无向连通性。；路径压缩和按秩合并接近 O(1)。；拓扑排序依赖入度为 0 的起点。；处理不完所有节点说明存在环。；Union-Find 用 parent 和 rank/size 合并集合，可判断两点是否连通或加边是否成环。。
- 工程落地要说清楚：Union-Find with path compression。；Union by rank/size。；Kahn topological sort。；DFS cycle detection。；冗余连接题中同集合再 union 表示出现环。；课程表题构建 prereq -> course 的邻接表。。
- 最后补指标、失败模式和取舍：component_count；find_path_length；topological_processed_count；cycle_detected；dependency_edge_count；find 不压缩导致退化；拓扑排序漏孤立节点；有向图环检测误用无向并查集。
- 并查集适合动态连通性和集合合并，拓扑排序适合有向无环依赖顺序，二者是图题高频补充模式。
- 并查集维护若干不相交集合，支持查找和合并。
- 拓扑排序给有向无环图的节点排依赖顺序。
- 入度表示有多少前置依赖指向该节点。
- 并查集适合无向连通性。
- 路径压缩和按秩合并接近 O(1)。
- 拓扑排序依赖入度为 0 的起点。
- 处理不完所有节点说明存在环。
- Union-Find 用 parent 和 rank/size 合并集合，可判断两点是否连通或加边是否成环。
- 拓扑排序通过入度队列逐步删除零入度节点，若处理数量少于节点数说明有环。
...
- DFS cycle detection。
- 冗余连接题中同集合再 union 表示出现环。
- 课程表题构建 prereq -> course 的邻接表。
- 拓扑结果可用于任务调度和依赖构建。
- 并查集初始化每个节点 parent 指向自己。
- 拓扑排序要明确节点全集，不能只收集出现在边里的节点。
...
- component_count
- find_path_length
- topological_processed_count
- cycle_detected
- dependency_edge_count

## 多轮追问模拟

### 追问 1：如果面试官深挖 并查集、连通性、环检测与拓扑排序 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：并查集维护若干不相交集合，支持查找和合并。；拓扑排序给有向无环图的节点排依赖顺序。；入度表示有多少前置依赖指向该节点。；并查集适合动态连通性和集合合并，拓扑排序适合有向无环依赖顺序，二者是图题高频补充模式。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：冗余连接题中同集合再 union 表示出现环。；课程表题构建 prereq -> course 的邻接表。；拓扑结果可用于任务调度和依赖构建。；并查集初始化每个节点 parent 指向自己。；拓扑排序要明确节点全集，不能只收集出现在边里的节点。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：component_count；find_path_length；topological_processed_count；cycle_detected；dependency_edge_count。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 并查集、连通性、环检测与拓扑排序 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；并查集初始化每个节点 parent 指向自己。；拓扑排序要明确节点全集，不能只收集出现在边里的节点。
