# Skill 和 Tool、Workflow 的区别是什么？

## 面试定位

这道题关联 Skill 化工作流与能力封装，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Skill 化工作流与能力封装」切入：Skill 把触发条件、执行步骤、工具契约、模板、参考资料和验证方法封装成 Agent 可加载能力。
...
我会先划边界：Skill 把触发条件、执行步骤、工具契约、模板、参考资料和验证方法封装成 Agent 可加载能力。；Skill 不是长 prompt；trigger 和 scope 决定能否稳定复用；eval 证明 Skill 有价值。
...
- 我会先划边界：Skill 把触发条件、执行步骤、工具契约、模板、参考资料和验证方法封装成 Agent 可加载能力。；Skill 不是长 prompt；trigger 和 scope 决定能否稳定复用；eval 证明 Skill 有价值。
- 再讲运行机制：book-to-skill、taste-skill、架构图 skill 都说明工程知识正在被打包成可加载过程资产。；Skill 要渐进加载，不应把所有材料塞进上下文。。
- 工程实现要落到可执行设计：Skill 包含 trigger、scope、instruction、references、templates、scripts、eval、version。；指标看 task_success_rate、eval_pass_rate、user_revision_rate。。
- 如果被要求画设计，可以按这个结构展开：。
- 最后补风险、指标和取舍：trigger 过宽；模板化严重；没有版本和回归评测；跟踪 task_success_rate；跟踪 eval_pass_rate；跟踪 user_revision_rate；跟踪 skill_misfire_rate；跟踪 latency_overhead。
...
### 追问 1：如何设计一个可版本化、可评测、不会污染上下文的 Skill？
...
**回答要点**：我会按输入、状态、执行、验证和指标展开：Skill 包含 trigger、scope、instruction、references、templates、scripts、eval、version。；指标看 task_success_rate、eval_pass_rate、user_revision_rate。；为 Skill 定义触发条件、反例、适用文件类型和风险等级。；入口 instruction 只放流程和验证点，长参考放 references，模板放 templates。；工具调用受 tool contract、权限、超时和 side effect 约束。；用 golden tasks 对比启用前后的 eval_pass_rate 和 user_revision_rate。。项目表达里要说明模块边界、数据流、错误恢复和上线后的指标，而不是只说用了某个框架。

**考察点**：系统设计、项目证据

### 追问 3：线上出问题时，你会看哪些日志、trace 或指标？

**回答要点**：我会先按失败类型归因，再看 trace 中的输入、状态、工具参数、返回结果、耗时、成本和 verdict；重点指标包括 task_success_rate；eval_pass_rate；user_revision_rate；skill_misfire_rate；latency_overhead。如果问题来自设计边界，还要回到 schema、权限、上下文和评测集，而不是只调 prompt。

**考察点**：trace、指标归因

### 延伸追问 1：如何设计一个可版本化、可评测、不会污染上下文的 Skill？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；Skill 包含 trigger、scope、instruction、references、templates、scripts、eval、version。；指标看 task_success_rate、eval_pass_rate、user_revision_rate。
