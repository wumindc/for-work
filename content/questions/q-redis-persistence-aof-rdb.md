# Redis RDB 和 AOF 有什么区别？如何设计持久化和恢复策略？

## 面试定位

这道题关联 Redis RDB、AOF 与持久化恢复、Redis 主从复制、哨兵与 Cluster，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Redis RDB、AOF 与持久化恢复」切入：Redis 持久化题要区分 RDB 快照、AOF 追加日志、fsync 策略、rewrite、恢复时间和数据丢失窗口。
...
### 延伸追问 1：为什么 AOF 也可能丢数据？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以讲限流状态、排行榜、配置缓存和任务状态的可丢失窗口。

### 延伸追问 2：AOF rewrite 有什么风险？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以讲限流状态、排行榜、配置缓存和任务状态的可丢失窗口。

### 延伸追问 3：Redis 重启后如何避免 DB 被打爆？
...
- [Redis Documentation](https://redis.io/docs/latest/)：用于确认 Redis 持久化、恢复和高可用相关基础语义。
- [Redis Persistence](https://redis.io/docs/latest/operate/oss_and_stack/management/persistence/)：用于支持 RDB、AOF、fsync、rewrite、恢复时间和数据丢失窗口的取舍。
- [Redis Replication](https://redis.io/docs/latest/operate/oss_and_stack/management/replication/)：用于说明持久化与主从复制、故障切换和写入丢失窗口的关系。
