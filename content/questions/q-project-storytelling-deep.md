...
图 1：可信失败案例从用户可见影响开始，经由 trace replay 定位根因，再进入修复、回归评测和指标复盘。

这张图强调的是证据方向：先讲用户或评测集看到的失败，再回放 `run_id`、`step_id`、工具调用、检索证据和 verifier verdict，最后才讲改了什么。这样回答时不会把“后来优化了一下 prompt”当成根因，也不会把没有证据的猜测包装成事故复盘。Metric improvement 必须同时包含收益和代价，例如 unsupported claim 下降但延迟上升。

数据流是从现象回到 trace，再从 trace 找到模块责任。不要只停留在“模型答错了”。如果没有真实线上事故，也可以用离线 regression 失败样本，但要明确样本来源和影响边界。
...
回答要点：我会拿 trace replay 说明失败路径：输入样本是什么、检索返回了哪些 evidence、rerank 排序如何、generator 生成了哪个 claim、verifier 为什么漏判。然后展示修复前后同一 regression case 的 verdict 和指标变化。根因要能映射到模块 contract，比如 citation verifier 只检查链接存在，没有做 claim-to-span 支持判断。

考察点：面试官在确认你是否真的复盘过，而不是事后补故事。

容易掉坑：只说“我们发现是 prompt 不够严谨”，但没有日志、样本、trace 字段或可复现步骤。

### 追问 2：如果没有线上用户，失败案例还能讲吗？

回答要点：可以讲，但要诚实说这是开发期、离线 eval 或灰度样本。关键是样本真实、可复现、有修复和回归，而不是一定要线上事故。比如“在 120 条 citation regression 中有 12 条引用不支持 claim”，这比虚构线上故障更可信。

考察点：是否能把边界讲清楚。

容易掉坑：把离线测试失败包装成大规模线上事故，后续被追问影响范围、SLA 和告警时露出矛盾。

### 追问 3：修复后怎么防止同类问题复发？

回答要点：我会把失败样本抽象成 bucket，而不是只记住单个 case。比如 citation_gap bucket 下新增 hard negative、claim verifier、发布门禁和 dashboard 指标；下次出现相似路径时，不论 query 是否相同，都能被 regression 或告警捕获。

考察点：是否能从单点修复上升到质量体系。

容易掉坑：只把那条样本加入测试，导致换个表达方式又失败。
...
- [OpenAI Agents SDK Tracing](https://openai.github.io/openai-agents-python/tracing/)：用于支持失败复盘应落到 run、span、tool call 和模型步骤，而不是只看最终答案。
- [LangSmith Evaluation](https://docs.smith.langchain.com/evaluation)：用于支持把失败样本纳入 regression set，用实验结果证明修复有效。
- [OpenTelemetry Traces](https://opentelemetry.io/docs/concepts/signals/traces/)：用于支持 trace 作为跨组件排障证据，帮助定位失败发生在哪个服务或步骤。
