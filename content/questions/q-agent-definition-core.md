...
图 1：LLM Agent 的最小闭环结构。图中 Goal 定义任务边界和成功标准，Context Builder 组织本轮可见证据，Model 只负责提出下一步 action，Tools 返回 observation，State 保存可信进展，Eval + Stop Policy 决定继续、重试、降级或停止，Guardrails 对工具和输出做风险控制。

这张图的关键不是“模型更聪明”，而是闭环控制。Agent 的每一步都应该能回答三个问题：它为什么采取这个 action，action 依赖哪些上下文和状态，执行后凭什么认为任务更接近完成。缺少 trace、state 或 verifier 的系统，即使接了很多工具，也只是难以复盘的 tool-calling demo。
...
事故处理要把影响面、止血、根因和回归分开。影响面先确认是某类任务成功率下降、工具错误增多、循环发散，还是越权动作被误放行；止血可以收紧 step budget、关闭高风险工具、把写操作切到人工确认；根因要查 goal、state_version、context_refs、tool args、observation、guardrail verdict 和 stop_reason；回归要把失败 run 变成 golden trajectory，而不是只在 prompt 里追加一句“请谨慎”。
...
**追问 1：Function calling 是否等于 Agent？**  
答题要点：function calling 只是 action 执行机制；只有当系统有 goal、state、feedback loop、verifier 和 stop policy，才构成 Agent。考察点是概念边界。陷阱是把一次函数调用包装成 Agent。

**追问 2：Agent 的“自主”应该放在哪里？**  
答题要点：模型可以自主选择下一步候选 action，但权限、状态持久化、工具执行、成本预算和回滚由宿主系统控制。考察点是生产级责任划分。陷阱是让模型直接承担安全和事务责任。

**追问 3：最小可上线 Agent 需要哪些日志字段？**  
答题要点：至少记录 run_id、goal、state_version、context_refs、action_type、tool_call、observation_id、verifier_verdict、risk_flags、stop_reason。考察点是可观测性。陷阱是只存聊天文本。
...
- [OpenAI Agents SDK](https://openai.github.io/openai-agents-python/)：官方文档用于支持 Agent 应包含规划、工具调用、协作和状态等多步运行能力。
- [OpenAI Agents SDK Tracing](https://openai.github.io/openai-agents-python/tracing/)：官方文档用于说明 LLM generation、tool call、handoff、guardrail 等运行事件需要进入 trace。
- [OpenAI Agents SDK Guardrails](https://openai.github.io/openai-agents-python/guardrails/)：官方文档用于支撑输入和输出侧风险检查在 Agent 架构中的位置。
- [OpenAI A practical guide to building agents](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf)：官方指南用于补充目标、工具、人类确认、评估和上线边界。
- [Anthropic Building effective agents](https://www.anthropic.com/research/building-effective-agents)：工程文章用于区分 workflow、agent 和 tool use 的适用边界。
