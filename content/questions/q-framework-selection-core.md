...
图 1：Agent 框架选型的基线、矩阵与适配层。先用 Native baseline 建立最低可行实现，再用同一批 golden cases 对比候选框架，最后通过 Adapter Layer 把业务逻辑和具体框架隔离。

这张图的边界是：框架不是起点，需求画像和 baseline 才是起点。`Framework Matrix` 只回答“引入框架是否比原生实现更好”，`Adapter Layer` 则回答“以后替换模型、状态存储或框架时会不会牵动业务代码”。如果没有 baseline 和 eval gate，选型很容易变成追热点。
...
追问 1：LangGraph 和 Agents SDK 应该怎么取舍？
答：先看运行时问题。状态图、checkpoint、恢复、人机协同和长流程更偏 LangGraph；OpenAI tools、handoffs、guardrails、tracing 一体化更偏 Agents SDK。两者都不是默认答案，简单任务仍应先用 Native API baseline。考察点是按能力选型；陷阱是按名称或热度选型。

追问 2：Adapter Layer 会不会过度设计？
答：小原型可能是，但生产系统很值得。Adapter 不需要一开始覆盖所有抽象，只要隔离模型调用、工具派发、状态存储、trace 和 eval 这些最容易变化的边界。考察点是控制抽象粒度；陷阱是把框架 API 深埋在业务流程里，后续无法迁移。

追问 3：如何量化框架带来的收益？
答：同一批 golden cases 对比 baseline 和框架实现，观察成功率、恢复率、调试时间、trace 覆盖、成本、延迟和缺陷定位时间。如果成功率没提升，调试更慢，迁移风险更高，就不应该为了“看起来 Agentic”而引入。考察点是工程验证；陷阱是只看 demo 跑通。
...
- [LangChain Frameworks, runtimes, and harnesses](https://docs.langchain.com/oss/python/concepts/products)：官方文档用于支持区分 framework、runtime、harness，避免把所有 Agent 能力都混成一个框架名。
- [LangGraph Overview](https://docs.langchain.com/oss/python/langgraph/overview)：用于支持状态图、持久化和可恢复执行更适合复杂状态流程的判断。
- [OpenAI Agents SDK Guardrails](https://openai.github.io/openai-agents-python/guardrails/)：用于支持 Agents SDK 在 guardrails、tools、handoffs 和 tracing 组合治理上的适用场景。
