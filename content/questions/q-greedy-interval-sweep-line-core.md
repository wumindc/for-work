# 贪心、区间问题与扫描线 的核心机制是什么？

## 面试定位

这道题关联 贪心、区间问题与扫描线，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「贪心、区间问题与扫描线」切入：贪心题通过局部最优推进全局最优，区间和扫描线常先排序，再维护当前覆盖、重叠、资源数量或事件增减。
...
先给定义和边界：贪心是在每一步选择当前看来最优的策略。；区间问题处理一组开始和结束位置。；扫描线按事件顺序维护当前状态。；贪心题通过局部最优推进全局最优，区间和扫描线常先排序，再维护当前覆盖、重叠、资源数量或事件增减。；贪心要能证明。
...
- 先给定义和边界：贪心是在每一步选择当前看来最优的策略。；区间问题处理一组开始和结束位置。；扫描线按事件顺序维护当前状态。；贪心题通过局部最优推进全局最优，区间和扫描线常先排序，再维护当前覆盖、重叠、资源数量或事件增减。；贪心要能证明。
- 再讲机制：贪心必须有正确性证明。；排序规则决定贪心选择。；端点开闭影响重叠判断。；扫描线事件同点顺序要按题意处理。；区间合并、无重叠区间、会议室、跳跃游戏和分发资源常用贪心。。
- 工程落地要说清楚：Sort by end time。；Interval merge。；Meeting room heap。；Sweep line events。；选择最多不重叠区间通常按结束时间排序。；会议室数量可用最小堆维护最早结束时间。。
- 最后补指标、失败模式和取舍：greedy_proof_quality；interval_boundary_errors；event_order_errors；heap_size；time_complexity；局部选择无法证明；区间端点开闭搞错；事件同位置排序规则错误。
...
### 追问 1：如果面试官深挖 贪心、区间问题与扫描线 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：贪心是在每一步选择当前看来最优的策略。；区间问题处理一组开始和结束位置。；扫描线按事件顺序维护当前状态。；贪心题通过局部最优推进全局最优，区间和扫描线常先排序，再维护当前覆盖、重叠、资源数量或事件增减。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：选择最多不重叠区间通常按结束时间排序。；会议室数量可用最小堆维护最早结束时间。；差分数组是离散扫描线的一种。；贪心面试必须给交换论证、反证或不变量，不能只说感觉。；区间排序要明确按 start、end 还是两者组合。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：greedy_proof_quality；interval_boundary_errors；event_order_errors；heap_size；time_complexity。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 贪心、区间问题与扫描线 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；贪心面试必须给交换论证、反证或不变量，不能只说感觉。；区间排序要明确按 start、end 还是两者组合。
