# 容器运行时、资源隔离与安全边界 的核心机制是什么？

## 面试定位

这道题关联 容器运行时、资源隔离与安全边界，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「容器运行时、资源隔离与安全边界」切入：容器通过 namespace、cgroup、filesystem、capability 和安全策略隔离进程、网络、挂载点和资源，但隔离边界仍弱于虚拟机。
...
先给定义和边界：容器运行时负责创建、启动、停止和隔离容器进程。；Namespace 控制进程看到什么资源视图，cgroup 控制能用多少资源。；容器安全边界由内核能力、文件系统权限、网络策略和运行用户共同决定。；容器通过 namespace、cgroup、filesystem、capability 和安全策略隔离进程、网络、挂载点和资源，但隔离边界仍弱于虚拟机。；namespace 隔离视图。
...
- 先给定义和边界：容器运行时负责创建、启动、停止和隔离容器进程。；Namespace 控制进程看到什么资源视图，cgroup 控制能用多少资源。；容器安全边界由内核能力、文件系统权限、网络策略和运行用户共同决定。；容器通过 namespace、cgroup、filesystem、capability 和安全策略隔离进程、网络、挂载点和资源，但隔离边界仍弱于虚拟机。；namespace 隔离视图。
- 再讲机制：容器共享宿主机内核，所以内核漏洞和过高权限仍可能影响宿主机。；资源限制是稳定性边界，也是容量规划输入。；只读根文件系统和非 root 用户能减少运行时写入和权限风险。；挂载 Docker socket、host network 或 privileged 模式会显著放大风险。；Namespace 让进程看到独立的 PID、网络、挂载、UTS、IPC 等视图。。
- 工程落地要说清楚：Least privilege container。；Read-only root filesystem。；Seccomp/AppArmor profile。；Resource requests and limits。；为 CPU、memory、ephemeral storage 设置 requests/limits，并把 OOMKilled 纳入告警。；把临时文件写入明确挂载点，避免根文件系统不可写导致启动失败。。
- 最后补指标、失败模式和取舍：container_cpu_usage；container_memory_working_set；oom_killed_count；restart_count；security_policy_violation_count；容器内 root 等于安全；不设内存限制导致节点被打爆；把 Docker socket 挂进业务容器。
...
### 追问 1：如果面试官深挖 容器运行时、资源隔离与安全边界 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：容器运行时负责创建、启动、停止和隔离容器进程。；Namespace 控制进程看到什么资源视图，cgroup 控制能用多少资源。；容器安全边界由内核能力、文件系统权限、网络策略和运行用户共同决定。；容器通过 namespace、cgroup、filesystem、capability 和安全策略隔离进程、网络、挂载点和资源，但隔离边界仍弱于虚拟机。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：为 CPU、memory、ephemeral storage 设置 requests/limits，并把 OOMKilled 纳入告警。；把临时文件写入明确挂载点，避免根文件系统不可写导致启动失败。；高风险工具执行放入隔离容器，限制网络、文件系统、CPU、内存和超时。；Java 容器要关注 JVM 对 cgroup 内存和 CPU 限制的识别，避免 OOMKilled 和 GC 抖动。；生产容器默认最小权限，禁止 privileged、hostPath 和宿主网络，除非有明确审批。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：container_cpu_usage；container_memory_working_set；oom_killed_count；restart_count；security_policy_violation_count。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 容器运行时、资源隔离与安全边界 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；Java 容器要关注 JVM 对 cgroup 内存和 CPU 限制的识别，避免 OOMKilled 和 GC 抖动。；生产容器默认最小权限，禁止 privileged、hostPath 和宿主网络，除非有明确审批。
