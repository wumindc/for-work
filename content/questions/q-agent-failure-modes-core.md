...
然后讲工程控制：schema validation、permission gate、stop policy、state rollback、human handoff 和 eval set。复杂 Agent 的可靠性来自这些系统设计，而不是期待模型持续保持正确。
...
图 1：复杂 Agent 的错误放大链路。数据流从 Step Error 进入 State Diff，再影响 Next Context 和 Next Action，最后放大为更难定位的多步失败；Trace 与 Failure Taxonomy 的作用是把 first bad step 找出来，并把修复样本沉淀成 Regression。图中最危险的是错误 observation 写入 State 后继续影响下一轮 Context。
...
### 追问 1：复杂 Agent 出错时先看哪里？

回答要点：先看 trace 里的 first bad step，而不是只看最终错误答案；逐步定位 goal、context、tool、state、policy、verifier 哪一层首次偏离。考察点是排障方法。容易踩坑的是直接改 prompt，忽略工具参数、状态写入和权限判断。

### 追问 2：如何处理上下文污染？

回答要点：外部网页、检索 chunk、用户上传内容都打 trust label，只能作为 data，不能覆盖 system/developer 指令；高风险工具调用要经过 permission gate 和 verifier。考察点是 instruction/data separation。容易踩坑的是把“不可信文档说要忽略规则”也当成指令执行。

### 追问 3：失败样本如何进入回归？

回答要点：保存输入、trace、工具返回、state diff、policy version、expected verdict 和修复版本；后续模型、prompt、tool schema 或 verifier 升级都要跑这批样本。考察点是 eval 工程化。容易踩坑的是只做人工复盘，不把失败转成可重复评测。
...
- [Anthropic Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)：用于支持复杂 Agent 需要 workflow、evaluator、tool use 和 human feedback 等可控模式组合。
- [OpenAI A practical guide to building agents](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf)：用于说明 tools、guardrails、handoff 和 eval 是 Agent 工程可靠性的基础构件。
- [OWASP Top 10 for LLM Applications - Prompt Injection](https://genai.owasp.org/llmrisk/llm01-prompt-injection/)：用于支撑上下文污染、指令/数据隔离和不可信内容边界的安全讨论。
