# volatile 修饰的变量执行 i++ 线程安全吗？为什么？

## 面试定位

这道题关联 Java 内存模型、volatile 与 happens-before、ConcurrentHashMap、CAS 与并发集合，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
...
### 延伸追问 1：AtomicInteger 和 LongAdder 怎么选？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以讲接口 QPS 计数、本地限流、任务成功数、Agent run 统计。；用 cas_retry_count、contention、统计误差和业务一致性要求做取舍。
