# 链表虚拟头节点、反转与快慢指针 的核心机制是什么？

## 面试定位

这道题关联 链表虚拟头节点、反转与快慢指针，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「链表虚拟头节点、反转与快慢指针」切入：链表题考察指针操作、边界保护和原地修改，常用虚拟头节点、三指针反转、快慢指针和递归分治。
...
先给定义和边界：链表由节点和 next 指针串联。；虚拟头节点是指向原头节点前的辅助节点。；快慢指针用不同速度遍历链表。；链表题考察指针操作、边界保护和原地修改，常用虚拟头节点、三指针反转、快慢指针和递归分治。；dummy 简化头节点。
...
- 先给定义和边界：链表由节点和 next 指针串联。；虚拟头节点是指向原头节点前的辅助节点。；快慢指针用不同速度遍历链表。；链表题考察指针操作、边界保护和原地修改，常用虚拟头节点、三指针反转、快慢指针和递归分治。；dummy 简化头节点。
- 再讲机制：头节点可能变化就用 dummy。；指针重连顺序必须先保存后断开。；快慢指针速度差决定相遇或定位语义。；链表题用 O(1) 空间更常见。；删除、插入、合并等涉及头节点变化的题优先使用 dummy。。
- 工程落地要说清楚：Dummy node。；Three-pointer reverse。；Fast/slow pointer。；Merge sort on linked list。；反转模板使用 prev、cur、next。；环检测相遇后从 head 和 meet 同步走找入口。。
- 最后补指标、失败模式和取舍：pointer_rewire_errors；dummy_usage_rate；edge_case_pass_rate；space_complexity；redo_pass_rate；忘记返回 dummy.next；反转时丢失后继节点；偶数长度中点定义不一致。
- 链表题考察指针操作、边界保护和原地修改，常用虚拟头节点、三指针反转、快慢指针和递归分治。
- 链表由节点和 next 指针串联。
- 虚拟头节点是指向原头节点前的辅助节点。
- 快慢指针用不同速度遍历链表。
- 头节点可能变化就用 dummy。
...
- 快慢指针速度差决定相遇或定位语义。
- 链表题用 O(1) 空间更常见。
- 删除、插入、合并等涉及头节点变化的题优先使用 dummy。
- 快慢指针可以检测环、找中点、找倒数第 k 个节点。
...
- 反转模板使用 prev、cur、next。
- 环检测相遇后从 head 和 meet 同步走找入口。
- 合并 k 个链表可用堆或分治。
- 写链表题要先画指针变化顺序，再写代码。
- 每次改 next 前先保存后继，避免链断裂。
...
### 追问 1：如果面试官深挖 链表虚拟头节点、反转与快慢指针 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：链表由节点和 next 指针串联。；虚拟头节点是指向原头节点前的辅助节点。；快慢指针用不同速度遍历链表。；链表题考察指针操作、边界保护和原地修改，常用虚拟头节点、三指针反转、快慢指针和递归分治。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：反转模板使用 prev、cur、next。；环检测相遇后从 head 和 meet 同步走找入口。；合并 k 个链表可用堆或分治。；写链表题要先画指针变化顺序，再写代码。；每次改 next 前先保存后继，避免链断裂。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：pointer_rewire_errors；dummy_usage_rate；edge_case_pass_rate；space_complexity；redo_pass_rate。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 链表虚拟头节点、反转与快慢指针 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；写链表题要先画指针变化顺序，再写代码。；每次改 next 前先保存后继，避免链断裂。
