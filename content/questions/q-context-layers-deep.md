...
图 1：外部证据隔离与工具权限二次校验链路。外部文档先被标记为不可信 evidence，再进入 prompt manifest；模型生成 tool_call 后，还要经过独立的 Permission Gate 才能执行。

这张图的重点是把“文本可信度”和“动作授权”拆开。恶意网页、邮件、PDF chunk 可以作为事实证据被引用，但不能改变 system policy、输出契约和工具权限。`Prompt Manifest` 负责让模型知道哪些内容是证据，`Tool Permission Gate` 负责决定动作能不能发生，两者缺一都会留下越权空间。
...
追问 1：把恶意 span 删除是不是更安全？
答：不一定。删除可以降低注入风险，但也可能破坏证据完整性和可溯源性。更稳的做法是保留原文、标记 dangerous span、限制其只能作为被引用内容，并在高风险任务中降低或排除该 evidence。考察点是安全和可审计的平衡；陷阱是简单清洗导致后续无法解释模型为什么忽略某段材料。

追问 2：如果 evidence 和用户指令冲突，谁优先？
答：用户任务和系统策略优先。evidence 只能提供世界事实，不能改写任务意图、输出格式、权限和安全策略。若 evidence 声称“用户已经授权删除数据”，也必须由业务 ACL 或显式用户确认验证。考察点是上下文层级；陷阱是把来源文本当成新的指令。

追问 3：如何证明 Permission Gate 不是摆设？
答：要做注入 fixture 和越权 tool_call 回归测试。给模型恶意网页、邮件和 RAG chunk，诱导它调用删除、发送、跨租户读取等工具，检查 gate 是否基于 user、tenant、resource、riskLevel、requiresConfirmation 独立拒绝或确认。考察点是执行层确定性；陷阱是 gate 只检查模型给出的自然语言理由。

## 项目化回答

我会说：我的 Context Builder 输出 manifest，每个证据都有 trustLevel。外部内容不应改写 system 层、任务目标和工具权限。工具执行还要经过 Permission Gate，所以即使模型被诱导，也不能越权执行。
...
- [LangChain Context engineering](https://docs.langchain.com/oss/python/langchain/context-engineering)：官方文档用于支持把上下文按任务、状态、记忆、证据和工具说明分层管理的设计。
- [OpenAI Agents SDK Guardrails](https://openai.github.io/openai-agents-python/guardrails/)：用于支持输入、输出与工具调用前后加 guardrail 的执行层治理思路。
- [OWASP LLM01: Prompt Injection](https://genai.owasp.org/llmrisk/llm01-prompt-injection/)：用于支持把 prompt injection 视为 LLM 应用的一类核心风险，而不是普通提示词问题。
