...
图 1：短期记忆从可信 State 到模型输入的投影链路。Trusted State 先由 Projector 生成 working memory，scratchpad 和 recent trace 再按 TTL、证据状态和预算进入 Context Budgeter，最终组成本轮 Model Input。

这张图的边界是：短期记忆不是最近聊天记录，也不是长期用户画像。它是当前任务的状态投影，必须区分可信事实、临时假设和最近观察。Observation 先进入 State 或 checkpoint，下一轮再被投影；未经验证的 scratchpad 不能直接升级为可信 State。
...
追问 1：为什么不能直接把最近 N 轮对话放进上下文？
答：最近对话包含重复、旧假设、被推翻结论和无关寒暄。短期记忆要从 State 投影出当前任务需要的 goal、constraints、plan、open risks 和 next actions，同时保留 source id 和 state_version。考察点是状态建模；陷阱是把聊天历史当工作记忆。

追问 2：scratchpad 里的假设什么时候能进入 State？
答：只有被工具结果、测试、引用证据或用户确认支持后才能升级。升级时要记录 evidence_status、source、版本和过期条件；如果新证据推翻它，要标记 superseded。考察点是可信度分层；陷阱是让“可能是缓存问题”直接变成根因。

追问 3：Context Budgeter 裁剪时最不能丢什么？
答：目标、硬约束、高风险权限、未完成动作、最近失败和关键 artifact refs 不能丢。大日志、网页全文和重复 observation 可以保留摘要、hash 和引用。考察点是预算策略；陷阱是按长度或时间简单裁剪，导致约束丢失。
...
- [LangChain Short-term memory](https://docs.langchain.com/oss/python/langchain/short-term-memory)：官方文档用于支持短期记忆是线程内状态管理问题，而不是无限追加聊天记录。
- [LangGraph Persistence](https://docs.langchain.com/oss/python/langgraph/persistence)：用于支持 checkpoint 和 state persistence 可以作为可恢复状态来源。
- [LangChain Context engineering](https://docs.langchain.com/oss/python/langchain/context-engineering)：用于支持按任务、状态、记忆、证据和工具说明组织上下文，减少污染和丢约束。
