...
这篇文章对外发布时，需要把 Graph 化从“框架选择”拉回“控制流治理”。读者应该能判断：如果系统只有一次工具调用、线性问答或低风险短任务，代码式 loop 就足够；如果系统出现恢复点、人工审批、多分支路由、状态合并和可回放需求，Graph 才有明显收益。Graph 不是高级感，而是把状态迁移变成可审计对象。

迁移前最好做一个 state transition inventory：当前 loop 里有哪些隐式状态、哪些 if/else 决定路由、哪些工具有副作用、哪些失败需要从中间恢复。只有这些东西被列出来，才能设计 State schema、node 粒度、edge 条件、reducer 和 checkpoint。否则只是把原来的混乱 loop 画成图，隐藏副作用仍然存在。

公开读者还需要看到失败成本。Graph 会引入建模成本、运行开销和团队学习成本，所以要用指标证明收益，例如 checkpoint 恢复成功率上升、排障时间下降、重复副作用减少、人工审批路径更清晰。如果这些指标没有改善，Graph 化就只是换框架；如果这些指标改善明显，它就是生产 Agent 的控制面升级。
