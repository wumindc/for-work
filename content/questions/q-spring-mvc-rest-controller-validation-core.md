# Spring MVC、REST Controller 与参数校验 的核心机制是什么？

## 面试定位

这道题关联 Spring MVC、REST Controller 与参数校验，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Spring MVC、REST Controller 与参数校验」切入：Spring MVC 将 HTTP 请求映射到 Controller 方法，参数绑定、校验、消息转换和返回值处理共同决定 REST API 的契约质量。
...
**不要只答**
- Controller 里堆业务和事务
- 只校验 body 不校验 path/query
- 返回中文 message 给前端做分支

## 30 秒回答

先给定义和边界：Spring MVC 是基于 Servlet 的 Web MVC 框架。；REST Controller 将资源操作表达为 HTTP API。；参数校验是 API 契约的一部分。；Spring MVC 将 HTTP 请求映射到 Controller 方法，参数绑定、校验、消息转换和返回值处理共同决定 REST API 的契约质量。；HandlerMapping 找方法。
...
- 先给定义和边界：Spring MVC 是基于 Servlet 的 Web MVC 框架。；REST Controller 将资源操作表达为 HTTP API。；参数校验是 API 契约的一部分。；Spring MVC 将 HTTP 请求映射到 Controller 方法，参数绑定、校验、消息转换和返回值处理共同决定 REST API 的契约质量。；HandlerMapping 找方法。
- 再讲机制：HTTP 方法、状态码和错误码要有语义。；输入校验在服务端必须可信执行。；DTO 和领域对象要区分。；API 变更要兼容旧客户端。；请求进入 DispatcherServlet 后经过 HandlerMapping、HandlerAdapter、参数解析、消息转换、Controller、返回值处理和异常处理。。
- 工程落地要说清楚：DispatcherServlet pipeline。；Bean Validation。；Request/response DTO。；OpenAPI/contract test。；对外接口保留 trace_id/request_id 方便定位。；分页接口限制 page size 并声明排序白名单。。
- 最后补指标、失败模式和取舍：api_p95；validation_error_rate；contract_break_count；idempotency_conflict_count；4xx_5xx_rate；Controller 里堆业务和事务；只校验 body 不校验 path/query；返回中文 message 给前端做分支。
- Spring MVC 将 HTTP 请求映射到 Controller 方法，参数绑定、校验、消息转换和返回值处理共同决定 REST API 的契约质量。
- Spring MVC 是基于 Servlet 的 Web MVC 框架。
- REST Controller 将资源操作表达为 HTTP API。
- 参数校验是 API 契约的一部分。
- HTTP 方法、状态码和错误码要有语义。
- 输入校验在服务端必须可信执行。
- DTO 和领域对象要区分。
- API 变更要兼容旧客户端。
- 请求进入 DispatcherServlet 后经过 HandlerMapping、HandlerAdapter、参数解析、消息转换、Controller、返回值处理和异常处理。
- 参数校验应覆盖 path、query、body、header 和业务语义，不只依赖前端。
...
- 对外接口保留 trace_id/request_id 方便定位。
- 分页接口限制 page size 并声明排序白名单。
- 高风险写接口加入 idempotency key 和权限检查。
- API 要统一 request_id、错误码、分页、排序、幂等键和 schema 版本。
- Controller 保持薄层，复杂业务进入 service，避免事务和 HTTP 细节缠在一起。
...
### 追问 1：如果面试官深挖 Spring MVC、REST Controller 与参数校验 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：Spring MVC 是基于 Servlet 的 Web MVC 框架。；REST Controller 将资源操作表达为 HTTP API。；参数校验是 API 契约的一部分。；Spring MVC 将 HTTP 请求映射到 Controller 方法，参数绑定、校验、消息转换和返回值处理共同决定 REST API 的契约质量。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：对外接口保留 trace_id/request_id 方便定位。；分页接口限制 page size 并声明排序白名单。；高风险写接口加入 idempotency key 和权限检查。；API 要统一 request_id、错误码、分页、排序、幂等键和 schema 版本。；Controller 保持薄层，复杂业务进入 service，避免事务和 HTTP 细节缠在一起。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：api_p95；validation_error_rate；contract_break_count；idempotency_conflict_count；4xx_5xx_rate。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 Spring MVC、REST Controller 与参数校验 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-web-agent、pe-coding-agent；API 要统一 request_id、错误码、分页、排序、幂等键和 schema 版本。；Controller 保持薄层，复杂业务进入 service，避免事务和 HTTP 细节缠在一起。
...
**项目证据怎么挂钩**
- 可以关联项目证据：pe-web-agent、pe-coding-agent
- API 要统一 request_id、错误码、分页、排序、幂等键和 schema 版本。
- Controller 保持薄层，复杂业务进入 service，避免事务和 HTTP 细节缠在一起。
