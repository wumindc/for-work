...
图 1：Tool Use 从候选工具注入、模型生成 tool_call 到宿主执行和 observation 回写的闭环。

这张图里，Context Builder 先根据任务和权限挑选候选工具，而不是把所有工具都塞进上下文。Model 只生成 `tool_call name + arguments`，不直接执行外部动作。宿主程序依次经过 JSON Schema Validation、Permission and Risk Gate、Tool Dispatcher 和真实 API/Runtime，把结果标准化成 Structured Observation。Observation 写入 State + Trace Store 后，下一轮 Context Builder 才能基于真实证据继续推理。这个闭环明确了模型、宿主和外部系统的责任边界。
...
第一轮追问：模型到底是在“调用工具”还是“请求工具”？
回答要点：模型只输出工具名和结构化参数，宿主程序负责校验、授权、执行、归一化结果和审计。考察点是执行边界。陷阱是说模型直接拿 API key 调后端。

第二轮追问：工具失败后模型应该看到什么？
回答要点：看到结构化 observation 或 error envelope，包括 `error_code`、`retryable`、`safe_to_retry`、`partial_data`、`hint` 和 `correlation_id`，而不是异常栈。考察点是错误恢复能力。陷阱是返回一段不可解析日志。

第三轮追问：写工具和读工具的权限策略有什么不同？
回答要点：读工具可以更自动化，但仍需权限和脱敏；写工具要 preview、confirmation、idempotency key、audit log 和必要的 rollback/compensation。考察点是 side effect 风险分级。陷阱是把所有工具按同一策略放行。

第四轮追问：如何评估 Tool Use 设计是否稳定？
回答要点：按调用链看 `tool_selection_accuracy`、`valid_call_rate`、`invalid_args_rate`、`permission_denial_rate`、`tool_latency_p95` 和 `tool_chain_success_rate`。考察点是指标分层。陷阱是只看最终回答满意度。
...
- [OpenAI Function Calling](https://platform.openai.com/docs/guides/function-calling)：用于说明模型如何输出结构化工具调用参数，以及为什么 schema 会影响调用稳定性。
- [OpenAI A practical guide to building agents](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf)：用于支撑工具执行、人类确认、评估和 trace 的 agent 工程实践。
