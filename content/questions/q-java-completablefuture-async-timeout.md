# CompletableFuture 并行调用多个下游，如何处理超时、异常和线程池？
...
- 使用 common pool
- 只在最外层超时
- 异常被 CompletionException 包住后丢 root cause
...
### 延伸追问 1：thenApply 和 thenApplyAsync 线程语义有什么差异？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以讲订单聚合页、推荐+库存并行查询、Agent 多工具执行、模型 API 并发调用。；用异步 DAG、线程池隔离、timeout/fallback 指标和 trace 传播作为项目证据。

### 延伸追问 2：allOf 里一个失败怎么办？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以讲订单聚合页、推荐+库存并行查询、Agent 多工具执行、模型 API 并发调用。；用异步 DAG、线程池隔离、timeout/fallback 指标和 trace 传播作为项目证据。

### 延伸追问 3：超时后底层任务真的停止了吗？
