# Spring Boot 自动配置、Starter 与条件装配 的核心机制是什么？
...
### 追问 1：如果面试官深挖 Spring Boot 自动配置、Starter 与条件装配 的生产落地和排障，你怎么回答？
...
### 延伸追问 1：如果面试官深挖 Spring Boot 自动配置、Starter 与条件装配 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；排查配置没生效先看 condition evaluation report、属性前缀、自动配置顺序和 Bean 覆盖。；自定义 starter 要提供 AutoConfiguration、Properties、metadata 和合理默认值。
