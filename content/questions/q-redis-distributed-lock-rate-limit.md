# Redis 分布式锁和限流器怎么设计才安全？

## 面试定位

这道题关联 Redis 分布式锁、限流与并发控制、Redis Lua、事务与 Pipeline，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Redis 分布式锁、限流与并发控制」切入：Redis 锁和限流题要讲 SET NX PX、唯一 token、Lua 释放、锁续期、fencing token、Redlock 争议、令牌桶/滑动窗口和失败降级。
...
回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：缓存重建锁、库存预扣、模型 API 限流、Agent 工具执行并发控制。

### 延伸追问 2：锁过期后业务还没执行完怎么办？
...
**项目证据怎么挂钩**
- 缓存重建锁、库存预扣、模型 API 限流、Agent 工具执行并发控制。
...
- [Redis Documentation](https://redis.io/docs/latest/)：用于确认 Redis 命令、过期时间和单线程执行模型的基础语义。
- [Redis: Distributed Locks with Redis](https://redis.io/docs/latest/develop/use/patterns/distributed-locks/)：用于支持 SET NX PX、随机 token、释放校验和 Redlock 边界讨论。
- [Redis Commands](https://redis.io/docs/latest/commands/)：用于核对 SET、EVAL、EXPIRE、ZSet 等命令的参数和复杂度。
