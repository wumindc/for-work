...
1. 追问：Full GC 告警出现后，为什么不能先把堆调大？
   - 回答要点：调大堆只能改变回收频率和可用空间，不能证明根因；如果对象保留路径没有变化，问题会推迟出现，单次 dump、恢复和停顿成本还可能变大。正确动作是先用时间线判断 `allocation_rate`、`heap_used_after_gc`、线程池队列和下游延迟谁先变化，再决定是代码修复、参数调整、限流还是回滚。
   - 考察点：候选人是否把 JVM 参数当成最后手段，而不是第一反应。
   - 常见坑：只说“内存不够就加内存”，没有解释 pause、dump、OOM 和根因掩盖风险。

2. 追问：如何判断内存泄漏和对象分配过快？
   - 回答要点：泄漏更关注多轮 GC 后 `heap_used_after_gc` 是否持续抬高，以及 heap dump 的 dominator tree 和引用链；分配过快更关注 `allocation_rate`、Young GC 频率、JFR allocation event 和 CPU 消耗。两者都可能导致 GC 频繁，但修复方向不同：泄漏要断引用，分配过快要减少对象 churn、分页、池化或调整数据结构。
   - 考察点：能否从“现象”拆到对象生命周期。
   - 常见坑：看到 GC 次数多就直接判断泄漏。

3. 追问：线上 heap dump 有什么风险，怎么安全采集？
   - 回答要点：heap dump 可能很大，可能带来停顿、磁盘占满和敏感数据泄露；要选单个异常实例、确认磁盘空间、设置审批和保留周期，必要时先用 JFR、class histogram、线程 dump 和指标缩小范围。采集后要记录 `dump_id`、实例、窗口和操作人，避免复盘时证据无法追溯。
   - 考察点：是否有真实生产操作意识。
   - 常见坑：事故中同时 dump 所有实例，导致容量二次下降。

4. 追问：如果 GC 只是结果，根因可能在哪里？
   - 回答要点：下游慢、线程池积压、MQ lag、日志暴增、trace buffer 变大、缓存大 value、批处理一次拉太多数据，都可能先让对象存活时间变长，随后 GC 加重。判断方式是拉齐发布事件、业务 p95、队列、下游 p95、allocation、GC pause 的时间线。
   - 考察点：候选人是否能跳出 JVM 看系统链路。
   - 常见坑：把所有 p99 抖动都归因到 GC。
...
- [Oracle Introduction to Garbage Collection Tuning](https://docs.oracle.com/en/java/javase/21/gctuning/introduction-garbage-collection-tuning.html)：用于确认 GC 调优目标、收集器取舍和堆参数并不能替代根因分析。
- [Oracle `jfr` Command](https://docs.oracle.com/en/java/javase/21/docs/specs/man/jfr.html)：用于支持 JFR 作为线上采样、摘要、筛选和事件分析工具的描述。
- [JDK Flight Recorder](https://dev.java/learn/jvm/jfr/)：用于支撑 JFR 低开销观测、分配分析和运行时诊断的工程表述。
- [Prometheus Alerting Rules](https://prometheus.io/docs/prometheus/latest/configuration/alerting_rules/)：用于支持 `gc_pause_p95`、`heap_used_after_gc` 等指标告警和回归验证。
