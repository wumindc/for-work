# Spring 测试切片、集成测试与契约回归 的核心机制是什么？

## 面试定位

这道题关联 Spring 测试切片、集成测试与契约回归，难度 3/5，出现频率 medium。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Spring 测试切片、集成测试与契约回归」切入：Spring 测试体系要区分单元测试、切片测试、集成测试和契约测试，覆盖 Controller、Service、Mapper、事务和外部依赖边界。
...
先给定义和边界：测试切片是只加载某一层 Spring 组件的测试方式。；集成测试验证多个真实组件之间的协作。；契约测试保证服务提供方和消费方对 API 的理解一致。；Spring 测试体系要区分单元测试、切片测试、集成测试和契约测试，覆盖 Controller、Service、Mapper、事务和外部依赖边界。；测试按边界分层。
...
- 先给定义和边界：测试切片是只加载某一层 Spring 组件的测试方式。；集成测试验证多个真实组件之间的协作。；契约测试保证服务提供方和消费方对 API 的理解一致。；Spring 测试体系要区分单元测试、切片测试、集成测试和契约测试，覆盖 Controller、Service、Mapper、事务和外部依赖边界。；测试按边界分层。
- 再讲机制：测试越靠近业务核心越要快。；边界测试要覆盖失败和异常语义。；集成测试验证配置和真实协议。；回归测试要从线上问题反哺。；@WebMvcTest、@DataJdbcTest、@MybatisTest 等切片测试只加载相关组件，适合快速验证边界。。
- 工程落地要说清楚：@SpringBootTest。；@WebMvcTest + MockMvc。；@MybatisTest。；Testcontainers。；Consumer-driven contract。；Controller 测 schema、状态码、错误码和鉴权。。
- 最后补指标、失败模式和取舍：test_pass_rate；spring_context_start_count；contract_break_count；flaky_test_rate；regression_escape_count；所有测试都启动完整 Spring 容器；测试只覆盖 happy path；Mock 太多导致没测真实配置。
- Spring 测试体系要区分单元测试、切片测试、集成测试和契约测试，覆盖 Controller、Service、Mapper、事务和外部依赖边界。
- 测试切片是只加载某一层 Spring 组件的测试方式。
- 集成测试验证多个真实组件之间的协作。
- 契约测试保证服务提供方和消费方对 API 的理解一致。
- 测试越靠近业务核心越要快。
- 边界测试要覆盖失败和异常语义。
- 集成测试验证配置和真实协议。
- 回归测试要从线上问题反哺。
- @WebMvcTest、@DataJdbcTest、@MybatisTest 等切片测试只加载相关组件，适合快速验证边界。
- 完整集成测试验证配置、容器、数据库、事务、序列化和外部依赖替身是否协同。
...
- Controller 测 schema、状态码、错误码和鉴权。
- Mapper 测 SQL、边界参数、事务回滚和慢查询风险。
- 外部依赖用 Testcontainers 或协议级 mock，而不是 mock 业务结果。
- 测试数据要可重复、可清理、可并行，避免依赖本地脏状态。
- 关键 API 用契约测试固定请求响应 schema、错误码和兼容字段。
...
### 追问 1：如果面试官深挖 Spring 测试切片、集成测试与契约回归 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：测试切片是只加载某一层 Spring 组件的测试方式。；集成测试验证多个真实组件之间的协作。；契约测试保证服务提供方和消费方对 API 的理解一致。；Spring 测试体系要区分单元测试、切片测试、集成测试和契约测试，覆盖 Controller、Service、Mapper、事务和外部依赖边界。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：Controller 测 schema、状态码、错误码和鉴权。；Mapper 测 SQL、边界参数、事务回滚和慢查询风险。；外部依赖用 Testcontainers 或协议级 mock，而不是 mock 业务结果。；测试数据要可重复、可清理、可并行，避免依赖本地脏状态。；关键 API 用契约测试固定请求响应 schema、错误码和兼容字段。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：test_pass_rate；spring_context_start_count；contract_break_count；flaky_test_rate；regression_escape_count。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 Spring 测试切片、集成测试与契约回归 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；测试数据要可重复、可清理、可并行，避免依赖本地脏状态。；关键 API 用契约测试固定请求响应 schema、错误码和兼容字段。
