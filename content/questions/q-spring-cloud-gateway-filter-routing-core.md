# Spring Cloud Gateway 路由、过滤器与入口治理 的核心机制是什么？

## 面试定位

这道题关联 Spring Cloud Gateway 路由、过滤器与入口治理，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「Spring Cloud Gateway 路由、过滤器与入口治理」切入：Spring Cloud Gateway 作为微服务入口，负责路由匹配、过滤器链、鉴权、限流、灰度、Header 透传和入口观测。
...
- 网关承担业务编排导致耦合
- 过滤器顺序混乱
- 入口没有保留 trace header 和真实客户端信息

## 30 秒回答

先给定义和边界：Gateway 是微服务入口和路由治理组件。；Predicate 决定请求是否命中路由。；Filter 在请求前后执行横切处理。；Spring Cloud Gateway 作为微服务入口，负责路由匹配、过滤器链、鉴权、限流、灰度、Header 透传和入口观测。；Gateway 是入口治理层。
...
- 先给定义和边界：Gateway 是微服务入口和路由治理组件。；Predicate 决定请求是否命中路由。；Filter 在请求前后执行横切处理。；Spring Cloud Gateway 作为微服务入口，负责路由匹配、过滤器链、鉴权、限流、灰度、Header 透传和入口观测。；Gateway 是入口治理层。
- 再讲机制：入口策略要简单、稳定、可回滚。；鉴权、限流、灰度和观测要在网关与服务间分工明确。；过滤器顺序是语义的一部分。；网关自身必须有高可用和容量保护。；Gateway 根据 predicate 匹配请求，再执行全局和路由过滤器，最终转发到目标服务。。
- 工程落地要说清楚：Route predicate。；GlobalFilter/GatewayFilter。；Rate limiter。；Canary routing。；Trace header propagation。；入口日志记录 route_id、upstream、status、duration、trace_id 和 client_ip。。
- 最后补指标、失败模式和取舍：gateway_request_rate；gateway_p95；route_error_rate；rate_limited_count；canary_error_rate；网关承担业务编排导致耦合；过滤器顺序混乱；入口没有保留 trace header 和真实客户端信息。
- Spring Cloud Gateway 作为微服务入口，负责路由匹配、过滤器链、鉴权、限流、灰度、Header 透传和入口观测。
- Gateway 是微服务入口和路由治理组件。
- Predicate 决定请求是否命中路由。
- Filter 在请求前后执行横切处理。
- 入口策略要简单、稳定、可回滚。
- 鉴权、限流、灰度和观测要在网关与服务间分工明确。
- 过滤器顺序是语义的一部分。
- 网关自身必须有高可用和容量保护。
- Gateway 根据 predicate 匹配请求，再执行全局和路由过滤器，最终转发到目标服务。
- 入口层适合做认证前置、限流、路由、CORS、Header 规范化、日志和灰度策略。
...
- 入口日志记录 route_id、upstream、status、duration、trace_id 和 client_ip。
- 灰度按 header、cookie、租户、权重或版本路由。
- 限流返回标准错误码和 retry_after，避免服务端无声丢请求。
- 不要把复杂业务逻辑塞进网关，网关要保持可观测、可回滚和高性能。
- 路由变更要有版本、灰度、回滚和指标观察。
...
### 追问 1：如果面试官深挖 Spring Cloud Gateway 路由、过滤器与入口治理 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：Gateway 是微服务入口和路由治理组件。；Predicate 决定请求是否命中路由。；Filter 在请求前后执行横切处理。；Spring Cloud Gateway 作为微服务入口，负责路由匹配、过滤器链、鉴权、限流、灰度、Header 透传和入口观测。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：入口日志记录 route_id、upstream、status、duration、trace_id 和 client_ip。；灰度按 header、cookie、租户、权重或版本路由。；限流返回标准错误码和 retry_after，避免服务端无声丢请求。；不要把复杂业务逻辑塞进网关，网关要保持可观测、可回滚和高性能。；路由变更要有版本、灰度、回滚和指标观察。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：gateway_request_rate；gateway_p95；route_error_rate；rate_limited_count；canary_error_rate。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 Spring Cloud Gateway 路由、过滤器与入口治理 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-web-agent、pe-coding-agent；不要把复杂业务逻辑塞进网关，网关要保持可观测、可回滚和高性能。；路由变更要有版本、灰度、回滚和指标观察。
...
- [Spring Cloud Gateway Reference Documentation](https://docs.spring.io/spring-cloud-gateway/reference/)：用于确认官方语义边界、命令行为和工程约束。
