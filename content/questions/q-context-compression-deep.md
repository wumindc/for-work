...
图 1：上下文压缩后的状态保真对账链路。左路从原始 trace 抽取期望状态，右路从压缩产物解析实际状态，中间的 verifier 只负责比较差异，不能用同一份摘要给自己打分。

数据流里要把 expected_state 和 actual_state 分开。这样 verifier 才能判断丢失，而不是对同一份摘要做自我解释。

这张图的关键边界是：压缩不是把所有上下文写短，而是把可恢复运行所需的关键状态保留下来。`safe_to_resume` 只有在目标、硬约束、证据、文件引用、未完成动作和风险提示都能对齐时才成立；否则应该进入 repair compression、补读 artifact 或追问用户，而不是让 Agent 带着不完整状态继续执行。
...
追问 1：只比较字段完整率够不够？
答：不够。字段完整率只能证明 schema 中的槽位没有空，但不能证明语义没有变形。比如 `constraints` 字段存在，却把“不要改数据库 schema”压成“谨慎改数据库”，这就是危险变形。所以要同时检查字段保留、语义一致、artifact 可访问、恢复后行为是否违反约束。考察点是你能否区分结构完整和任务可恢复；陷阱是把 JSON 字段非空当成保真。

追问 2：如果 verifier 判定 unsafe，但用户希望继续怎么办？
答：应该降级而不是硬继续。可以展示缺失字段、风险原因和补救路径：重新压缩、补读指定 artifact、让用户确认缺失约束，或者只允许只读动作。对写文件、发请求、删数据这类副作用动作，要在关键状态不完整时阻断。考察点是风险控制；陷阱是为了流畅体验隐藏不确定性。

追问 3：resume eval 如何构造样本？
答：从真实长任务 trace 中截断上下文，保留原始后半段作为期望行为，再用压缩产物启动 Agent，看它是否能继续找到正确文件、遵守约束、避免重复劳动，并产出相同质量的 artifact。样本要覆盖 coding、RAG、web、文档编辑等任务类型。考察点是评测设计；陷阱是只用人工编造的简单摘要样例。
...
- [LangGraph Persistence](https://docs.langchain.com/oss/python/langgraph/persistence)：官方文档用于支持 checkpoint、state persistence 与恢复语义的设计边界。
- [LangChain Short-term memory](https://docs.langchain.com/oss/python/langchain/short-term-memory)：用于支持短期记忆与上下文状态需要被显式管理，而不是无限塞入 prompt 的判断。
- [LangSmith Evaluation](https://docs.smith.langchain.com/evaluation)：用于支持把 resume success、约束违反和回归样本纳入评测集，而不是只依赖人工阅读摘要。
