...
图 1：RAG 错答排障从最终答案反推正确证据所在层级的决策树。

图中 Wrong Answer 只是排障入口，不是根因。第一层先问正确证据是否进入 topK：如果没有，优先查 ingest、chunk、metadata、embedding、query rewrite、filter 和 retriever。第二层看正确证据是否被 rerank 选入上下文：如果没选中，问题常在 reranker、去重、预算或上下文截断。第三层看答案 claim 是否被 selected evidence 支撑：如果证据在上下文但 claim 不忠实，才重点看 prompt、generator、citation policy 和 verifier。如果 claim 有支撑但用户仍认为错，则可能是文档过期、权限策略、业务口径或用户预期问题。
...
第一轮追问：topK 没有正确证据时，你先改什么？
回答要点：先查 ingest 完整性、chunk 策略、metadata filter、embedding version、BM25/vector 混合召回和 query rewrite 是否丢实体；必要时补 hybrid search 或同义词扩展。考察点是检索层归因。陷阱是直接调生成 prompt。

第二轮追问：正确证据进了 topK，但没进上下文怎么办？
回答要点：检查 rerank 特征、候选去重、context budget、排序阈值、chunk 合并和证据多样性；用 rerank eval 看正确 evidence 是否进入 selected evidence。考察点是候选筛选与预算。陷阱是把所有 topK 都塞进上下文，导致噪声和成本上升。

第三轮追问：证据进了上下文，模型还是答错怎么办？
回答要点：把答案拆成 claim，逐条做 claim-to-evidence verifier；要求引用 evidence id，降低无关上下文，调整 answer contract 或加 abstention policy。考察点是生成忠实度。陷阱是只看 citation 链接存在，不检查 claim 是否真的被支持。

第四轮追问：如何把一次错答沉淀成长期质量提升？
回答要点：给失败样本打 `retrieval_miss`、`rerank_drop`、`unsupported_generation`、`stale_evidence` 等标签，加入 golden set，并在 CI 或离线评测里分别跑 recall、MRR、citation precision 和 faithfulness。考察点是回归体系。陷阱是修一次线上样本但没有防复发机制。
...
- [Elastic Search API](https://www.elastic.co/guide/en/elasticsearch/reference/current/search-search.html)：用于理解检索请求、过滤、候选返回和搜索层排障。
- [OpenAI A practical guide to building agents](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf)：用于支撑 Agent 中工具、评估、追踪和人工审核的工程化做法。
