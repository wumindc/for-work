# 二叉树遍历、深度、高度与递归返回值 的核心机制是什么？

## 面试定位

这道题关联 二叉树遍历、深度、高度与递归返回值，难度 4/5，出现频率 high。面试官真正想看的是：你能否把概念回答升级成架构、数据流、指标、取舍和真实故障处理。
回答主轴可以从「二叉树遍历、深度、高度与递归返回值」切入：二叉树题核心是递归定义和返回值设计，前中后序、层序、深度、高度、路径和子树信息都能由 DFS/BFS 模板表达。
...
先给定义和边界：树是层级结构，二叉树每个节点最多两个子节点。；DFS 深度优先遍历子树。；BFS 按层扩展节点。；二叉树题核心是递归定义和返回值设计，前中后序、层序、深度、高度、路径和子树信息都能由 DFS/BFS 模板表达。；递归函数要定义返回值。
...
- 先给定义和边界：树是层级结构，二叉树每个节点最多两个子节点。；DFS 深度优先遍历子树。；BFS 按层扩展节点。；二叉树题核心是递归定义和返回值设计，前中后序、层序、深度、高度、路径和子树信息都能由 DFS/BFS 模板表达。；递归函数要定义返回值。
- 再讲机制：树题递归要先定义子问题。；后序能利用左右子树结果。；层序需要队列和层大小。；路径题要处理回溯和叶子判断。；前序适合自顶向下传递状态，后序适合自底向上汇总子树信息。。
- 工程落地要说清楚：Preorder/inorder/postorder。；Level-order BFS。；Postorder aggregation。；Path backtracking。；最大深度返回 1 + max(left,right)。；平衡树后序返回高度并携带是否合法。。
- 最后补指标、失败模式和取舍：recursion_base_case_errors；tree_height；bfs_queue_max_size；path_backtrack_errors；edge_case_pass_rate；递归返回值语义不清；把深度和高度混淆；层序遍历没有固定当前层 size。
- 二叉树题核心是递归定义和返回值设计，前中后序、层序、深度、高度、路径和子树信息都能由 DFS/BFS 模板表达。
- 树是层级结构，二叉树每个节点最多两个子节点。
- DFS 深度优先遍历子树。
- BFS 按层扩展节点。
- 树题递归要先定义子问题。
- 后序能利用左右子树结果。
- 层序需要队列和层大小。
- 路径题要处理回溯和叶子判断。
- 前序适合自顶向下传递状态，后序适合自底向上汇总子树信息。
- 层序遍历用队列按层扩展，可处理最短深度、右视图、锯齿遍历等问题。
...
- Path backtracking。
- 最大深度返回 1 + max(left,right)。
- 平衡树后序返回高度并携带是否合法。
- 路径和题注意到叶子才判断完整路径。
- 写树题先定义函数含义：处理以 root 为根的子树，返回什么。
- 空节点是最常见边界，递归出口必须先写。
...
**重点指标**
- recursion_base_case_errors
- tree_height
- bfs_queue_max_size
- path_backtrack_errors
- edge_case_pass_rate

## 多轮追问模拟

### 追问 1：如果面试官深挖 二叉树遍历、深度、高度与递归返回值 的生产落地和排障，你怎么回答？

**回答要点**：我会先划清边界：树是层级结构，二叉树每个节点最多两个子节点。；DFS 深度优先遍历子树。；BFS 按层扩展节点。；二叉树题核心是递归定义和返回值设计，前中后序、层序、深度、高度、路径和子树信息都能由 DFS/BFS 模板表达。。然后再解释机制、生产约束和指标，避免只背名词。

**考察点**：边界、机制

### 追问 2：如果把这个点落到真实项目，你会怎么设计？

**回答要点**：我会按输入、配置、运行、失败处理和观测展开：最大深度返回 1 + max(left,right)。；平衡树后序返回高度并携带是否合法。；路径和题注意到叶子才判断完整路径。；写树题先定义函数含义：处理以 root 为根的子树，返回什么。；空节点是最常见边界，递归出口必须先写。。项目表达里要说明数据流、配置来源、回滚方式和指标。

**考察点**：项目设计、数据流

### 追问 3：线上出问题时先看什么？

**回答要点**：先确认影响面和最近变更，再看关键指标：recursion_base_case_errors；tree_height；bfs_queue_max_size；path_backtrack_errors；edge_case_pass_rate。排查时按入口、运行态、依赖、配置、资源和发布逐层收敛。

**考察点**：排障、指标

### 延伸追问 1：如果面试官深挖 二叉树遍历、深度、高度与递归返回值 的生产落地和排障，你怎么回答？

回答时继续沿着边界、架构、数据流、指标、失败模式和取舍展开。可以落到这些项目证据：可以关联项目证据：pe-coding-agent；写树题先定义函数含义：处理以 root 为根的子树，返回什么。；空节点是最常见边界，递归出口必须先写。
