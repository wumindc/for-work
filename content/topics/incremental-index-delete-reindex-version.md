# 增量索引、删除传播、Re-index 与版本切换

## 面试定位

增量索引、删除传播、Re-index 与版本切换 属于 生产级 RAG 数据工程 / 增量索引与版本生命周期。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：企业知识库会持续更新，RAG 索引必须支持增量同步、删除传播、别名切换、回滚、失败重试和重建窗口。
...
- incremental sync、delete propagation、alias switch、backfill、retry and rollback 要服务生产问题
- index job record、alias switch plan、reindex report 必须可版本化和可复盘
- 旧文档继续被召回、重建失败和索引/数据库版本不一致 要有门禁和降级
...
图 1：增量索引、删除传播、Re-index 与版本切换 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 增量索引、删除传播、Re-index 与版本切换 的面试级设计题
