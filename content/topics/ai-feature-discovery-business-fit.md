# AI Feature Discovery 与业务适配判断

## 面试定位

AI Feature Discovery 与业务适配判断 属于 AI 求职作品集与项目表达 / AI 功能发现与业务适配。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：不是所有需求都该用 AI，面试里要能说明何时用规则、搜索、工作流、RAG、Agent 或人工流程。
...
- business problem framing、AI suitability、risk、ROI and success criteria 要服务生产问题
- feature discovery one-pager、success metric、risk matrix 必须可版本化和可复盘
- 为了用 AI 而用 AI，最终没有业务价值或风险过高 要有门禁和降级
...
图 1：AI Feature Discovery 与业务适配判断 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### AI Feature Discovery 与业务适配判断 的面试级设计题
