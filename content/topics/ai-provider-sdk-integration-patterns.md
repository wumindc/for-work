# OpenAI、Anthropic、Azure、Vertex SDK 集成模式

## 面试定位

OpenAI、Anthropic、Azure、Vertex SDK 集成模式 属于 Python AI 工程与 API 服务 / AI Provider SDK 集成。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：多 Provider 集成要抽象请求、响应、错误、工具调用、流式输出和成本字段，同时保留 provider-specific 能力边界。
...
- Provider adapter、model gateway、error mapping、tool schema 和 fallback 要服务生产问题
- provider adapter interface、model routing config、cost ledger 必须可版本化和可复盘
- 供应商锁定、错误语义不一致和 fallback 质量退化 要有门禁和降级
...
图 1：OpenAI、Anthropic、Azure、Vertex SDK 集成模式 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### OpenAI、Anthropic、Azure、Vertex SDK 集成模式 的面试级设计题
