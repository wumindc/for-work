...
公开发布时要把“有序”拆成可验证的工程承诺，而不是一句“同一个 key 就有序”。更准确的说法是：顺序消息先定义顺序域，例如 `order_id`、`account_id` 或 `workflow_run_id`；再保证同一顺序域进入同一 partition 或 message group；消费端在同一顺序域内串行处理；业务层用 version、状态机约束和 idempotency 防止重复和旧事件覆盖。少任何一层，都不能说系统真正保证了顺序。

读者还需要理解并发上限。Consumer 数量超过 partition 数不会继续提升同一 topic 的消费并行度；同一 partition 内为了保持 offset 顺序，不能随意并发提交。某些系统可以在消费者内部按 key 分桶并行，但提交和失败处理仍要非常谨慎，否则会出现后续消息先提交、失败消息被跳过、状态机被旧事件覆盖的问题。文章应让候选人知道“顺序语义”和“吞吐扩展”本来就是一组取舍。

生产上最常见的事故不是 MQ 不保证顺序，而是业务改变了路由规则。比如 `order_id` 改成 `user_id`，partition 扩容导致 key 映射变化，或者生产端某个版本漏传 key，都可能让同一业务对象进入不同顺序域。上线检查要包含 key 完整率、partition 分布、hot key 排名、rebalance 次数、ordering violation 采样和状态机版本冲突。能说出这些检查项，才算从文档语义走到了项目治理。

回答热点 key 时不要承诺“拆 key 后仍保持强顺序”。如果单个 key 过热，可以做业务层限流、单独队列、按子状态拆分、异步补偿或把强顺序缩小到更细的 aggregate；但如果两个事件确实必须按先后生效，就不能为了吞吐把它们拆到不同并发路径。这个边界能帮助读者避开“既要严格顺序又要无限并发”的伪方案。

一个可落地的验收办法是建立顺序回放用例。为同一业务 key 构造多条事件，覆盖正常顺序、重复投递、失败后重试、consumer 重启、rebalance 和旧版本事件补发，检查最终状态、版本号、offset 提交和幂等记录是否一致。只有这些场景都通过，才能说明系统不是“理论上分区有序”，而是业务上真的不会乱序生效。
