# Python AI 服务配置、Secret 与部署

## 面试定位

Python AI 服务配置、Secret 与部署 属于 Python AI 工程与 API 服务 / 配置、Secret 与部署。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：AI 服务部署要治理 provider key、model name、prompt version、feature flag、环境变量、health check 和回滚策略。
...
- config layering、secret management、health/readiness、deployment and rollback 要服务生产问题
- config manifest、secret policy、deployment checklist 必须可版本化和可复盘
- 密钥泄漏、配置误发和模型版本不可回滚 要有门禁和降级
...
图 1：Python AI 服务配置、Secret 与部署 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Python AI 服务配置、Secret 与部署 的面试级设计题
