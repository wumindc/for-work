...
质量、成本与延迟三角取舍 属于 LLMOps、Eval 与 AI 质量工程 / 质量、成本与延迟取舍。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：AI 系统不是质量越高越好，而是在业务价值、模型质量、TTFT、总延迟、token 成本和稳定性之间做约束优化。
...
- quality metric、latency budget、cost budget、model routing and fallback 要服务生产问题
- quality-cost-latency dashboard、routing policy 必须可版本化和可复盘
- 为追求质量导致成本不可控，或为降成本牺牲核心体验 要有门禁和降级
...
图 1：质量、成本与延迟三角取舍 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 质量、成本与延迟三角取舍 的面试级设计题
...
AI 系统不是质量越高越好，而是在业务价值、模型质量、TTFT、总延迟、token 成本和稳定性之间做约束优化。 质量、成本与延迟三角取舍 是 AI 工程生产化能力的一部分，关注 quality metric、latency budget、cost budget、model routing and fallback。 quality-cost-latency dashboard、routing policy 是团队复盘、验收和面试表达的核心证据。 为追求质量导致成本不可控，或为降成本牺牲核心体验 是这个主题最容易被追问的生产风险。 生产 AI 系统要先定义可验证边界，再谈模型效果。 所有关键配置、数据、prompt、模型、工具和评测结果都要可追溯。 质量、延迟、成本、安全和用户体验要一起权衡，不能只优化单一指标。 失败样本要进入回归集，避免同类问题重复发生。
