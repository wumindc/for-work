# Safety Eval、Red Teaming 与安全回归

## 面试定位

Safety Eval、Red Teaming 与安全回归 属于 LLMOps、Eval 与 AI 质量工程 / Safety Eval 与 Red Teaming。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Safety eval 要覆盖 prompt injection、PII、越权检索、工具滥用和敏感输出，并把红队样本变成持续回归。
...
- safety scenarios、red team cases、policy verdict、block/allow/confirm and regression 要服务生产问题
- red team dataset、policy verdict report、safety regression suite 必须可版本化和可复盘
- 安全样本只做一次演示，后续发布重新引入漏洞 要有门禁和降级
...
图 1：Safety Eval、Red Teaming 与安全回归 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Safety Eval、Red Teaming 与安全回归 的面试级设计题
