# HashMap、HashSet、频次统计与去重

## 面试定位

HashMap、HashSet、频次统计与去重 属于 算法题与编码面试 / 哈希、集合、栈队列与单调结构。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：哈希题用 Map/Set 在 O(1) 平均时间内做存在性、频次、映射、去重和前缀状态记忆，是两数之和、异位词、子数组和等题型基础。
...
图 1：HashMap、HashSet、频次统计与去重 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### HashMap、HashSet、频次统计与去重 的面试级设计题
...
哈希题用 Map/Set 在 O(1) 平均时间内做存在性、频次、映射、去重和前缀状态记忆，是两数之和、异位词、子数组和等题型基础。 HashMap 存储 key 到 value 的映射。 HashSet 存储不重复 key。 频次统计是把元素映射到出现次数或状态。 key 必须表达题目中的唯一状态。 哈希平均 O(1)，最坏情况和常数成本也要意识到。 计数归零后是否删除取决于窗口合法性判断。 有序需求不能用普通 HashMap 保证。
