...
| 字段 | 所属对象 | 作用 | 排障价值 |
| :--- | :--- | :--- | :--- |
| `route_id` | Gateway 路由 | 标识路径、方法、版本和目标服务 | 定位请求被哪条规则转发 |
| `consumer_id` | API 调用方 | 标识 Web、移动端、BFF、第三方或 Agent 工具 | 判断变更影响哪个客户端 |
| `api_version` | 契约版本 | 管理字段、语义和错误码兼容 | 复盘旧客户端是否仍在使用 |
| `bff_profile` | BFF 适配 | 绑定客户端形态、聚合字段和裁剪策略 | 定位聚合层字段缺失或过度暴露 |
| `deprecation_deadline` | 版本治理 | 标识旧接口下线窗口 | 防止盲目下线破坏存量客户端 |
| `fallback_route` | 兼容路由 | 新版本异常时回退旧实现 | 控制灰度发布风险 |

## 公开阅读校验

Gateway/BFF 文章要说清职责边界：Gateway 做认证、限流、路由、观测、协议适配等横切能力；BFF 面向具体客户端做聚合和裁剪；领域规则仍在后端服务，不应被 BFF 偷偷复制。API 版本治理不是给 URL 加 `/v2` 就结束，而是要有消费者分布、兼容窗口、弃用公告、契约测试和回滚策略。

一个可讲案例是移动端老版本仍在使用旧接口：Gateway 按 `consumer_id`、`api_version` 和客户端版本路由到兼容 BFF；新接口灰度给少量客户端，发现 `error_code_unknown_count` 上升后立即走 `fallback_route`。旧版本下线前要统计调用量、通知客户端、保留迁移文档，并在 `deprecation_deadline` 后分阶段拒绝。

验收指标包括 `route_error_rate`、`gateway_auth_fail_count`、`bff_aggregation_latency`、`api_version_distribution`、`deprecated_api_qps`、`fallback_route_count` 和 consumer 级错误率。反例是 BFF 持有核心业务规则、Gateway 静默改写错误码、旧客户端不可观测、版本下线没有迁移窗口。
