# PDF、Word、HTML、表格与公式解析

## 面试定位

PDF、Word、HTML、表格与公式解析 属于 生产级 RAG 数据工程 / 文档解析与结构化。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：生产 RAG 的质量从文档解析开始，必须保留标题层级、表格、公式、图片说明、页码和来源版本。
...
- document parser、structure extraction、table/formula handling and source mapping 要服务生产问题
- parsed document JSON、section tree、page/table/formula reference 必须可版本化和可复盘
- 解析丢结构导致召回命中但证据不可用 要有门禁和降级
...
图 1：PDF、Word、HTML、表格与公式解析 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### PDF、Word、HTML、表格与公式解析 的面试级设计题
