# Spring MVC、REST Controller 与参数校验

## 面试定位

Spring MVC、REST Controller 与参数校验 属于 Spring Java 后端体系 / Spring MVC、REST 与参数校验。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Spring MVC 将 HTTP 请求映射到 Controller 方法，参数绑定、校验、消息转换和返回值处理共同决定 REST API 的契约质量。
...
图 1：Spring MVC、REST Controller 与参数校验 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Spring MVC、REST Controller 与参数校验 的面试级设计题
...
- Request/response DTO。
- OpenAPI/contract test。
- 严格契约减少线上联调问题但前期设计成本更高。
- 统一 DTO 增加转换代码但保护领域模型。
- 强校验提高安全但可能影响兼容性。
...
- REST 题能和 Web 工程、网关、鉴权、幂等、前后端契约一起讲。
- 以前管理端或开放 API 项目都能拿来做案例。
...
Spring MVC 将 HTTP 请求映射到 Controller 方法，参数绑定、校验、消息转换和返回值处理共同决定 REST API 的契约质量。 Spring MVC 是基于 Servlet 的 Web MVC 框架。 REST Controller 将资源操作表达为 HTTP API。 参数校验是 API 契约的一部分。 HTTP 方法、状态码和错误码要有语义。 输入校验在服务端必须可信执行。 DTO 和领域对象要区分。 API 变更要兼容旧客户端。
