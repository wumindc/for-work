...
数组双指针、前缀和与滑动窗口 属于 算法题与编码面试 / 数组、双指针与滑动窗口。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：数组题高频模式是双指针、前缀和、差分和滑动窗口，用 O(n) 扫描维护区间、计数、和或单调约束。
...
图 1：数组双指针、前缀和与滑动窗口 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 数组双指针、前缀和与滑动窗口 的面试级设计题
...
数组题高频模式是双指针、前缀和、差分和滑动窗口，用 O(n) 扫描维护区间、计数、和或单调约束。 双指针是用两个下标表示扫描位置或区间边界。 滑动窗口是动态维护连续区间的双指针模式。 前缀和是数组前 i 项的累计值。 窗口内状态必须能 O(1) 更新。 有序性或单调性决定能否移动指针不回头。 前缀和要定义 prefix[0]=0。 负数会破坏部分滑动窗口单调性。
