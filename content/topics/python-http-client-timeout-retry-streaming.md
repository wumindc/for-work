# Python HTTP Client、超时重试与 Streaming

## 面试定位

Python HTTP Client、超时重试与 Streaming 属于 Python AI 工程与 API 服务 / HTTP Client 与 Streaming。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：AI Provider 调用必须显式处理连接池、timeout、retry、streaming、取消和部分输出，不能把远程模型当成本地函数。
...
- HTTP client、timeout budget、retry with backoff、SSE/streaming 和 cancellation 要服务生产问题
- provider client wrapper、timeout policy、stream event log 必须可版本化和可复盘
- 重试风暴、连接泄漏、流式输出中断和用户等待无反馈 要有门禁和降级
...
图 1：Python HTTP Client、超时重试与 Streaming 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Python HTTP Client、超时重试与 Streaming 的面试级设计题
