...
| 字段 | 所属对象 | 作用 | 排障价值 |
| :--- | :--- | :--- | :--- |
| `sli_name` | SLO 定义 | 标识可用性、延迟或质量指标 | 避免告警目标含糊 |
| `slo_target` | SLO 定义 | 例如 99.9% 或 p95 < 300ms | 计算错误预算 |
| `error_budget_window` | SLO 策略 | 统计周期，例如 30 天 | 判断消耗是否过快 |
| `burn_rate_short` | 告警规则 | 短窗口预算消耗速度 | 捕捉快速故障 |
| `burn_rate_long` | 告警规则 | 长窗口预算消耗速度 | 过滤短暂抖动 |
| `severity` | 告警事件 | 区分 page、ticket 和 notice | 决定升级路径 |
| `runbook_url` | 告警事件 | 绑定排查与止血步骤 | 降低值班临场猜测 |
...
这篇文章发布给外部读者时，最重要的是把 SLO 告警和普通阈值告警区分开。CPU、内存、单实例错误率可以作为根因线索，但不应替代用户可感知 SLI。更专业的回答是先定义分母、失败条件和统计窗口，例如“过去 30 天有效请求成功率达到 99.9%”，再用 burn rate 判断预算是否被快速消耗。

多窗口告警也要讲出为什么是组合，而不是简单堆两个阈值。短窗口用于捕捉快速大故障，长窗口用于确认问题持续存在；只有两者共同满足时才触发高优先级告警，可以降低偶发抖动带来的夜间误报。低优先级告警可以用更长窗口和更低 burn rate，进入工单或白天处理。

项目里还要把错误预算和发布治理连起来。如果某服务本月预算消耗过快，团队应该暂停高风险发布、优先修复可靠性问题、补充压测和回归，而不是继续按原节奏上线功能。这个口径能说明 SLO 不是监控报表，而是工程决策机制。

面试时可以补一个可验证例子：可用性 SLI 用 `5xx + timeout + 业务失败码` 做失败分子，用所有有效请求做分母；延迟 SLI 用 histogram bucket 计算 p95/p99；RAG 质量 SLI 用固定评测集的 `citation_precision` 或 `eval_pass_rate`。不同 SLI 的采集延迟不同，所以告警窗口、严重级别和 runbook 也不同。
