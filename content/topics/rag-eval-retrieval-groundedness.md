# RAG Eval：Retrieval、Citation 与 Groundedness

## 面试定位

RAG Eval：Retrieval、Citation 与 Groundedness 属于 LLMOps、Eval 与 AI 质量工程 / RAG Eval。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：RAG eval 要拆成召回是否命中、证据是否可回答、引用是否支持结论和无答案是否正确拒答。
...
- retrieval recall、citation precision、groundedness、answerability and no-answer detection 要服务生产问题
- retrieval eval case、expected evidence ids、citation verdict 必须可版本化和可复盘
- 向量召回看似相关但不支持答案，或越权证据进入上下文 要有门禁和降级
...
图 1：RAG Eval：Retrieval、Citation 与 Groundedness 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### RAG Eval：Retrieval、Citation 与 Groundedness 的面试级设计题
