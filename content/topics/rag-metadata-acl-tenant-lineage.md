# RAG Metadata、ACL、Tenant 与 Lineage

## 面试定位

RAG Metadata、ACL、Tenant 与 Lineage 属于 生产级 RAG 数据工程 / Metadata、ACL 与 Lineage。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：企业 RAG 必须把 tenant、部门、权限、版本、时间、来源和 lineage 作为检索前置条件，而不是召回后再过滤。
...
- metadata schema、ACL filter、tenant scope、freshness and lineage 要服务生产问题
- document metadata schema、ACL filter policy、lineage record 必须可版本化和可复盘
- 越权召回、旧版本证据和跨租户污染 要有门禁和降级
...
图 1：RAG Metadata、ACL、Tenant 与 Lineage 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### RAG Metadata、ACL、Tenant 与 Lineage 的面试级设计题
...
企业 RAG 必须把 tenant、部门、权限、版本、时间、来源和 lineage 作为检索前置条件，而不是召回后再过滤。 RAG Metadata、ACL、Tenant 与 Lineage 是 AI 工程生产化能力的一部分，关注 metadata schema、ACL filter、tenant scope、freshness and lineage。 document metadata schema、ACL filter policy、lineage record 是团队复盘、验收和面试表达的核心证据。 越权召回、旧版本证据和跨租户污染 是这个主题最容易被追问的生产风险。 生产 AI 系统要先定义可验证边界，再谈模型效果。 所有关键配置、数据、prompt、模型、工具和评测结果都要可追溯。 质量、延迟、成本、安全和用户体验要一起权衡，不能只优化单一指标。 失败样本要进入回归集，避免同类问题重复发生。
