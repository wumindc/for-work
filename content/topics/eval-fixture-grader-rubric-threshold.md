# Eval Fixture、Grader、Rubric 与 Threshold

## 面试定位

Eval Fixture、Grader、Rubric 与 Threshold 属于 LLMOps、Eval 与 AI 质量工程 / Eval 基本模型。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Eval 的基本单元是固定输入、期望行为、grader、rubric 和阈值，用来把 AI 功能从主观体验变成可回归门禁。
...
- fixture、grader、rubric、threshold、version 和 pass/fail semantics 要服务生产问题
- eval case、rubric、grader result、threshold config 必须可版本化和可复盘
- 评测标准不稳定、误拦发布或漏掉真实退化 要有门禁和降级
...
图 1：Eval Fixture、Grader、Rubric 与 Threshold 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Eval Fixture、Grader、Rubric 与 Threshold 的面试级设计题
...
Eval 的基本单元是固定输入、期望行为、grader、rubric 和阈值，用来把 AI 功能从主观体验变成可回归门禁。 Eval Fixture、Grader、Rubric 与 Threshold 是 AI 工程生产化能力的一部分，关注 fixture、grader、rubric、threshold、version 和 pass/fail semantics。 eval case、rubric、grader result、threshold config 是团队复盘、验收和面试表达的核心证据。 评测标准不稳定、误拦发布或漏掉真实退化 是这个主题最容易被追问的生产风险。 生产 AI 系统要先定义可验证边界，再谈模型效果。 所有关键配置、数据、prompt、模型、工具和评测结果都要可追溯。 质量、延迟、成本、安全和用户体验要一起权衡，不能只优化单一指标。 失败样本要进入回归集，避免同类问题重复发生。
