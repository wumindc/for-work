# Python Typing、Ruff、Mypy 与工程规范

## 面试定位

Python Typing、Ruff、Mypy 与工程规范 属于 Python AI 工程与 API 服务 / Python 工程质量。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：AI Python 代码不能停留在脚本风格，生产服务要有 typing、lint、format、依赖边界、测试和 CI 质量门禁。
...
- typing、lint、format、module boundaries、CI quality gate and maintainability 要服务生产问题
- pyproject、type check report、lint report、CI quality gate 必须可版本化和可复盘
- 脚本化代码难维护、隐式类型错误和团队协作成本高 要有门禁和降级
...
图 1：Python Typing、Ruff、Mypy 与工程规范 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Python Typing、Ruff、Mypy 与工程规范 的面试级设计题
...
AI Python 代码不能停留在脚本风格，生产服务要有 typing、lint、format、依赖边界、测试和 CI 质量门禁。 Python Typing、Ruff、Mypy 与工程规范 是 AI 工程生产化能力的一部分，关注 typing、lint、format、module boundaries、CI quality gate and maintainability。 pyproject、type check report、lint report、CI quality gate 是团队复盘、验收和面试表达的核心证据。 脚本化代码难维护、隐式类型错误和团队协作成本高 是这个主题最容易被追问的生产风险。 生产 AI 系统要先定义可验证边界，再谈模型效果。 所有关键配置、数据、prompt、模型、工具和评测结果都要可追溯。 质量、延迟、成本、安全和用户体验要一起权衡，不能只优化单一指标。 失败样本要进入回归集，避免同类问题重复发生。
