...
容量复盘时可以把线程池指标和下游指标放在一张时间线上：入口 QPS、submit rate、active count、queue size、oldest task age、reject count、downstream p95、timeout count、consumer lag。若 downstream p95 先升高，扩线程通常会放大下游压力；若 submit rate 突刺先出现，应在入口限流或按优先级丢弃；若 queue age 超过 SLA，即使 active count 没满，也说明用户已经感知排队。这个判断比单看 `active == maxPoolSize` 更接近生产现场。

## 公开阅读校验

公开文章里，线程池治理不能停留在参数解释。读者应该能用文章反推出自己的线程池设计：任务类型是什么，是否 CPU 或 IO 密集，下游容量是多少，队列最长允许排多久，拒绝后同步请求如何返回、异步任务如何 retry/DLQ，ThreadLocal 上下文如何捕获和清理。能把 `queue_size` 转换成 `oldest_task_age` 和业务 SLA，是这篇文章区别于普通八股的关键。
...
进一步追问容量规划时，可以给出一句更稳的回答：线程数不是单独算出来的，它受 CPU、任务耗时、目标吞吐、下游连接池、队列等待预算和失败策略共同约束。压测要覆盖正常流量、突刺流量、下游慢、拒绝策略和上下文传播，不能只测“最大 QPS”。

如果面试官继续问“如何防止改配置后变差”，可以回答：线程池配置要版本化，灰度时只放少量实例，比较同类流量下的 queue age、reject、timeout、下游错误和业务成功率；回滚标准也要写清，比如 `oldest_task_age` 连续超过预算、reject 超过阈值或核心链路 p95 恶化。这样线程池治理不是拍脑袋调参，而是一套可验证的变更流程。
