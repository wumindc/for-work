# Spring Cloud 熔断、限流、隔离与韧性设计

## 面试定位

Spring Cloud 熔断、限流、隔离与韧性设计 属于 Spring Java 后端体系 / 熔断、限流与微服务韧性。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：微服务韧性设计要组合超时、重试、熔断、限流、隔离、降级和回退，避免单点故障扩散成系统雪崩。
...
图 1：Spring Cloud 熔断、限流、隔离与韧性设计 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Spring Cloud 熔断、限流、隔离与韧性设计 的面试级设计题
...
- 这类题能直接连接你过去的 Spring Cloud 微服务经验和系统设计高可用题。
- 面试中用一次下游故障扩散复盘非常有说服力。
...
微服务韧性设计要组合超时、重试、熔断、限流、隔离、降级和回退，避免单点故障扩散成系统雪崩。 熔断是在依赖异常时快速失败或降级的保护机制。 限流限制单位时间请求量或并发量。 隔离把不同依赖或任务放入独立资源池。 先有超时预算，再谈重试和熔断。 韧性策略按依赖重要性和业务影响分级。 降级结果必须可识别和可恢复。 故障演练验证策略是否有效。
