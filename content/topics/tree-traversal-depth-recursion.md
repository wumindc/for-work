...
二叉树遍历、深度、高度与递归返回值 属于 算法题与编码面试 / 树、BST 与递归分治。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：二叉树题核心是递归定义和返回值设计，前中后序、层序、深度、高度、路径和子树信息都能由 DFS/BFS 模板表达。
...
图 1：二叉树遍历、深度、高度与递归返回值 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 二叉树遍历、深度、高度与递归返回值 的面试级设计题
...
二叉树题核心是递归定义和返回值设计，前中后序、层序、深度、高度、路径和子树信息都能由 DFS/BFS 模板表达。 树是层级结构，二叉树每个节点最多两个子节点。 DFS 深度优先遍历子树。 BFS 按层扩展节点。 树题递归要先定义子问题。 后序能利用左右子树结果。 层序需要队列和层大小。 路径题要处理回溯和叶子判断。
