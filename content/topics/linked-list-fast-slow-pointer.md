...
链表虚拟头节点、反转与快慢指针 属于 算法题与编码面试 / 链表与指针操作。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：链表题考察指针操作、边界保护和原地修改，常用虚拟头节点、三指针反转、快慢指针和递归分治。
...
图 1：链表虚拟头节点、反转与快慢指针 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 链表虚拟头节点、反转与快慢指针 的面试级设计题
...
链表题考察指针操作、边界保护和原地修改，常用虚拟头节点、三指针反转、快慢指针和递归分治。 链表由节点和 next 指针串联。 虚拟头节点是指向原头节点前的辅助节点。 快慢指针用不同速度遍历链表。 头节点可能变化就用 dummy。 指针重连顺序必须先保存后断开。 快慢指针速度差决定相遇或定位语义。 链表题用 O(1) 空间更常见。
