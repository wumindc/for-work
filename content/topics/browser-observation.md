...
这张图的重点是融合而不是堆料。模型拿到的是 observation，调试系统保留 screenshot、DOM snapshot 和元素候选引用。
...
- [Playwright Locators](https://playwright.dev/docs/api/class-locator)：理解 locator 的自动等待与重试能力。
- [Playwright Best Practices](https://playwright.dev/docs/best-practices)：理解用户可见属性和稳定定位。
- [Playwright Debugging](https://playwright.dev/docs/debug)：理解 actionability logs、截图和定位调试。
