...
图 1：OpenAI Agents SDK 的核心运行单元。

图里 Runner 负责推进 run，并把用户请求、上下文、预算和 trace 关联起来。Agent 定义 instructions、model、tools、handoffs 和输出契约。Tool Runtime 是业务系统边界，负责参数校验、权限、幂等和错误封装。Guardrails 覆盖输入、输出和工具动作。Tracing 记录模型、工具、handoff、guardrail verdict 和成本，Eval Store 则把失败轨迹沉淀为回归样本。
...
图 2：旅行 Agent 中 handoff、工具和 guardrail 的一次协作轨迹。

这个案例里，SDK 负责 Agent 编排，业务安全仍由工具运行时、guardrails 和确认流程共同完成。

图 2 的关键不是“多 Agent 越多越好”，而是职责边界清楚：Triage Agent 只抽取约束和决定是否转交，Flight Agent 只处理航班候选，Tool Runtime 只执行受控查询，Guardrails 判断 proposed itinerary 是否违反风险策略。真正的预订动作仍应由确定性 workflow 和用户确认完成，而不是让 Agent 直接完成高风险写操作。
...
一个典型事故是 handoff 循环：Triage Agent 把复杂订单问题转给 Billing Agent，Billing Agent 又因为缺少订单状态转回 Triage，最终用户只看到反复确认。止血可以限制最大 handoff 次数并返回人工或 workflow fallback；根因通常是 handoff payload 缺少 completed steps、open risks 和 return policy；回归样本应断言同类请求只允许一次明确转交，并要求最终 Agent 给出可执行下一步。

另一个事故是工具权限被误认为 guardrail 能兜底。Guardrail 可以判断输出风险，但不能替代后端 ACL。若 Agent 生成“退款”tool call，Tool Runtime 必须检查订单归属、操作人角色、金额阈值和幂等键。否则 prompt 里写再多安全规则，也挡不住错误工具调用进入业务系统。
...
- [OpenAI Agents SDK](https://platform.openai.com/docs/guides/agents-sdk)：官方文档，用于支持 Agent、Runner、tools、handoff、guardrails 和 tracing 的概念边界。
- [OpenAI Agents SDK Python 文档](https://openai.github.io/openai-agents-python/)：官方文档，用于说明 SDK 的 Agent、Runner、工具、handoff 与 tracing 运行方式。
- [OpenAI A practical guide to building agents](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf)：用于支持生产化设计、工具治理、guardrails 和上线评估思路。
- [Anthropic Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)：用于说明简单优先、多 Agent 复杂度和 workflow/agent 取舍。
