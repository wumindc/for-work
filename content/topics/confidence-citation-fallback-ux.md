# 置信度、引用、解释与 Fallback UX

## 面试定位

置信度、引用、解释与 Fallback UX 属于 AI 求职作品集与项目表达 / 置信度、引用与降级体验。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：AI 产品体验要让用户知道答案依据、置信度、可操作下一步和失败时的降级路径。
...
- answer card design、citation verdict、fallback policy 必须可版本化和可复盘
- 用户把低置信答案当确定事实，或无法从失败中恢复 要有门禁和降级
...
图 1：置信度、引用、解释与 Fallback UX 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 置信度、引用、解释与 Fallback UX 的面试级设计题
