...
Spring IoC、依赖注入与 Bean 生命周期 属于 Spring Java 后端体系 / Spring IoC、Bean 与 AOP。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Spring IoC 通过容器管理对象创建、依赖注入、生命周期回调和扩展点，是理解 Spring Boot、AOP、事务和 MVC 的基础。
...
图 1：Spring IoC、依赖注入与 Bean 生命周期 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Spring IoC、依赖注入与 Bean 生命周期 的面试级设计题
...
- Conditional bean。
- ApplicationContext diagnostics。
- 容器管理降低样板代码但增加启动链路复杂度。
- 构造器注入可测试性强但依赖太多会暴露类职责过大。
- 自动装配便利但可能让依赖来源不透明。
...
- 可以把 IoC 容器类比为后端系统的依赖编排器。
- 以前项目中启动失败、Bean 冲突、配置没生效都能落到这条线讲。
...
Spring IoC 通过容器管理对象创建、依赖注入、生命周期回调和扩展点，是理解 Spring Boot、AOP、事务和 MVC 的基础。 IoC 是把对象创建和依赖关系交给容器管理。 BeanDefinition 描述 Bean 的类、作用域、依赖、初始化方法和条件。 Bean 生命周期是 Spring 扩展能力的主线。 容器先有元数据，再有对象实例。 依赖注入应显式表达业务依赖。 后置处理器可以改 Bean 定义或包装 Bean 实例。 Bean 生命周期异常通常会阻断应用启动。
