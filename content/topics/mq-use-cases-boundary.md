...
这篇文章需要把“适合 MQ”和“不适合 MQ”写得同样清楚。适合 MQ 的场景通常满足三个条件：调用方不需要立即拿到下游最终结果，下游处理可以最终一致，失败后可以通过 retry、DLQ、补偿和人工处理恢复。典型例子是发券、通知、索引同步、审计日志、异步报表、模型任务队列和跨系统事件广播。

不适合 MQ 的场景也要给出边界：同步查询、强事务扣款、库存强校验、用户必须立刻看到最终状态的主流程、低复杂度小系统内部调用，都不应为了“架构高级”强行加 MQ。否则会把原本简单的调用失败变成异步积压、重复消费、死信补偿和跨服务排障。MQ 不是减少复杂度，而是把复杂度换成更可伸缩、更可恢复的异步治理。

选型表达要避免泛化。Kafka 更适合高吞吐事件流、日志型存储、分区顺序和回放；RabbitMQ 更适合任务队列、路由、ack 和传统工作队列；RocketMQ 在事务消息、延迟消息和业务型消息场景常见。真正的选型还要看团队运维经验、已有生态、消息保留策略、监控、权限、多租户和跨机房需求，而不是只比较 QPS。

项目中判断 MQ 是否必要，可以问四个问题：失败后是否能接受最终一致，消息是否有稳定业务 key，消费者是否能幂等，DLQ 是否有人负责处理。如果任何一个问题答不上来，说明 MQ 会带来新的风险。这个判断链路让读者不只会说“异步解耦削峰”，还会说“什么时候不该用”。

还可以加入一个系统设计反例：用户支付接口不能先返回成功、再把扣款和记账都丢到 MQ 里慢慢处理，因为用户看到的结果、资金流水和风控校验需要处在强约束链路里。更合理的是支付主流程先完成必要的本地事务和强校验，再把发票、通知、积分、风控异步分析等后续动作作为事件发布。这个边界能让 MQ 从“万金油”回到正确位置。
