...
这篇文章公开阅读时，要让读者清楚 rerank 的定位：它不是补救一切检索问题的魔法层，而是在“已经召回到候选”的前提下提高证据筛选质量。正确证据完全没进 candidate set，rerank 无能为力；正确证据进了但排不上来，才是 reranker 目标、样本或 metadata 设计的问题。

一个成熟 rerank 方案需要把排序目标写清楚。普通语义相关不等于 answerability，主题相似也不等于能支撑 claim。技术文档问答要关注版本、错误码、步骤完整度；政策问答要关注时效和权限；论文问答要关注实验段落、结论和出处。排序目标不清，reranker 可能把“看起来像”的噪声送进上下文。

公开评审还应该要求两类样本：positive evidence 和 hard negative。Hard negative 是那些关键词很像、语义很近、但不能回答问题或版本不对的片段。没有 hard negative，rerank 指标很容易虚高，线上一遇到相似噪声就把模型带偏。

最后，rerank 的收益要和成本一起看。候选集更大、模型更强、LLM judge 更复杂，通常会提升质量也会推高 p95 latency。上线时可以做两阶段 rerank：先用规则和 metadata 过滤，再用 cross-encoder 或 LLM 处理高价值候选；低风险、高频 query 则用缓存和轻量模型控制成本。

排障时还要保存被拒绝的候选，而不是只保存最终 top_k。很多线上错答来自正确证据排在第 k+1、被 dedup 误删、被 metadata 过滤或被旧版本文档压过。保留 rejected candidates 和 filter_reason，才能在事故后判断问题出在召回池、过滤规则、精排模型还是上下文预算。

给公开读者的最终判断是：rerank 的目标不是让候选“更像问题”，而是让进入上下文的证据“更能支撑答案”。如果评测只看语义相关性，不看 answerability 和 citation support，那么 rerank 很可能提升了检索分数，却没有提升最终回答可信度。
