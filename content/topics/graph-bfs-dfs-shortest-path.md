...
图建模、BFS/DFS 与最短路径 属于 算法题与编码面试 / 图搜索与最短路径。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：图题关键是把问题建成节点和边，再选择 BFS、DFS、Dijkstra 或多源搜索处理连通性、层数、路径和最短距离。
...
图 1：图建模、BFS/DFS 与最短路径 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 图建模、BFS/DFS 与最短路径 的面试级设计题
...
图题关键是把问题建成节点和边，再选择 BFS、DFS、Dijkstra 或多源搜索处理连通性、层数、路径和最短距离。 图由节点和边组成。 BFS 按距离层扩展。 DFS 沿路径深入搜索。 最短路径是在图中寻找总代价最小的路径。 建图方式决定复杂度。 BFS 的第一次到达在无权图中就是最短距离。 DFS 适合连通块、拓扑前置遍历和回溯搜索。
