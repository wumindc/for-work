# Kubernetes Service、Ingress 与网络入口

## 面试定位

Kubernetes Service、Ingress 与网络入口 属于 Docker / Kubernetes / DevOps / Kubernetes Service、Ingress 与网络。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Service 为一组 Pod 提供稳定访问入口和负载均衡，Ingress 管理集群外部 HTTP/HTTPS 入口、路由、TLS 和网关策略。
...
图 1：Kubernetes Service、Ingress 与网络入口 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Kubernetes Service、Ingress 与网络入口 的面试级设计题
...
- TLS termination。
- NetworkPolicy。
- LoadBalancer 简单但成本较高。
- Ingress 统一入口便于治理但成为关键依赖。
- NetworkPolicy 提升安全但增加排障复杂度。
...
- Service/Ingress 可以和网关、BFF、负载均衡知识串起来。
- readiness 摘流和传统服务注册健康检查是同一类问题。
...
Service 为一组 Pod 提供稳定访问入口和负载均衡，Ingress 管理集群外部 HTTP/HTTPS 入口、路由、TLS 和网关策略。 Service 是 Kubernetes 内部访问一组 Pod 的稳定抽象。 Ingress 是外部 HTTP/HTTPS 流量进入集群的规则集合。 Ingress Controller 是真正执行路由和入口策略的组件。 Pod 是短生命周期对象，不能把 Pod IP 当服务地址。 readiness 影响 Service endpoints，探针失败会让 Pod 被摘流。 Ingress 负责七层入口，但不替代服务内部鉴权。 网络排障要从请求路径逐层缩小，而不是只看应用日志。
