# 算法面试模拟、Debug 复盘与刷题节奏

## 面试定位

算法面试模拟、Debug 复盘与刷题节奏 属于 算法题与编码面试 / 并查集、拓扑排序与面试复盘。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：算法面试准备最终要落到限时表达、白板/IDE 编码、主动沟通、debug 策略、错题复盘和间隔重做。
...
图 1：算法面试模拟、Debug 复盘与刷题节奏 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 算法面试模拟、Debug 复盘与刷题节奏 的面试级设计题
...
算法面试准备最终要落到限时表达、白板/IDE 编码、主动沟通、debug 策略、错题复盘和间隔重做。 算法面试模拟是在时间限制和沟通压力下复现真实面试流程。 Debug 复盘是把错误分类并转化为下一次可避免的规则。 间隔重做通过遗忘曲线巩固题型。 沟通优先于盲写。 先正确再优化。 错题必须分类复盘。 题型覆盖比随机刷题更重要。
