...
面试深挖时要把并发正确性写成可以审查的状态协议：共享状态由谁写、由谁读、通过哪条 happens-before 边发布、复合操作是否需要锁或原子类、对象是否安全发布。JMM 不是玄学，真正落地时要能用代码路径、状态转换日志和并发压测证明没有旧读、半初始化对象和竞态更新。

## 关键数据结构与协议

| 字段 | 所属对象 | 作用 | 排障价值 |
| :--- | :--- | :--- | :--- |
| `shared_state` | 并发对象 | 被多个线程访问的状态 | 明确需要保护的对象 |
| `writer_thread` | 写路径 | 修改状态的线程或 executor | 定位发布点 |
| `reader_thread` | 读路径 | 观察状态的线程或 executor | 判断可见性需求 |
| `publication_point` | 同步动作 | volatile 写、锁释放、队列提交等 | 建立 happens-before 证据 |
| `invariant` | 业务不变量 | 多字段关系或状态机约束 | 判断 volatile 是否不够 |
| `atomicity_guard` | 同步方案 | lock、Atomic、CAS 或线程封闭 | 保护复合操作 |
| `stress_case_id` | 回归用例 | 并发复现脚本 | 让偶现问题可验证 |
| `stale_read_count` | 指标/日志 | 旧读或状态倒退次数 | 量化可见性问题 |

## 公开阅读校验

公开读者读这篇时，应该能把 volatile、锁、final 和原子类放到同一张语义地图里：volatile 解决可见性和有序性，不解决复合原子性；锁同时提供互斥和可见性；final 依赖正确构造和安全发布；Atomic 类封装单变量 CAS。文章要避免只背 happens-before 规则，而要让读者知道如何用这些规则审查停止标志、双重检查锁、配置热更新和状态机。
