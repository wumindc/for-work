# Online Eval、Shadow Run 与灰度评测

## 面试定位

Online Eval、Shadow Run 与灰度评测 属于 LLMOps、Eval 与 AI 质量工程 / Online 与 Shadow Eval。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：线上评测要在不影响用户的前提下抽样真实请求、影子运行候选模型或 prompt，并比较质量、延迟和成本。
...
- online sampling、shadow run、canary、A/B、human review and risk isolation 要服务生产问题
- shadow eval report、canary cohort、online verdict 必须可版本化和可复盘
- 候选版本未经真实流量验证就全量发布 要有门禁和降级
...
图 1：Online Eval、Shadow Run 与灰度评测 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Online Eval、Shadow Run 与灰度评测 的面试级设计题
