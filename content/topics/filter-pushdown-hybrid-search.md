# Filter Pushdown、BM25 与 Hybrid Search

## 面试定位

Filter Pushdown、BM25 与 Hybrid Search 属于 生产级 RAG 数据工程 / Filter Pushdown 与混合检索。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：生产检索通常需要权限/元数据过滤前置，再组合 BM25、向量召回和 RRF/加权融合，兼顾精确词和语义泛化。
...
- metadata filter pushdown、BM25、vector search、fusion and permission-first retrieval 要服务生产问题
- hybrid retriever trace、single-path candidates、fusion report 必须可版本化和可复盘
- 权限过滤后置、语义召回错过错误码或旧文档被命中 要有门禁和降级
...
图 1：Filter Pushdown、BM25 与 Hybrid Search 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Filter Pushdown、BM25 与 Hybrid Search 的面试级设计题
...
生产检索通常需要权限/元数据过滤前置，再组合 BM25、向量召回和 RRF/加权融合，兼顾精确词和语义泛化。 Filter Pushdown、BM25 与 Hybrid Search 是 AI 工程生产化能力的一部分，关注 metadata filter pushdown、BM25、vector search、fusion and permission-first retrieval。 hybrid retriever trace、single-path candidates、fusion report 是团队复盘、验收和面试表达的核心证据。 权限过滤后置、语义召回错过错误码或旧文档被命中 是这个主题最容易被追问的生产风险。 生产 AI 系统要先定义可验证边界，再谈模型效果。 所有关键配置、数据、prompt、模型、工具和评测结果都要可追溯。 质量、延迟、成本、安全和用户体验要一起权衡，不能只优化单一指标。 失败样本要进入回归集，避免同类问题重复发生。
