...
Project Storytelling 的公开稿要服务读者拿来复用，所以必须把“讲故事”变成可检查的材料清单。一个可信项目表达至少包含问题背景、用户或业务约束、架构图、端到端数据流、失败案例、指标口径、上线边界和下一步风险。缺少其中任一项，项目就容易滑向宣传文案。

验收时可以把每个项目故事当成一次技术评审：结论是否有证据，指标是否有 baseline 和样本口径，架构图是否标出外部依赖和安全边界，失败案例是否包含 symptoms、root cause、fix、regression，未完成能力是否明确标为 unsupported。尤其是 AI 项目，必须区分模型能力、prompt 技巧、系统工程和人工流程各自贡献了什么。

面试或公开文章还要经得起反事实追问：如果不用 Agent 会怎样，如果不用向量库会怎样，如果用户输入恶意内容会怎样，如果数据过期会怎样，如果模型升级回归会怎样。准备这些 counterfactual，能让项目表达从“我做了一个东西”升级为“我理解为什么这样设计，以及什么时候不该这样设计”。

写作时也要控制证据密度。每个重要判断最好对应一个可检查材料：架构图证明边界，trace 证明运行路径，eval 表证明质量，incident 证明失败处理，roadmap 证明未完成边界。公开读者不一定认识你，但会通过这些材料判断项目是否真实、成熟、可复现。

还可以准备一个“一分钟版本”和“深挖版本”。一分钟版本讲清问题、方案、结果和边界；深挖版本准备模块字段、失败样本、评测方法、成本取舍和安全策略。公开文章可以默认采用深挖版本，因为网站读者不是只听电梯演讲，他们会反复返回文章检查细节。

最后要避免把所有项目都写成同一种 Agent 模板。RAG 项目强调证据和 citation，Browser Agent 强调动作 verifier，Coding Agent 强调 diff、测试和回滚，Data Agent 强调 schema、权限和结果校验。按项目类型调整叙事重点，专业度会明显更高。

一个好项目故事读完以后，读者应该能复述系统为什么存在、如何运行、哪里会失败、如何证明有效，以及当前边界是什么。只要这五个问题有一个答不出来，文章就还需要继续补证据。

这也是公开知识站区别于简历项目清单的地方。
