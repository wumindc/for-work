# Agent Eval：Tool、State 与 Task Success

## 面试定位

Agent Eval：Tool、State 与 Task Success 属于 LLMOps、Eval 与 AI 质量工程 / Agent Eval。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Agent eval 要评估工具选择、参数正确性、状态更新、任务完成、停止条件和副作用安全，而不只是最终文本。
...
- tool selection、argument accuracy、state correctness、trajectory and side-effect safety 要服务生产问题
- trajectory eval trace、tool verdict、state assertion 必须可版本化和可复盘
- 任务成功表象下隐藏错误工具调用或危险副作用 要有门禁和降级
...
图 1：Agent Eval：Tool、State 与 Task Success 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Agent Eval：Tool、State 与 Task Success 的面试级设计题
...
Agent eval 要评估工具选择、参数正确性、状态更新、任务完成、停止条件和副作用安全，而不只是最终文本。 Agent Eval：Tool、State 与 Task Success 是 AI 工程生产化能力的一部分，关注 tool selection、argument accuracy、state correctness、trajectory and side-effect safety。 trajectory eval trace、tool verdict、state assertion 是团队复盘、验收和面试表达的核心证据。 任务成功表象下隐藏错误工具调用或危险副作用 是这个主题最容易被追问的生产风险。 生产 AI 系统要先定义可验证边界，再谈模型效果。 所有关键配置、数据、prompt、模型、工具和评测结果都要可追溯。 质量、延迟、成本、安全和用户体验要一起权衡，不能只优化单一指标。 失败样本要进入回归集，避免同类问题重复发生。
