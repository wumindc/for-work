# MyBatis SqlSession、Mapper 代理与 Spring 集成

## 面试定位

MyBatis SqlSession、Mapper 代理与 Spring 集成 属于 Spring Java 后端体系 / MyBatis Mapper 与动态 SQL。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：MyBatis 通过 SqlSession 执行 mapped statement，Mapper 接口由代理对象转成 SQL 调用，Spring 集成负责事务同步和 Mapper 扫描。
...
图 1：MyBatis SqlSession、Mapper 代理与 Spring 集成 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### MyBatis SqlSession、Mapper 代理与 Spring 集成 的面试级设计题
...
- MappedStatement。
- Executor plugin。
- MyBatis 保留 SQL 控制力但需要手写和维护 SQL。
- XML 清晰但跳转和重构成本较高。
- 注解简洁但复杂 SQL 可读性差。
...
- MyBatis 很适合讲你过去 Java 后端工作的真实 CRUD、复杂查询和性能优化经验。
- 架构师回答要把 Mapper、事务、SQL 和数据库执行计划连起来。
...
MyBatis 通过 SqlSession 执行 mapped statement，Mapper 接口由代理对象转成 SQL 调用，Spring 集成负责事务同步和 Mapper 扫描。 SqlSession 是 MyBatis 执行 SQL、获取 Mapper 和管理事务的核心接口。 Mapper 代理把 Java 方法映射到 SQL statement。 Spring 集成让 MyBatis 参与 Spring 事务和 Bean 管理。 Mapper 接口和 XML statement 要保持一一对应。 事务连接应由 Spring 管理。 SQL 边界要清楚，复杂查询要可 explain。 批量操作要控制参数数量和事务大小。
