...
排序、TopK、堆与选择问题 属于 算法题与编码面试 / 二分、排序与答案空间。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：排序和选择题要判断是否需要全量排序，TopK 可以用堆、快速选择、桶计数或有序结构按复杂度取舍。
...
图 1：排序、TopK、堆与选择问题 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 排序、TopK、堆与选择问题 的面试级设计题
...
排序和选择题要判断是否需要全量排序，TopK 可以用堆、快速选择、桶计数或有序结构按复杂度取舍。 排序将元素按比较规则排列。 堆是支持快速取最小或最大元素的数据结构。 选择问题是寻找第 k 个或前 k 个元素。 只需要局部顺序时不必全排序。 动态数据流 TopK 优先堆。 排序后的相邻关系经常产生新信息。 比较器必须满足传递性和一致性。
