...
公开文章讲 Tool Registry，要让读者看到它不是工具列表，而是工具治理平面。Registry 管的是工具资产的生命周期：定义、owner、版本、权限、风险、健康、废弃、灰度和审计。没有 Registry，小项目可以靠 if-else 运行；工具和 Agent 数量上来后，权限不一致、schema 漂移和重复实现会很快变成系统性风险。

这篇文章应给出两个关键协议。第一是 visibility protocol：Context Builder 为什么向模型暴露这些候选工具，暴露的是哪个版本和哪些精简字段。第二是 execution protocol：Dispatcher 如何把模型的 tool call 解析到 `tool_id@version`，并重新检查权限、健康、schema 和 side effect。可见性和执行授权分开，才能防止 prompt 层误导变成真实越权。

读者还应学会用 Registry 复盘工具事故。误选时看候选召回、description 重叠和 router 评分；越权时看 permission scope、tenant context 和 policy verdict；schema 错误时看 version pinning 和 deprecation policy；下游失败时看 health snapshot 和 executor version。这样的分层排查，比简单说“模型选错工具”更接近生产系统。
