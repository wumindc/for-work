...
Redis 和数据库之间没有一个天然的本地事务。写数据库成功后，删除 Redis 可能失败；删除 Redis 成功后，DB 事务可能回滚；并发读可能在写库前读到旧值，随后把旧值回填到缓存；MQ 失效事件可能延迟、重复或进入死信。只要系统跨了两个存储，就要面对这些窗口。

所以缓存一致性题不能承诺跨存储瞬时一致。更严谨的说法是：按照业务等级定义一致性 SLA。例如价格展示允许 3 秒内收敛，用户权限要求写后读强校验，推荐特征可以分钟级更新。不同 SLA 对应不同策略：短 TTL、绕缓存读 DB、版本校验、延迟双删、CDC 失效、后台刷新或人工补偿。
...
延迟双删的机制是：写 DB 后立即删除缓存，再等待一个短延迟后删除第二次。它能降低并发读回填旧值的概率，但不能证明跨存储瞬时一致。延迟太短覆盖不了慢查询，太长又增加删除流量和维护复杂度。因此延迟双删只能作为降低风险的补丁，不能替代 outbox、MQ 失效、版本校验和补偿巡检。
...
列表页和聚合页是缓存一致性最容易漏掉的地方。详情 key 往往能从实体 id 直接定位，但列表缓存可能和类目、排序、价格区间、租户、活动状态、权限过滤有关。工程上要维护 `changed_fields -> affected_cache_keys` 的映射，或者让列表页用短 TTL 与版本水位控制风险。否则后台改价只删详情页，用户仍可能在活动列表里看到旧价格。
...
- [Redis Cache Aside](https://redis.io/docs/latest/develop/use-cases/cache-aside/)：官方文档，用于确认 cache-aside、缓存失效和事实源边界。
- [Redis EXPIRE command](https://redis.io/docs/latest/commands/expire/)：官方文档，用于说明 TTL、过期行为和缓存窗口控制的命令语义。
- [Apache Kafka Documentation](https://kafka.apache.org/documentation/)：官方文档，用于支持失效事件的 at-least-once、重试和消费积压治理。
- [MySQL InnoDB Transaction Model](https://docs.oracle.com/cd/E17952_01/mysql-8.0-en/innodb-transaction-model.html)：官方文档，用于说明数据库事实源、事务提交和缓存删除事件之间的边界。
