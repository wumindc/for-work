...
图 1：上下文压缩从长 trace 到可恢复状态的投影链路。

图中 State Extractor 从原始 trace 和 artifacts 抽取事实，Compression Policy 决定哪些字段无损、哪些字段可摘要，Verifier 再检查压缩产物是否覆盖目标、约束、证据和下一步。关键边界在于：Compression Artifact 只是工作视图，不是事实源；一旦 verifier 发现缺状态，必须通过 Retrieval Fallback 回原始 trace 拉取，而不是让模型猜。
...
生产里最容易被低估的是“压缩漂移”。第一次摘要可能只丢一个小限制，第二次滚动压缩会把这个缺口固化成新事实，后续 Agent 就会沿着错误状态继续执行。因此压缩产物要保存 `source_trace_range`，并禁止只基于上一轮摘要继续摘要。滚动压缩可以复用上一轮 artifact，但必须能按需回查原始 trace、diff、截图或检索证据。

另一个实践细节是把恢复任务变成 eval。不要只看压缩比例，而要把压缩前的长任务和压缩后的 artifact 分别交给恢复 Agent，比较它是否保留硬约束、是否重复已完成步骤、是否能找到关键证据、是否执行正确下一步。这样的 resume eval 才能证明压缩不是“写得像总结”，而是能继续工作。
...
上下文压缩与保真要写得严谨，必须把三件事分开：事实源、压缩产物和恢复上下文。事实源是 trace、tool output、diff、截图、文档 span；压缩产物是从事实源投影出的状态对象；恢复上下文是本轮模型实际收到的输入。很多事故来自把压缩产物当事实源，导致旧错误被滚动摘要固化。

生产验收可以用 loss budget 表达保真策略。用户硬约束、安全策略、当前状态、证据引用和未完成动作应低损或无损保留；重复讨论、模型草稿和已否定方案可以高损压缩或丢弃。每次压缩后，Verifier 不只检查字段是否存在，还要检查字段是否足以恢复下一步动作。例如 `test_results` 不能只写“构建失败”，还要有命令、退出码、失败摘要和日志引用。

最后要补上降级策略。压缩产物验证失败时，系统不应继续 resume，而应触发 retrieval fallback：回原始 trace、artifact store 或仓库当前状态重新取证。若关键证据已过期，应明确标记 state stale，并要求重新验证。这样读者能看到压缩不是 token 技巧，而是一套长任务状态治理机制。

## 来源与延伸阅读

- [Anthropic: Effective context engineering for AI agents](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents)：用于支持“上下文是需要工程化管理的状态空间”，尤其是选择、压缩、隔离和工具反馈的取舍。
- [Anthropic: Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)：用于说明 Agent 工作流应先设计清晰控制流和边界，再决定模型如何参与。
- [OpenAI Agents SDK Tracing](https://openai.github.io/openai-agents-python/tracing/)：官方文档用于支持 trace、span 和工具调用记录是恢复与复盘的事实来源。
- [Model Context Protocol 文档](https://modelcontextprotocol.io/docs/learn/architecture)：用于解释上下文和工具能力可以通过协议化资源暴露，支撑 artifact refs 与 retrieval fallback 的设计。
