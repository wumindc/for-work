# Spring 事务传播、隔离级别与一致性边界

## 面试定位

Spring 事务传播、隔离级别与一致性边界 属于 Spring Java 后端体系 / 数据访问、事务与一致性。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Spring 声明式事务要同时理解代理生效条件、传播行为、隔离级别、回滚规则、连接绑定和数据库锁/MVCC。
...
图 1：Spring 事务传播、隔离级别与一致性边界 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Spring 事务传播、隔离级别与一致性边界 的面试级设计题
...
- 可以把 Spring 事务和数据库 MVCC、MQ Outbox、Redis 缓存一致性串起来。
- 架构师面试常会从 @Transactional 一路追到分布式事务。
...
Spring 声明式事务要同时理解代理生效条件、传播行为、隔离级别、回滚规则、连接绑定和数据库锁/MVCC。 事务传播定义当前方法如何参与已有事务。 隔离级别定义并发事务之间的可见性。 一致性边界应优先落在本地数据库事务内。 事务越短越稳定。 远程调用和本地事务不能假装一个原子动作。 回滚规则要和业务异常体系一致。 幂等和补偿是分布式一致性的基础。
