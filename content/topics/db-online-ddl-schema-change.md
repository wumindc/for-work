...
| 字段 | 所属对象 | 作用 | 排障价值 |
| :--- | :--- | :--- | :--- |
| `migration_id` | Schema 迁移 | 标识一次 expand/backfill/switch/contract 流程 | 串联 DDL、代码发布和校验 |
| `compatibility_phase` | 发布阶段 | 区分 expand、dual-write、switch、contract | 判断新旧代码是否兼容 |
| `backfill_cursor` | 回填任务 | 记录 last_id、batch_size、错误样本和限速 | 支持暂停、重跑和排障 |
| `metadata_lock_wait` | DDL 风险 | 记录元数据锁等待和阻塞 SQL | 定位 Online DDL 是否影响线上 |
| `schema_checksum` | 校验结果 | 对比新旧字段、影子表或新旧索引一致性 | 防止切流后数据错乱 |
| `rollback_flag` / `cutover_gate` | 切流控制 | 保存灰度开关、回滚条件和审批人 | 防止不可逆发布 |
...
公开读者读这一篇，应该明确 Online DDL 不是“无锁变更”的同义词。不同数据库、不同版本、不同变更类型的锁行为和资源消耗都不同；即使 DDL 本身在线，也可能带来复制延迟、磁盘水位上涨、写入 p95 上升和连接池抖动。专业文章必须把数据库变更和应用发布顺序放在一起讲。

Schema 迁移的核心方法是 expand/contract：先加兼容字段或新表，再让代码双写或双读，回填和校验通过后切读，最后等旧代码和旧数据退出再删除旧结构。删除字段、改枚举语义、收紧非空约束这类 contract 操作风险最高，必须证明没有旧代码仍在读写。

项目表达可以讲“订单状态字段重构”：先新增 status_v2 和转换逻辑，发布兼容代码双写新旧字段，按主键范围限速回填，用 checksum 对比新旧状态，再灰度切读。若发现差异，可以关闭切读开关、继续读旧字段并修复回填任务。这个例子能说明 schema 变更是多阶段工程，而不是一条 DDL。
