# AI 线上事故复盘与 Regression Case

## 面试定位

AI 线上事故复盘与 Regression Case 属于 LLMOps、Eval 与 AI 质量工程 / AI 事故与回归样本。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：AI 事故复盘要把 bad answer、错误引用、工具误操作和权限问题转成可重复 eval case，而不是只改一次 prompt。
...
- incident timeline、impact、root cause、fix、regression case and runbook 要服务生产问题
- incident report、regression case、runbook update 必须可版本化和可复盘
- 同类 AI 事故反复出现但无法证明修复有效 要有门禁和降级
...
图 1：AI 线上事故复盘与 Regression Case 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### AI 线上事故复盘与 Regression Case 的面试级设计题
