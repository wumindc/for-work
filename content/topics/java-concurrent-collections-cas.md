...
面试深挖时要把并发集合的保证边界讲清楚。ConcurrentHashMap 保护的是集合内部结构和单次操作的线程安全，不负责跨 key 事务、不负责多实例一致性，也不负责值对象内部的线程安全。CAS 也只是单变量条件更新，高竞争下会自旋浪费 CPU，ABA、活锁和热点 key 仍然要靠设计规避。

## 关键数据结构与协议

| 字段 | 所属对象 | 作用 | 排障价值 |
| :--- | :--- | :--- | :--- |
| `map_name` | 本地状态 | 标识并发集合用途 | 区分缓存、注册表、去重表 |
| `key_hash` | key | 定位热点 key 或 hash 分布 | 判断是否单点竞争 |
| `operation_type` | 操作 | get、put、compute、remove | 还原原子边界 |
| `compute_latency_p95` | 指标 | compute 回调耗时 | 发现慢初始化或锁内阻塞 |
| `cas_retry_count` | CAS 指标 | 自旋重试次数 | 判断高竞争退化 |
| `value_size_bytes` | value | 单值大小 | 发现本地内存膨胀 |
| `eviction_policy` | 容量策略 | 过期、上限或清理规则 | 防止无限增长 |
| `consistency_boundary` | 设计说明 | 本地状态还是事实源 | 避免把进程内缓存当分布式状态 |

## 公开阅读校验

公开读者最需要的是边界感：并发集合能让多线程安全访问同一个 Java 对象，但不能自动解决业务事务、跨进程一致性和容量治理。文章应该让读者看懂为什么 `computeIfAbsent` 的回调不能做慢 IO，为什么 `LongAdder` 适合统计但不适合余额，为什么本地缓存一定要有上限和清理策略。讲清这些，才不会把“线程安全容器”误用成“业务一致性方案”。
