# Spring Boot 自动配置、Starter 与条件装配

## 面试定位

Spring Boot 自动配置、Starter 与条件装配 属于 Spring Java 后端体系 / Spring Boot 自动配置与启动。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Spring Boot 通过 starter 聚合依赖，通过自动配置和条件装配按 classpath、Bean、属性和环境生成默认 Bean。
...
图 1：Spring Boot 自动配置、Starter 与条件装配 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Spring Boot 自动配置、Starter 与条件装配 的面试级设计题
...
- Actuator conditions endpoint。
- 自动配置提升开发效率但隐藏装配来源。
- starter 统一依赖版本但可能引入不必要依赖。
- 约定优于配置适合通用场景但复杂业务要显式治理。
...
- 可以讲公司内部 starter 如何统一日志、鉴权、trace、错误码和数据源。
- 自动配置题能自然连接架构师如何做基础框架治理。
...
Spring Boot 通过 starter 聚合依赖，通过自动配置和条件装配按 classpath、Bean、属性和环境生成默认 Bean。 Starter 是依赖和约定的组合包。 自动配置是在满足条件时提供默认 Bean 的机制。 条件装配让 Boot 能在不同 classpath 和配置下自适应。 默认配置应可覆盖。 自动配置应尽量无副作用。 条件判断要清晰可诊断。 业务配置和基础设施配置要分层。
