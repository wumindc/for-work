...
图 1：Component Eval 的最小闭环。

图中 Eval Case Library 是稳定样本来源，fixture data 用来冻结索引切片、工具 mock、网页 snapshot 或测试仓库状态。Component 代表被测单元，Observed Output 进入 Judge 后生成结构化 verdict。Gate 不只看平均分，而是看关键 case、关键断言和 failure taxonomy 是否触发发布阻断。Judge 不一定是 LLM。很多核心约束应该用确定性规则，例如工具参数是否符合 schema、返回是否有 error_code、检索是否命中 expected evidence id。
...
生产里更容易出问题的是“组件边界没切干净”。例如 RAG 答案错了，如果 eval case 同时跑检索、重排、生成和 citation，就很难判断失败来自哪一层。更好的做法是把 retriever case 只断言 evidence id，把 reranker case 只断言排序，把 citation verifier case 只断言 claim 是否被 evidence 支持。组件 eval 的粒度越清楚，事故复盘时越能把修复动作落到 owner。

CI Gate 也要分层。高风险组件如权限过滤、工具参数校验、PII 拦截，应使用硬阈值，失败直接阻断。开放文本质量可以使用趋势阈值或人工抽检，不宜因为一次 judge 分数波动就阻断所有发布。面试里可以强调：Component Eval 不是为了追求一个漂亮总分，而是为了把“不能坏的能力”变成可重复检查的工程门禁。
...
一个常见事故是工具 schema 改名后，模型仍按旧字段调用。影响面表现为 tool call 成功率下降、invalid_args 上升、用户看到“我无法完成操作”。止血可以临时兼容旧字段或回滚 schema；根因通常是 tool eval 没覆盖字段重命名和 optional/required 变化；回归样本应同时包含旧 prompt、旧参数、新 schema 和预期 structured error，防止未来 schema 演进再次破坏工具调用。
...
- [Promptfoo](https://github.com/promptfoo/promptfoo)：用于说明 prompt、模型输出和断言式 case 管理如何进入自动化评测。
- [DeepEval](https://github.com/confident-ai/deepeval)：用于支持 RAG、LLM 输出和指标化评测的组件化拆分思路。
- [Inspect](https://inspect.aisi.org.uk/)：官方文档，用于解释可复现 eval、样本、评分器和任务组织方式。
- [OpenAI Evals](https://github.com/openai/evals)：用于支持把样本、评测逻辑和回归任务沉淀成可运行资产。
