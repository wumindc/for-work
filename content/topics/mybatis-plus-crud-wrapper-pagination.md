# MyBatis Plus CRUD、Wrapper、分页与插件机制

## 面试定位

MyBatis Plus CRUD、Wrapper、分页与插件机制 属于 Spring Java 后端体系 / MyBatis Plus 工程化。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：MyBatis Plus 在 MyBatis 之上提供 BaseMapper、Service、Wrapper、分页、逻辑删除和插件能力，提升效率但也需要治理 SQL 可见性和插件副作用。
...
图 1：MyBatis Plus CRUD、Wrapper、分页与插件机制 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### MyBatis Plus CRUD、Wrapper、分页与插件机制 的面试级设计题
...
- TenantLineInnerInterceptor。
- OptimisticLockerInnerInterceptor。
- 通用 CRUD 降低开发成本但可能弱化 SQL 设计意识。
- 插件统一治理但增加排查 SQL 的难度。
- 逻辑删除方便恢复但会影响索引和唯一约束设计。
...
- 可以讲 MyBatis Plus 如何提高后台管理系统开发效率。
- 也要主动说清它的边界，体现架构师判断。
...
MyBatis Plus 在 MyBatis 之上提供 BaseMapper、Service、Wrapper、分页、逻辑删除和插件能力，提升效率但也需要治理 SQL 可见性和插件副作用。 MyBatis Plus 是 MyBatis 的增强工具，提供通用 CRUD 和插件能力。 Wrapper 是类型化构造 SQL 条件的 DSL。 插件通过拦截执行链扩展分页、租户、乐观锁等能力。 通用 CRUD 提效但复杂业务仍要显式 SQL 设计。 插件改写 SQL 必须可观测和可解释。 逻辑删除、租户和权限过滤属于数据安全边界。 分页要关注 count、排序和深页性能。
