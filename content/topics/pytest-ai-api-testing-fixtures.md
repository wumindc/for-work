# pytest、Fixtures 与 AI API 测试

## 面试定位

pytest、Fixtures 与 AI API 测试 属于 Python AI 工程与 API 服务 / pytest 与 AI API 测试。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Python AI 服务测试要冻结输入、Provider 响应、错误场景和 golden cases，避免每次依赖实时模型结果。
...
- pytest fixtures、mock provider、contract tests、golden responses 和 regression samples 要服务生产问题
- test fixture、golden file、provider mock、contract test report 必须可版本化和可复盘
- 测试依赖实时模型导致 flakiness 和回归不可复现 要有门禁和降级
...
图 1：pytest、Fixtures 与 AI API 测试 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### pytest、Fixtures 与 AI API 测试 的面试级设计题
...
Python AI 服务测试要冻结输入、Provider 响应、错误场景和 golden cases，避免每次依赖实时模型结果。 pytest、Fixtures 与 AI API 测试 是 AI 工程生产化能力的一部分，关注 pytest fixtures、mock provider、contract tests、golden responses 和 regression samples。 test fixture、golden file、provider mock、contract test report 是团队复盘、验收和面试表达的核心证据。 测试依赖实时模型导致 flakiness 和回归不可复现 是这个主题最容易被追问的生产风险。 生产 AI 系统要先定义可验证边界，再谈模型效果。 所有关键配置、数据、prompt、模型、工具和评测结果都要可追溯。 质量、延迟、成本、安全和用户体验要一起权衡，不能只优化单一指标。 失败样本要进入回归集，避免同类问题重复发生。
