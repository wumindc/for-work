...
LangGraph 文章要让读者看懂“为什么图能解决问题”，而不是只列出 node 和 edge。公开稿建议围绕三条线校验：状态是否有 schema、边是否由结构化 verdict 驱动、恢复是否依赖 checkpoint 而不是聊天历史。只要 state、edge、checkpoint 三者有一个是自然语言黑盒，图就很难测试和回放。

生产验收可以构造一个长任务 graph：planner 生成计划，executor 调工具，verifier 判断是否通过，高风险动作触发 interrupt，人工确认后 resume。验收时强制注入工具失败、人工超时、并发 update、checkpoint 丢失和 reducer 冲突，观察 graph 是否能进入预期边、保存必要 state diff、拒绝错误恢复。大对象要通过 artifact ref 引用，不能把 PDF、截图和长日志直接塞进 state。

可观测性指标要按 graph 层拆开：`node_success_rate`、`edge_transition_error_rate`、`checkpoint_write_latency_p95`、`checkpoint_resume_rate`、`state_merge_conflict_count`、`interrupt_resume_abandon_rate` 和 `artifact_ref_missing_count`。如果 resume 失败率高，优先查 checkpoint 与 state schema；如果边跳错，优先查 verifier verdict 和 edge condition，而不是笼统怪模型。

还要把版本迁移纳入验收。Graph 一旦上线，state schema、reducer 和 edge condition 都会成为持久化协议的一部分。升级前要准备旧 checkpoint 的 replay set，验证旧状态能否被迁移到新 schema，不能只跑新请求 happy path。
