...
图里的重点是 state 是共享事实源，node 只产出 state update，edge 负责路由。
...
- [LangGraph Graph API](https://docs.langchain.com/oss/python/langgraph/graph-api)：理解 State、Node、Edge。
- [LangGraph Persistence](https://docs.langchain.com/oss/python/langgraph/persistence)：理解 checkpoint 和 thread-scoped state。
- [LangGraph Interrupts](https://docs.langchain.com/oss/python/langgraph/interrupts)：理解暂停、人工输入和 resume。
