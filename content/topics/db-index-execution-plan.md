...
联合索引的字段顺序要服务查询谓词。等值条件通常可以放在前面，范围条件会影响后续字段继续利用索引有序性，排序字段是否能利用索引要看前面的条件是否固定。低选择性字段不是绝对不能建索引，但如果它不能显著减少扫描行数，就要结合组合字段、覆盖索引、分区或读模型重新设计。
...
- MySQL InnoDB Index Types 官方文档：用于确认聚簇索引、二级索引和 InnoDB 索引语义边界。
- PostgreSQL MVCC 官方文档：用于理解执行计划和事务并发之间的关系。
- Redis 官方文档：用于说明缓存作为读模型时不能替代数据库事实源。
