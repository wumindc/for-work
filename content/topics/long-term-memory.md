...
图中 Candidate Extractor 只负责从对话或任务事件里提出“可能值得记住”的候选，Write Policy 才决定是否写入。Memory Store 同时包含 profile、episodic、semantic 三类存储，但新任务读取时必须先经过 Memory Retriever 和 Ranker，再由 Context Builder 贴上来源与置信度标签。Trace and correction feedback 会把使用结果、用户纠错和冲突证据反馈给写入策略，避免旧记忆长期污染后续任务。
...
一个典型事故是 workspace memory 串租户：某次任务把客户 A 的项目约束写成了全局 profile memory，客户 B 后续任务被错误约束影响。影响面包括回答偏差、越权提示和用户信任下降。止血要先禁用相关 scope 的读取，隔离可疑记录并回放最近使用 trace；根因往往是 Write Policy 没有强制 `tenant_id`、`workspace_id` 和 `sourceEventId`；回归检查要增加跨租户 retrieval fixture，断言不同 scope 下无法召回对方记忆。

长期记忆还需要“反写入”机制。用户说“这不是我的偏好”时，系统不应只在当前回答道歉，而要把对应 MemoryRecord 标为 deprecated、降低 confidence、写入纠错事件，并触发向量索引删除或重建。否则 embedding 检索可能继续召回旧内容，造成用户以为删了但系统仍在使用的体验问题。
...
- [OpenAI Agents SDK 文档](https://openai.github.io/openai-agents-python/)：官方文档，用于支持 Agent 运行、工具调用、handoff 与 trace 进入工程闭环。
- [Anthropic: Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)：用于说明 Agent 复杂度边界，以及何时需要更明确的状态和控制流。
- [Anthropic: Writing effective tools for agents](https://www.anthropic.com/engineering/writing-tools-for-agents)：用于支持工具和上下文设计需要面向模型可用性，而不是只面向人类 API 文档。
- [Model Context Protocol 文档](https://modelcontextprotocol.io/docs)：官方文档，用于解释资源、工具和上下文边界如何被协议化暴露给模型应用。
