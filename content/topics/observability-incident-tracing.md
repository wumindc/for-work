...
这篇文章对外发布时要强调一个顺序：指标先确认影响面，Trace 再定位异常路径，日志最后解释局部事实。很多候选人一上来就“查日志”，但在真实事故里，日志量巨大且噪声很高；没有指标确定范围、没有 Trace 限定路径，很容易被单个错误样本带偏。

Trace 设计的专业点在异步边界。HTTP 同步调用只是一部分，MQ publish/consume、线程池、定时任务、工作流节点、Agent tool call、模型调用和 RAG 检索都要恢复上下文。每个边界都应记录 span 名称、稳定属性、错误状态、耗时和父子关系。只在网关生成 `trace_id`，但后台任务完全断链，不能算完整 Trace。

采样策略也要和事故证据绑定。头部采样适合低成本常规流量，尾部采样适合保留错误、慢请求和高价值链路；安全、支付、权限、写操作、Agent 工具调用可以设置更高保留概率。采样不是简单省钱开关，而是“哪些失败证据必须留下”的工程策略。

如果用于项目表达，可以补一句：“我们把失败 run 保存为 replay case，包含 trace、结构化日志、模型版本、tool schema、policy verdict、observation 摘要和修复后的 verifier 结果。”这能把可观测性从排障工具延伸到回归测试和质量治理。
