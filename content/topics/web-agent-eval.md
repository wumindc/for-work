...
图 1：Web Agent Eval 以固定 fixture 驱动浏览器任务，收集 step trace，再由任务验证器和安全规则共同产出指标、失败分类和可回放样本。

这张图的核心边界是 Fixture、Trace 和 Verifier。Fixture 冻结起始页面、登录态、网络返回和策略版本，保证不同模型或策略在同一环境下比较；Trace 保存每一步 observation、action、after snapshot 和 verifier verdict，用来定位失败步骤；Verifier 不只看“有没有点击”，而是检查最终业务状态和禁止副作用。Trace Replay 则把线上或灰度失败沉淀成回归样本，避免同类问题反复出现。
...
发布门禁要分核心任务和探索任务。核心任务如登录、资料修改、取消订阅、表单提交，必须有稳定 fixture、明确断言和安全规则；探索任务如网页信息查找，可以允许部分非确定性，但要记录证据来源和输出一致性。不要用一个总体成功率掩盖高风险 case，因为 90% 的普通点击成功不能抵消 1 次错误付款或错误删除。
...
Step Eval 的价值在于归因。最终任务失败可能来自观察缺失、locator 错误、动作等待不足、弹窗遮挡、权限拒绝、verifier 太弱或安全策略误拦。每个失败都要落到 taxonomy，否则团队只会看到 task_success_rate 下降，却不知道应该改 observation builder、selector ranking、wait strategy、policy gate 还是 verifier。

Trace Replay 需要存 artifact，而不是只存文字摘要。至少保存 DOM snapshot、accessibility tree 摘要、截图引用、storage、network mock、action args、tool result、policy version 和 verifier output。这样网页变化后仍能复现旧失败；如果只保存视频或日志片段，回归测试就会退化成人工观看。
...
- [Playwright Timeouts](https://playwright.dev/docs/test-timeouts)：用于支持等待、超时和 fixture 稳定性会直接影响浏览器任务评测结论。
- [BrowserGym](https://github.com/ServiceNow/BrowserGym)：用于支持浏览器任务可以抽象成环境、动作、观察和评测轨迹，适合沉淀为 benchmark 或回归样本。
