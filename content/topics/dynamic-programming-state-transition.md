...
动态规划状态定义、转移方程与空间优化 属于 算法题与编码面试 / 动态规划。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：DP 用状态表示子问题，用转移方程复用重叠子问题结果，关键是状态定义、初始化、遍历顺序和空间优化。
...
图 1：动态规划状态定义、转移方程与空间优化 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 动态规划状态定义、转移方程与空间优化 的面试级设计题
...
DP 用状态表示子问题，用转移方程复用重叠子问题结果，关键是状态定义、初始化、遍历顺序和空间优化。 动态规划是利用重叠子问题和最优子结构求解问题的方法。 状态是对子问题结果的抽象。 转移方程描述状态之间如何递推。 先判断是否有重叠子问题。 状态要覆盖必要信息但不过度膨胀。 遍历顺序必须保证依赖已计算。 空间优化不能破坏依赖。
