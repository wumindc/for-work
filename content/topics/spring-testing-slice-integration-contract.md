# Spring 测试切片、集成测试与契约回归

## 面试定位

Spring 测试切片、集成测试与契约回归 属于 Spring Java 后端体系 / 测试、质量与发布门禁。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Spring 测试体系要区分单元测试、切片测试、集成测试和契约测试，覆盖 Controller、Service、Mapper、事务和外部依赖边界。
...
图 1：Spring 测试切片、集成测试与契约回归 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Spring 测试切片、集成测试与契约回归 的面试级设计题
...
- 测试题可以和质量门禁、CI/CD、发布回滚和 Coding Agent harness 连接。
- 架构师回答要强调测试金字塔和事故回归。
...
Spring 测试体系要区分单元测试、切片测试、集成测试和契约测试，覆盖 Controller、Service、Mapper、事务和外部依赖边界。 测试切片是只加载某一层 Spring 组件的测试方式。 集成测试验证多个真实组件之间的协作。 契约测试保证服务提供方和消费方对 API 的理解一致。 测试越靠近业务核心越要快。 边界测试要覆盖失败和异常语义。 集成测试验证配置和真实协议。 回归测试要从线上问题反哺。
