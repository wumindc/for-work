...
面试深挖时要把锁当成运行时资源协议：谁持有、谁等待、等了多久、保护什么状态、获取顺序是否固定、失败时是否能超时退出。AQS 源码细节可以讲，但更关键的是能从 thread dump、JFR lock event、业务 trace 和临界区代码还原一次阻塞或死锁事故。

## 关键数据结构与协议

| 字段 | 所属对象 | 作用 | 排障价值 |
| :--- | :--- | :--- | :--- |
| `lock_name` | 锁对象 | 标识临界区 | 定位热点锁 |
| `lock_object_id` | JVM 对象 | thread dump 中的锁对象 | 关联 BLOCKED/WAITING 线程 |
| `owner_thread` | 线程 | 当前持有者 | 判断谁阻塞了系统 |
| `waiter_threads` | 线程集合 | 等待队列 | 评估影响面 |
| `hold_time_ms` | 指标 | 持锁时间 | 发现锁内慢 IO 或慢 SQL |
| `wait_time_ms` | 指标 | 等锁时间 | 量化尾延迟来源 |
| `acquire_order` | 设计约束 | 多锁获取顺序 | 排查循环等待 |
| `try_lock_timeout_ms` | 失败路径 | 超时退出策略 | 避免永久等待 |

## 公开阅读校验

公开文章不应只比较 synchronized 和 ReentrantLock 谁快，而要让读者知道什么时候需要锁、锁保护的状态是什么、锁内不能做什么，以及线上如何证明锁是瓶颈。读者应该能带走一套排障动作：看 thread dump 的 BLOCKED/WAITING，看 JFR 的 lock event，看业务 trace 的临界区耗时，看是否存在多锁顺序反转。这样 AQS 才不是背源码，而是能服务生产事故分析。
