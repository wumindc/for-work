...
并查集、连通性、环检测与拓扑排序 属于 算法题与编码面试 / 并查集、拓扑排序与面试复盘。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：并查集适合动态连通性和集合合并，拓扑排序适合有向无环依赖顺序，二者是图题高频补充模式。
...
图 1：并查集、连通性、环检测与拓扑排序 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 并查集、连通性、环检测与拓扑排序 的面试级设计题
...
- 拓扑排序和构建系统、任务依赖、微服务启动顺序很像。
- 并查集和账号合并、连通区域、网络连通性相关。
...
并查集适合动态连通性和集合合并，拓扑排序适合有向无环依赖顺序，二者是图题高频补充模式。 并查集维护若干不相交集合，支持查找和合并。 拓扑排序给有向无环图的节点排依赖顺序。 入度表示有多少前置依赖指向该节点。 并查集适合无向连通性。 路径压缩和按秩合并接近 O(1)。 拓扑排序依赖入度为 0 的起点。 处理不完所有节点说明存在环。
