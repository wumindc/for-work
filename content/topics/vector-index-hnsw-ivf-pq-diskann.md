# 向量索引：HNSW、IVF、PQ 与 DiskANN

## 面试定位

向量索引：HNSW、IVF、PQ 与 DiskANN 属于 生产级 RAG 数据工程 / 向量索引底层。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：向量索引不是黑盒，HNSW、IVF、PQ、DiskANN 等结构会影响 recall、latency、内存、构建时间和过滤能力。
...
- ANN index type、recall/latency tradeoff、build、compaction and memory 要服务生产问题
- index config、benchmark report、capacity plan 必须可版本化和可复盘
- 索引参数错误导致召回退化或资源打爆 要有门禁和降级
...
图 1：向量索引：HNSW、IVF、PQ 与 DiskANN 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 向量索引：HNSW、IVF、PQ 与 DiskANN 的面试级设计题
...
向量索引不是黑盒，HNSW、IVF、PQ、DiskANN 等结构会影响 recall、latency、内存、构建时间和过滤能力。 向量索引：HNSW、IVF、PQ 与 DiskANN 是 AI 工程生产化能力的一部分，关注 ANN index type、recall/latency tradeoff、build、compaction and memory。 index config、benchmark report、capacity plan 是团队复盘、验收和面试表达的核心证据。 索引参数错误导致召回退化或资源打爆 是这个主题最容易被追问的生产风险。 生产 AI 系统要先定义可验证边界，再谈模型效果。 所有关键配置、数据、prompt、模型、工具和评测结果都要可追溯。 质量、延迟、成本、安全和用户体验要一起权衡，不能只优化单一指标。 失败样本要进入回归集，避免同类问题重复发生。
