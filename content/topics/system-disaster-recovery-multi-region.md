...
| 字段 | 所属对象 | 作用 | 排障价值 |
| :--- | :--- | :--- | :--- |
| `failure_domain` | 容灾建模 | 描述实例、可用区、区域或外部依赖的故障边界 | 判断事故是否跨越了原设计边界 |
| `rpo_target` | 业务目标 | 最多可接受的数据丢失窗口 | 对比复制延迟和备份恢复点 |
| `rto_target` | 业务目标 | 最多可接受的恢复时间 | 判断切流、恢复和人工确认是否超时 |
| `replication_lag` | 数据复制 | 主备或多区域之间的数据延迟 | 评估切流后的数据缺口 |
| `failover_runbook_id` | 演练流程 | 绑定切流、回滚、依赖检查和责任人 | 复盘是否按可执行流程恢复 |

## 公开阅读校验

容灾文章要避免一句“部署到多地域就高可用”。公开阅读时，读者应该能看到先定义故障域，再把业务拆成核心写、核心读、异步任务、报表和后台运营等不同等级，然后给每一类链路设定 RPO/RTO。支付确认和订单提交通常不能和日志分析、推荐刷新使用同一套恢复目标。

一个可讲的案例是订单系统主区域故障：入口 DNS 或全局负载均衡切到备区域，备区域先开放只读和查询，再按复制位点确认可写窗口；MQ 消费端使用幂等键处理重复消息，缓存按热点清单预热，外部支付回调进入待核对状态。切流不是单动作，而是依赖、配置、密钥、队列、缓存和观测一起切。

验收证据要来自演练而不是文档。至少记录 `rpo_gap`、`rto_actual`、`cross_region_lag`、`failover_time`、`rollback_time`、`drill_success_rate` 和切流后错误率。多活方案还要解释冲突解决和租户归属，否则只是把单区域故障换成跨区域一致性事故。

面试表达可以按四步讲：第一步给出业务分级，例如核心交易 RPO 30 秒、RTO 5 分钟，报表链路 RPO 1 小时、RTO 4 小时；第二步说明部署形态，例如同城多可用区承载常规故障，异地热备承载区域级故障；第三步说明切流顺序，先冻结高风险写入，再切入口流量，再恢复消费和后台任务；第四步说明回切条件，不能因为主区域恢复心跳就立刻回切，要确认数据追平、缓存预热、依赖可用和告警稳定。

反例也要主动讲：如果业务要求跨区域强同步写入，RPO 可以接近 0，但延迟、可用性和成本都会上升；如果使用异步复制，RPO 就不可能承诺 0，必须设计待核对、补偿和人工修复入口。公开文章能把这种边界写清楚，读者才不会误以为“多活”天然等于“永不丢数据”。
