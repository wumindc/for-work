# Prompt、Model、Config Registry 与灰度发布

## 面试定位

Prompt、Model、Config Registry 与灰度发布 属于 LLMOps、Eval 与 AI 质量工程 / Prompt / Model / Config Registry。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：Prompt、模型、解码参数、工具 schema 和安全配置都应像代码一样版本化、审批、灰度和回滚。
...
- prompt/model/config registry、diff、approval、canary、rollback and owner 要服务生产问题
- registry entry、config diff、release record 必须可版本化和可复盘
- 线上 prompt/config 漂移不可追溯 要有门禁和降级
...
图 1：Prompt、Model、Config Registry 与灰度发布 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### Prompt、Model、Config Registry 与灰度发布 的面试级设计题
...
Prompt、模型、解码参数、工具 schema 和安全配置都应像代码一样版本化、审批、灰度和回滚。 Prompt、Model、Config Registry 与灰度发布 是 AI 工程生产化能力的一部分，关注 prompt/model/config registry、diff、approval、canary、rollback and owner。 registry entry、config diff、release record 是团队复盘、验收和面试表达的核心证据。 线上 prompt/config 漂移不可追溯 是这个主题最容易被追问的生产风险。 生产 AI 系统要先定义可验证边界，再谈模型效果。 所有关键配置、数据、prompt、模型、工具和评测结果都要可追溯。 质量、延迟、成本、安全和用户体验要一起权衡，不能只优化单一指标。 失败样本要进入回归集，避免同类问题重复发生。
