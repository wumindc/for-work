...
二分查找、边界模板与答案空间二分 属于 算法题与编码面试 / 二分、排序与答案空间。面试里它不是背概念题，而是用来判断你是否能把知识落到架构、数据流、指标和取舍上。
一句话定位：二分题本质是利用单调性缩小搜索空间，既可以在有序数组中找边界，也可以在答案空间中找最小可行值或最大可行值。
...
图 1：二分查找、边界模板与答案空间二分 的回答要从业务入口进入，先讲边界和数据结构，再讲机制、失败模式、指标和取舍。

## 系统设计案例

### 二分查找、边界模板与答案空间二分 的面试级设计题
...
- Rotated array handling。
- 统一模板减少错，但要理解开闭区间。
- 答案二分易优化复杂题，但 check 设计是关键。
- 排序后二分可能改变原顺序，需要确认题意允许。
...
二分题本质是利用单调性缩小搜索空间，既可以在有序数组中找边界，也可以在答案空间中找最小可行值或最大可行值。 二分查找是在单调搜索空间中每次排除一半候选。 边界二分寻找满足条件的第一个或最后一个位置。 答案二分用可行性函数搜索最优答案。 先证明单调性。 区间语义全程不变。 check(mid) 必须 O(n) 或更优。 返回值要和循环不变量一致。
